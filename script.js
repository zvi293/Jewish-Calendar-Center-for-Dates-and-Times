...
function resolveSeasonalPrayerContext() {
  ...
  const isAseretYemeiTeshuva = includesAny([
    "Rosh Hashana",
    "ראש השנה",
    "Yom Kippur",
    "יום כיפור",
  ...
  const isNoTachanun = includesAny([
    "Erev Yom Kippur",
    "ערב יום כיפור",
    "Tu BiShvat",
    "ט״ו בשבט",
    ...
    parts.push(
      sup(
        p("[אִם הַמְסֻבִּים שְׁלוֹשָׁה אוֹ יוֹתֵר אוֹמְרִים:]") +
          ...
          p(
            "וְאוֹמֵר הַמְזַמֵּן: בִּרְשׁוּת מַלְכָּא עִלָּאָה קַדִּישָׁא וּבִרְשׁוּת מוֹרַי וְרַבּוֹתַי וּבִרְשֽׁוּתְכֶֽם — נְבָרֵךְ (בַּעֲשָׂרָה: אֱלֹהֵֽינוּ) שֶׁאָכַֽלְנוּ מִשֶּׁלּוֹ:",
          ) +
          p(
...
function normalizePrayerRuleText(text) {
  return (text || "")
    .replace(/\u05BE/g, " ")
    .replace(/[\u0591-\u05BD\u05BF-\u05C7]/g, "")
    .replace(/\s+/g, " ")
...
function wrapShacharitSupplementRange(startEl, shouldContinue) {
  if (!startEl || startEl.closest(".prayer-supplement")) return;
  const parent = startEl.parentNode;
  const box = document.createElement("div");
  box.className = "prayer-supplement";
  parent.insertBefore(box, startEl);

  let current = startEl;
  while (current && shouldContinue(current)) {
...
function applyShacharitDisplayRules(html, context) {
  if (!html) return html;
  const wrapper = document.createElement("div");
  wrapper.innerHTML = html;

  const isAseret = !!context.isAseretYemeiTeshuva;
  const isFast = !!(context.isFastDay || context.isTaanit);

  wrapper.querySelectorAll("small").forEach((small) => {
    const text = normalizePrayerRuleText(small.textContent);
    if (!isAseret && /בעשרת ימי תשובה|בעשי["״]?ת/.test(text)) {
      small.remove();
      return;
    }
    const isAseretAddition =
      isAseret &&
      (text.includes("זכרנו לחיים") ||
        text.includes("מי כמוך אב הרחמן") ||
        text.includes("המלך הקדוש") ||
        text.includes("המלך המשפט") ||
        text.includes("וכתב לחיים טובים") ||
        text.includes("ובספר חיים") ||
        text.includes("השלום במרומיו"));
    if (isAseretAddition) {
      small.classList.add("prayer-supplement");
    }
  });

  wrapper.querySelectorAll("p").forEach((paragraph) => {
    const text = normalizePrayerRuleText(paragraph.textContent);
    if (!text) return;

    const regularPrayerBody =
      text.includes("ויושע יהוה ביום ההוא") ||
      text.includes("אז ישיר משה") ||
      text.includes("שירת הים") ||
      text.includes("שיר של יום") ||
      text.includes("יתגדל ויתקדש שמה רבא");
    if (regularPrayerBody) {
      paragraph.classList.remove("prayer-supplement");
    }

    const isFastAddition =
      text.includes("בתענית אומר היחיד עננו") ||
      text.includes("נוסח עננו בג") ||
      text.includes("בתענית ציבור השליח ציבור אומר בחזרה");
    if (isFastAddition) {
      if (isFast) paragraph.classList.add("prayer-supplement");
    ...
    const isAseretAddition =
      ...
      text.includes("ובספר חיים") ||
      text.includes("השלום במרומיו");
    if (isAseretAddition && !isAseret) {
      paragraph.innerHTML = paragraph.innerHTML
        .replace(
          /\s*<small[^>]*>[^<]*(?:בעשרת ימי תשובה|בעשי["״]?ת)[\s\S]*?<\/small>\s*/g,
          " ",
        )
        .replace(/\(\s*\)/g, "")
        .replace(/\s{2,}/g, " ")
  ...
  wrapper.querySelectorAll("div.prayer-supplement").forEach((box) => {
    const text = normalizePrayerRuleText(box.textContent);
    if (!isFast && text.includes("בתענית ציבור השליח ציבור אומר בחזרה")) {
      box.remove();
    }
  });
  ...
  Array.from(wrapper.querySelectorAll("p")).forEach((paragraph) => {
    const text = normalizePrayerRuleText(paragraph.textContent);
    if (text.startsWith("ואומר החזן והכהנים אחריו מילה במילה")) {
      wrapShacharitSupplementRange(paragraph, (el) => {
        const currentText = normalizePrayerRuleText(el.textContent);
        return currentText && !currentText.startsWith("שים שלום");
      });
    }
  });
  ...
  wrapper.querySelectorAll("p, div").forEach((el) => {
    if (
      !el.textContent.trim() &&
      !el.querySelector("img, hr") &&
      !el.classList.contains("prayer-divider")
    ) {
      el.remove();
    }
  });

  let processed = wrapper.innerHTML;
  if (!isAseret) {
    processed = processed
      .replace(/\(\s*הַשָּׁלוֹם\s*\)/g, "")
      .replace(/\(\s*וּלְ֒עֵֽלָּא מִכָּל\s*\)/g, "")
      .replace(/\s{2,}/g, " ");
  }
  return processed;
}
function buildShacharitMizrahiPayload(context) {
  ...
  parts.push(
    p(
      "<b>מוֹדֶה</b> <small>האשה אומרת: מוֹדָה</small> אֲנִי לְפָנֶיךָ מֶלֶךְ חַי וְקַיָּם שֶהֶחֱזַרְתָּ בִּי נִשְׁמָתִי בְחֶמְלָה, רַבָּה אֱמוּנָתֶךָ: ",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p(
      '<small>יעשה צרכיו תחלה, ואחר כך יברך על נטילת ידים ואשר יצר (בא"ח ויצא ה"ב). אחר שנטל ידיו תכף ומיד תזקיף ידיו ואצבעותיו למעלה ותגביהם עד הראש, ולפחות תגביהם עד כנגד הפנים, ותברך על נטילת ידים, <small>(בא"ח תולדות ה"ד) ואם היה ער כל הלילה לא יברך אלא יטול בלא ברכה (שם הי"ג)</small></small> ',
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ עַל נְטִילַת יָדָיִם: ",
    ),
  );
  parts.push(
    p(
      '<small>אם הטיל מים או עשה צרכיו יברך תוך חצי שעה (בא"ח ויצא הי"ב)</small> ',
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אֲשֶׁר יָצַר אֶת הָאָדָם בְּחָכְמָה, וּבָרָא בוֹ נְקָבִים נְקָבִים, חֲלוּלִים חֲלוּלִים, גָּלוּי וְיָדוּעַ לִפְנֵי כִסֵּא כְבוֹדֶךָ, שֶׁאִם יִסָּתֵם אֶחָד מֵהֶם, אוֹ אִם יִפָּתֵח אֶחָד מֵהֶם, אֵי אֶפְשָׁר לְהִתְקַיֵּם אַפִלּוּ שָׁעָה אֶחָת. בָּרוּךְ אַתָּה יְהֹוָה, רוֹפֵא כָל־בָּשָׂר וּמַפְלִיא לַעֲשׂוֹת: ",
    ),
  );
  parts.push(
    p(
      '<small>יסמוך ברכה זו של אלהי וכו\' לברכת אשר יצר (בא"ח וישב ה"ב)</small> ',
    ),
  );
  parts.push(
    p(
      "<b>אֱלֹהַי</b>, נְשָׁמָה שֶׁנָּתַתָּ בִּי טְהוֹרָה, אַתָּה בְרָאתָהּ, אַתָּה יְצַרְתָּהּ, אַתָּה נְפַחְתָּהּ בִּי, וְאַתָּה מְשַׁמְּרָהּ בְּקִרְבִּי, וְאַתָּה עָתִיד לִטְּלָהּ מִמֶּנִּי, וּלְהַחֲזִירָהּ בִּי לֶעָתִיד לָבוֹא, כָּל־זְמַן שֶׁהַנְּשָׁמָה בְקִרְבִּי, מוֹדֶה <small>(<small>האשה אומרת</small> מוֹדָה)</small> אֲנִי לְפָנֶיךָ יְהֹוָה אֱלֹהַי וֵאלֹהֵי אֲבוֹתַי, רִבּוֹן כָּל־הַמַּעֲשִׂים אֲדוֹן כָּל־הַנְּשָׁמוֹת. בָּרוּךְ אַתָּה יְהֹוָה, הַמַּחֲזִיר נְשָׁמוֹת לִפְגָרִים מֵתִים: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, הַנוֹתֵן לַשֶּׂכְוִי בִינָה, לְהַבְחִין בֵּין יוֹם וּבֵין לָיְלָה: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, פּוֹקֵחַ עִוְרִים: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, מַתִּיר אֲסוּרִים: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, זוֹקֵף כְּפוּפִים: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, מַלְבִּישׁ עֲרֻמִּים: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, הַנּוֹתֵן לַיָּעֵף כֹּחַ: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, רוֹקַע הָאָרֶץ עַל הַמָּיִם: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, הַמֵּכִין מִצְעֲדֵי גָבֶר: ",
    ),
  );
  parts.push(
    p(
      '<small>בתשעה באב ויום הכיפורים אין אומרים ברכה זו <small>(בא"ח וישב ה"ט)</small></small> ',
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, שֶׁעָשָׂה לִי כָּל־צָרְכִּי: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אוֹזֵר יִשְׂרָאֵל בִּגְבוּרָה: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, עוֹטֵר יִשְׂרָאֵל בְּתִפְאָרָה: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, שֶׁלֹּא עָשַׂנִי גוֹי: <small>האשה אומרת: <b>גּוֹיָה</b></small> ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, שֶׁלֹּא עָשַׂנִי עָבֶד: <small>האשה אומרת: <b>שִׁפְחָה</b></small> ",
    ),
  );
  parts.push(p("<small>האיש מברך</small> "));
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהוָֹה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, שֶׁלֹּא עָשַׂנִי אִשָּׁה: ",
    ),
  );
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ עַל דִּבְרֵי תוֹרָה: ",
    ),
  );
  parts.push(
    p(
      "<b>וְהַעֲרֵב</b> נָא, יְהֹוָה אֱלֹהֵֽינוּ, אֶת־דִּבְרֵי תוֹרָתְךָ בְּפִינוּ וּבְפִיפִיּוֹת עַמְּךָ בֵּית יִשְׂרָאֵל, וְנִהְיֶה אֲנַחְנוּ וְצֶאֱצָאֵינוּ, וְצֶאֱצָאֵי צֶאֱצָאֵינוּ, כֻּלָּנוּ יוֹדְעֵי שְׁמֶךָ, וְלוֹמְדֵי תוֹרָתְךָ לִשְׁמָהּ. בָּרוּךְ אַתָּה יְהֹוָה, הַמְלַמֵּד תּוֹרָה לְעַמּוֹ יִשְׂרָאֵל: ",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אֲשֶׁר בָּחַר בָּנוּ מִכָּל־הָעַמִּים וְנָתַן לָנוּ אֶת־תּוֹרָתוֹ. בָּרוּךְ אַתָּה יְהֹוָה נוֹתֵן הַתּוֹרָה: ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      '<small>אחר המזמורים של תיקון לאה המנהג לומר פתיחת אליהו <small>(בא"ח וישלח הי"ב)</small></small> ',
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לְהִתְעַטֵּף בְּצִיצִית: ",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p("<small>ואומר החזן:</small> בָּרְכוּ אֶת יְהֹוָה הַמְּבֹרָךְ: "),
  );
  parts.push(
    p(
      "<small>ועונים הקהל:</small> בָּרוּךְ יְהֹוָה הַמְּבֹרָךְ לְעוֹלָם וָעֵד: ",
    ),
  );
  parts.push(
    p(
      "<small>ואומר החזן:</small> בָּרוּךְ יְהֹוָה הַמְּבֹרָךְ לְעוֹלָם וָעֵד: ",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p(
      "<small><small>בלחש:</small> בָּרוּךְ, שֵׁם כְּבוֹד מַלְכוּתוֹ, לְעוֹלָם וָעֶד:</small> ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p("<b>אֲ֭דֹנָי</b> שְׂפָתַ֣י תִּפְתָּ֑ח וּ֝פִ֗י יַגִּ֥יד תְּהִלָּתֶֽךָ׃"),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>אַתָּה</b> חוֹנֵן לְאָדָם דַּֽעַת וּמְלַמֵּד לֶאֱנוֹשׁ בִּינָה. וְחָנֵּֽנוּ מֵאִתְּךָ חָכְמָה בִּינָה וָדָֽעַת: בָּרוּךְ אַתָּה יַהַוַהַ, חוֹנֵן הַדָּֽעַת:",
    ),
  );
  parts.push(
    p(
      "<b>הֲשִׁיבֵֽנוּ</b> אָבִֽינוּ לְתֽוֹרָתֶֽךָ, וְקָֽרְבֵֽנוּ מַלְכֵּֽנוּ לַֽעֲבֽוֹדָתֶֽךָ, וְהַֽחֲזִירֵֽנוּ בִּתְשׁוּבָה שְׁלֵמָה לְפָנֶֽיךָ: בָּרוּךְ אַתָּה יֵהֵוֵהֵ, הָרוֹצֶה בִּתְשׁוּבָה:",
    ),
  );
  parts.push(
    p(
      "<b>סְלַח</b> לָֽנוּ אָבִֽינוּ כִּי חָטָֽאנוּ, מְחוֹל לָֽנוּ מַלְכֵּֽנוּ כִּי פָשָֽׁעְנוּ, כִּי אֵל טוֹב וְסַלָּח אָֽתָּה: בָּרוּךְ אַתָּה יֶהֶוֶהֶ, חַנּוּן הַמַּרְבֶּה לִסְלֹחַ:",
    ),
  );
  parts.push(
    p(
      "<b>רְאֵה</b> נָא בְעָנְיֵֽנוּ, וְרִיבָֽה רִיבֵֽנוּ, וּמַהֵר לְגָאֳלֵֽנוּ גְאוּלָּה שְׁלֵמָה לְמַֽעַן שְׁמֶֽךָ, כִּי אֵל גּוֹאֵל חָזָק אָֽתָּה: בָּרוּךְ אַתָּה יְהְוְהְ, גּוֹאֵל יִשְׂרָאֵל:",
    ),
  );
    ...
    parts.push(
      sup(
        p("<small>בתענית ציבור השליח ציבור אומר בחזרה</small>") +
          p(
            "<small><b>עֲנֵֽנוּ</b> אָבִינוּ עֲנֵֽנוּ בְּיוֹם צוֹם הַתַּֽעֲנִית הַזֶּה, כִּי בְצָרָה גְדוֹלָה אֲנָֽחְנוּ. אַל־תֵּֽפֶן לְרִשְׁעֵֽנוּ, וְאַל־תִּתְעַלָּם מַלְכֵּֽנוּ מִבַּקָּשָׁתֵֽנוּ. הֱיֵה נָא קָרוֹב לְשַׁוְעָתֵֽנוּ, טֶֽרֶם נִקְרָא אֵלֶֽיךָ אַתָּה תַעֲנֶה, נְדַבֵּר וְאַתָּה תִשְׁמַע, כַּדָּבָר שֶׁנֶּאֱמַר: וְהָיָ֥ה טֶֽרֶם־יִקְרָ֖אוּ וַאֲנִ֣י אֶעֱנֶ֑ה ע֛וֹד הֵ֥ם מְדַבְּרִ֖ים וַאֲנִ֥י אֶשְׁמָֽע: כִּי אַתָּה יְהֹוָה פּוֹדֶה וּמַצִּיל, וְעוֹנֶה וּמְרַחֵם בְּכָל־עֵת צָרָה וְצוּקָה:</small>",
          ) +
          p(
            "<small>בָּרוּךְ אַתָּה יְהֹוָה הָעוֹנֶה לְעַמּוֹ יִשְׂרָאֵל בְּעֵת צָרָה:</small>",
          ),
      ),
    );
  }
  parts.push(
    p(
      "<b>רְפָאֵֽנוּ</b> יְהֹוָה וְנֵֽרָפֵא, הֽוֹשִׁיעֵֽנוּ וְנִוָּשֵֽׁעָה, כִּי תְהִלָּתֵֽנוּ אָֽתָּה, וְהַֽעֲלֵה אֲרוּכָה וּמַרְפֵּא לְכָל־תַּֽחֲלוּאֵֽינוּ וּלְכָל־מַכְאוֹבֵֽינוּ וּלְכָל־מַכּוֹתֵֽינוּ. כִּי אֵל רוֹפֵא רַחְמָן וְנֶֽאֱמָן אָֽתָּה: בָּרוּךְ אַתָּה יֹהֹוֹהֹ, רוֹפֵא חוֹלֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  if (isSummer) {
  ...
  parts.push(
    p(
      "<b>תְּקַע</b> בְּשׁוֹפָר גָּדוֹל לְחֵֽרוּתֵֽנוּ, וְשָׂא נֵס לְקַבֵּץ גָּֽלֻיּוֹתֵֽינוּ, וְקַבְּצֵֽנוּ יַֽחַד מֵאַרְבַּע כַּנְפוֹת הָאָֽרֶץ לְאַרְצֵֽנוּ: בָּרוּךְ אַתָּה יֻהֻוֻהֻ, מְקַבֵּץ נִדְחֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>לַמִּינִים</b> וְלַמַּלְשִׁינִים אַל־תְּהִי תִקְוָה, וְכָל־הַזֵּדִים כְּרֶֽגַע יֹאבֵֽדוּ, וְכָל־אֽוֹיְבֶֽיךָ וְכָל־שֽׂוֹנְאֶֽיךָ מְהֵרָה יִכָּרֵֽתוּ, וּמַלְכוּת הָֽרִשְׁעָה מְהֵרָה תְעַקֵּר וּתְשַׁבֵּר וּתְכַלֵּם וְתַכְנִיעֵם בִּמְהֵרָה בְיָמֵֽינוּ: בָּרוּךְ אַתָּה יָהָוָהָ, שׁוֹבֵר אוֹיְבִים וּמַכְנִֽיעַ מִינִים:",
    ),
  );
  parts.push(
    p(
      "<b>עַל</b> הַצַּדִּיקִים וְעַל הַֽחֲסִידִים, וְעַל שְׁאֵרִית עַמְּךָ בֵּית יִשְׂרָאֵל, וְעַל פְּלֵיטַת בֵּית סֽוֹפְרֵיהֶם, וְעַל גֵּרֵי הַצֶּֽדֶק וְעָלֵֽינוּ, יֶֽהֱמוּ נָא רַֽחֲמֶֽיךָ, יְהֹוָה אֱלֹהֵֽינוּ, וְתֵן שָׂכָר טוֹב לְכָל־הַבּֽוֹטְחִים בְּשִׁמְךָ בֶּֽאֱמֶת, וְשִׂים חֶלְקֵֽנוּ עִמָּהֶם. וּלְעוֹלָם לֹא נֵבוֹשׁ כִּי בְךָ בָטָֽחְנוּ, וְעַל חַסְדְּךָ הַגָּדוֹל בֶּֽאֱמֶת נִשְׁעָֽנְנוּ: בָּרוּךְ אַתָּה יוּהוּווּהוּ, מִשְׁעָן וּמִבְטָח לַצַּדִּיקִים:",
    ),
  );
  parts.push(
    p(
      "<b>תִּשְׁכּוֹן</b> בְּתוֹךְ יְרוּשָׁלַֽיִם עִֽירְךָ כַּאֲשֶׁר דִּבַּֽרְתָּ, וְכִסֵּא דָוִד עַבְדְּךָ מְהֵרָה בְּתוֹכָהּ תָּכִין, וּבְנֵה אוֹתָהּ בִּנְיַן עוֹלָם בִּמְהֵרָה בְיָמֵֽינוּ: בָּרוּךְ אַתָּה יֻהֻוֻהֻ, בּוֹנֵה יְרוּשָׁלָיִם: ",
    ),
  );
  parts.push(
    p(
      "<b>אֶת</b> צֶֽמַח דָּוִד עַבְדְּךָ מְהֵרָה תַצְמִֽיחַ, וְקַרְנוֹ תָּרוּם בִּישֽׁוּעָתֶֽךָ, כִּי לִֽישׁוּעָֽתְךָ קִוִּֽינוּ כָּל־הַיּוֹם: בָּרוּךְ אַתָּה יִהִוִהִ, מַצְמִֽיחַ קֶֽרֶן יְשׁוּעָה:",
    ),
  );
  parts.push(
    p(
      "<b>שְׁמַע</b> קוֹלֵֽנוּ, יְהֹוָה אֱלֹהֵֽינוּ, אָב הָרַֽחֲמָן, רַחֵם עָלֵֽינוּ, וְקַבֵּל בְּרַֽחֲמִים וּבְרָצוֹן אֶת־תְּפִלָּתֵֽנוּ, כִּי אֵל שׁוֹמֵֽעַ תְּפִלּוֹת וְתַֽחֲנוּנִים אָֽתָּה. וּמִלְּפָנֶֽיךָ מַלְכֵּֽנוּ, רֵיקָם אַל־תְּשִׁיבֵֽנוּ, חָנֵּֽנוּ וַֽעֲנֵֽנוּ וּשְׁמַע תְּפִלָּתֵֽנוּ. ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>רְצֵה</b> יְהֹוָה אֱלֹהֵֽינוּ בְּעַמְּךָ יִשְׂרָאֵל וְלִתְפִלָּתָם שְׁעֵה, וְהָשֵׁב הָֽעֲבוֹדָה לִדְבִיר בֵּיתֶֽךָ, וְאִשֵּׁי יִשְׂרָאֵל וּתְפִלָּתָם, מְהֵרָה בְּאַֽהֲבָה תְקַבֵּל בְּרָצוֹן, וּתְהִי לְרָצוֹן תָּמִיד עֲבוֹדַת יִשְׂרָאֵל עַמֶּֽךָ: ",
    ),
  );
  if (context.isRoshChodesh || context.isPesach || context.isSukkot) {
    var yv =
      p("<small>בראש חודש ובחול המועד אומרים:</small>") +
      p(
        "<small>אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ, יַעֲלֶה וְיָבֹא, וְיַגִּֽיעַ וְיֵרָאֶה, וְיֵרָצֶה וְיִשָּׁמַע, וְיִפָּקֵד וְיִזָּכֵר, זִכְרוֹנֵֽנוּ וְזִכְרוֹן אֲבוֹתֵֽינוּ, זִכְרוֹן יְרוּשָׁלַֽיִם עִירָךְ, וְזִכְרוֹן מָשִֽׁיחַ בֶּן־דָּוִד עַבְדָּךְ, וְזִכְרוֹן כָּל־עַמְּךָ בֵּית יִשְׂרָאֵל לְפָנֶֽיךָ, לִפְלֵיטָה, לְטוֹבָה, לְחֵן, לְחֶֽסֶד וּלְרַחֲמִים, לְחַיִּים טוֹבִים וּלְשָׁלוֹם, בְּיוֹם</small> ",
      ) +
      (context.isRoshChodesh
        ? p("<small><small>בראש חדש:</small> רֹאשׁ חֹֽדֶשׁ הַזֶּה,</small>")
        : "") +
      (context.isPesach
        ? p(
            '<small><small>בחוה"מ פסח:</small> חַג הַמַּצּוֹת הַזֶּה, בְּיוֹם מִקְרָא קֹֽדֶשׁ הַזֶּה,</small>',
          )
        : "") +
      (context.isSukkot
        ? p(
            '<small><small>בחוה"מ סוכות:</small> חַג הַסֻּכּוֹת הַזֶּה, בְּיוֹם מִקְרָא קֹֽדֶשׁ הַזֶּה,</small>',
          )
        : "") +
      p(
        "<small>לְרַחֵם בּוֹ עָלֵֽינוּ וּלְהוֹשִׁיעֵֽנוּ. זָכְרֵֽנוּ יְהֹוָה אֱלֹהֵֽינוּ בּוֹ לְטוֹבָה, וּפָקְדֵֽנוּ בוֹ לִבְרָכָה, וְהוֹשִׁיעֵֽנוּ בוֹ לְחַיִּים טוֹבִים, בִּדְבַר יְשׁוּעָה וְרַחֲמִים. חוּס וְחָנֵּֽנוּ, וַחֲמוֹל וְרַחֵם עָלֵֽינוּ, וְהוֹשִׁיעֵֽנוּ כִּי אֵלֶֽיךָ עֵינֵֽינוּ, כִּי אֵל מֶֽלֶךְ חַנּוּן וְרַחוּם אָֽתָּה:</small> ",
      );
    parts.push(sup(yv));
  }
  parts.push(
    p(
      "<b>וְאַתָּה</b> בְּרַֽחֲמֶֽיךָ הָרַבִּים, תַּחְפֹּץ בָּֽנוּ וְתִרְצֵֽנוּ, וְתֶֽחֱזֶֽינָה עֵינֵֽינוּ בְּשֽׁוּבְךָ לְצִיּוֹן בְּרַֽחֲמִים: בָּרוּךְ אַתָּה יִהִוִהִ, הַמַּֽחֲזִיר שְׁכִֽינָתוֹ לְצִיּוֹן:",
    ),
  );
  parts.push(
    p(
      '<small>בברכת "מודים" יכרע ב"מודים" ויזקוף בשם <small>(בא"ח בשלח הכ"א)</small></small>',
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    sup(
      p("ברכת כהנים ") +
        p(
          '<small><small>כשעוקר הכהן רגליו לישא כפיו יאמר (בא"ח תצוה י"ב)</small></small><br><small>לְשֵׁם יִחוּד קֻדְשָׁא בְּרִיךְ הוּא וּשְׁכִינְתֵּיהּ. בִּדְחִֽילוּ וּרְחִֽימוּ. וּרְחִֽימוּ וּדְחִֽילוּ. לְיַחֲדָא שֵׁם יוֹ"ד קֵ"א בְּוָא"ו קֵ"א בְּיִחוּדָא שְׁלִים. בְּשֵׁם כָּל־יִשְׂרָאֵל הִנֵּה אָנֹכִי מוּכָן וּמְזוּמָן לְקַיֵּם מִצְוַת עֲשֵׂה לְבָרֵךְ אֶת יִשְׂרָאֵל בִּרְכַּת כֹּהַנִים בִּנְשִׂיאוּת כַּפַּיִם, לַעֲשׂוֹת נַֽחַת רֽוּחַ לְיֽוֹצְרֵֽנוּ וּלְהַמְשִׁיךְ שֶׁפַע וּבְרָכָה לְכָל־הָעוֹלָמוֹת וְלַעֲשׂוֹת רְצוֹן בּֽוֹרְאֵֽנוּ. וִיהִ֤י ׀ נֹ֤עַם אֲדֹנָ֥י אֱלֹהֵ֗ינוּ עָ֫לֵ֥ינוּ וּמַעֲשֵׂ֣ה יָ֭דֵינוּ כּוֹנְנָ֥ה עָלֵ֑ינוּ וּֽמַעֲשֵׂ֥ה יָ֝דֵ֗ינוּ כּוֹנְנֵֽהוּ: </small>',
        ) +
        p(
          '<small><small>וכשעומדים הכהנים על הדוכן לאחר שענו "מודים דרבנן", אומרים</small></small><br><small>יְהִי רָצוֹן מִלְּפָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁתִּהְיֶה בְּרָכָה זוֹ שֶׁצִּּוִיתָֽנוּ לְבָרֵךְ אֶת־עַמְּךָ יִשְׂרָאֵל בְּרָכָה שְׁלֵמָה וְלֹא יִהְיֶה בָהּ מִכְשׁוֹל וְעָוֹן מֵעַתָּה וְעַד עוֹלָם:</small>',
        ) +
        p(
          '<small>בחזרת הש"ץ אומרים ברכת כהנים, אם יש יותר מכהן אחד קורא להם החזן: כֹּהֲנִים: הכהנים מברכים:</small><br><small>בָּרוּךְ אַתָּה יְהֹוָה, אֱלֹהֵֽינוּ מֶֽלֶךְ הָֽעוֹלָם, אֲשֶׁר קִדְּשָֽׁנוּ בִּקְדֻשָּׁתוֹ שֶׁל־אַֽהֲרֹן, וְצִוָּֽנוּ לְבָרֵךְ אֶת־עַמּוֹ יִשְׂרָאֵל בְּאַֽהֲבָה: <small>הקהל עונים</small> אָמֵן:</small>',
        ),
    ),
  );
  ...
  parts.push(
    p(
      "<small><b>אֱלֹהֵינוּ</b> וֵאלֹהֵי אֲבוֹתֵינוּ, בָּרְכֵנוּ בַּבְּרָכָה הַמְשֻׁלֶּשֶת בַּתּוֹרָה הַכְּתוּבָה עַל יְדֵי מֹשֶׁה עַבְדֶּֽךָ, הָאֲמוּרָה מִפִּי אַהֲרֹן וּבָנָיו הַכֹּהֲנִים עַם קְדוֹשֶׁיךָ כָּאָמוּר:</small>",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      '<small>כשמתחיל שליח ציבור "שים שלום" מחזירין כהנים פניהם כלפי ההיכל ואומרים:</small>',
    ),
  );
  parts.push(
    p(
      "<small>רִבּוֹן הָעוֹלָמִים עָשִֽׂינוּ מַה־שֶּׁגָּזַֽרְתָּ עָלֵֽינוּ, עַשֵׂה אַתָּה מַה־שֶּׁהִבְטָחְתָּֽנוּ. הַשְׁקִ֩יפָה֩ מִמְּע֨וֹן קׇדְשְׁךָ֜ מִן־הַשָּׁמַ֗יִם וּבָרֵ֤ךְ אֶֽת־עַמְּךָ֙ אֶת־יִשְׂרָאֵ֔ל:</small>",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<small>יְהִי רָצוֹן מִלְּפָנֶֽיךָ, יְהֹוָה אֱלֹהֵֽינוּ וֵֽאלֹהֵי אֲבוֹתֵֽינוּ, שֶׁתִּבְנֶה בֵּית הַמִּקְדָשׁ בִּמְהֵרָה בְיָמֵֽינוּ, וְתֵן חֶלְקֵֽנוּ בְתוֹרָתָךְ לַעֲשׂוֹת חֻקֵּי רְצוֹנָךְ וּלְעָבְדָךְ בְּלֵבָב שָׁלֵם:</small>",
    ),
  );
  if (isHallelDay) {
  ...
  if (context.isAseretYemeiTeshuva) {
    ...
    am_parts.push(
      p(
        "<small><b>אָבִינוּ מַלְכֵּנוּ</b> חָטָאנוּ לְפָנֶיךָ רַחֵם עָלֵינוּ:</small>",
      ),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ אֵין לָנוּ מֶלֶךְ אֶלָּא אָתָּה:</small>"),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ עֲשֵׂה עִמָּנוּ לְמַעַן שְׁמֶךָ:</small>"),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ חַדֵּשׁ עָלֵינוּ שָׁנָה טוֹבָה:</small>"),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ בַּטֵּל מֵעָלֵינוּ כָּל־גְּזֵרוֹת קָשׁוֹת וְרָעוֹת:</small>",
      ),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ בַּטֵּל מַחְשְׁבוֹת שֹׂנְאֵינוּ:</small>"),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ הָפֵר עֲצַת אוֹיְבֵינוּ:</small>"),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כַּלֵּה כָּל צַר וּמַשְׂטִין מֵעָלֵינוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כַּלֵּה דֶבֶר וְחֶרֶב וְרָעָה וְרָעָב וּשְׁבִי וּבִזָּה וּמַשְׁחִית וּמַגֵּפָה וְיֵצֶר הָרָע וְחוֹלָאִים רָעִים מִבְּנֵי בְרִיתֶךָ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ שְׁלַח רְפוּאָה שְׁלֵמָה לְכָל חוֹלֵי עַמֶּךָ:</small>",
      ),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ מְנַע מַגֵּפָה מִנַּחֲלָתֶךָ:</small>"),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ זָכוּר כִּי עָפָר אֲנָחְנוּ:</small>"),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ מְחוֹל וּסְלַח לְכָל עֲוֹנוֹתֵינוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        '<small>אָבִינוּ מַלְכֵּנוּ קְרַע רוֹעַ גְּזַר דִּינֵנוּ: <small>(יכוין בשם קר"ע שט"ן)</small></small>',
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ מְחוֹק בְּרַחֲמֶיךָ הָרַבִּים כָּל שִׁטְרֵי חוֹבוֹתֵינוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ מְחֵה וְהַעֲבֵר פְּשָׁעֵינוּ מִנֶּגֶד עֵינֶיךָ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר חַיִּים טוֹבִים:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר צַדִּיקִים וַחֲסִידִים:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר יְשָׁרִים וּתְמִימִים:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר פַּרְנָסָה וְכַלְכָּלָה טוֹבָה:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר מְחִילָה וּסְלִיחָה וְכַפָּרָה:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ כָּתְבֵנוּ בְּסֵפֶר גְּאֻלָּה וִישׁוּעָה:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ זָכְרֵנוּ בְּזִכְרוֹן טוֹב מִלְּפָנֶיךָ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ הַצְמַח לָנוּ יְשׁוּעָה בְּקָרוֹב:</small>",
      ),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ הָרֵם קֶרֶן יִשְׂרָאֵל עַמֶּךָ:</small>"),
    );
    am_parts.push(
      p("<small>אָבִינוּ מַלְכֵּנוּ וְהָרֵם קֶרֶן מְשִׁיחֶךָ:</small>"),
    );
    am_parts.push(p("<small>אָבִינוּ מַלְכֵּנוּ חָנֵּנוּ וַעֲנֵנוּ:</small>"));
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ הַחֲזִירֵנוּ בִּתְשׁוּבָה שְׁלֵמָה לְפָנֶיךָ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ שְׁמַע קוֹלֵנוּ חוּס וְרַחֵם עָלֵינוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ עֲשֵׂה לְמַעֲנָךְ אִם לֹא לְמַעֲנֵנוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ קַבֵּל בְּרַחֲמִים וּבְרָצוֹן אֶת תְּפִלָּתֵנוּ:</small>",
      ),
    );
    am_parts.push(
      p(
        "<small>אָבִינוּ מַלְכֵּנוּ אַל תְּשִׁיבֵנוּ רֵיקָם מִלְּפָנֶיךָ:</small>",
      ),
    );
  ...
  if (!isTachanunDay) {
    ...
    parts.push(
      p(
        "<b>יְהִ֤י</b> שֵׁ֣ם יְהֹוָ֣ה מְבֹרָ֑ךְ מֵ֝עַתָּ֗ה וְעַד־עוֹלָֽם׃ מִמִּזְרַח־שֶׁ֥מֶשׁ עַד־מְבוֹא֑וֹ מְ֝הֻלָּ֗ל שֵׁ֣ם יְהֹוָֽה׃ רָ֖ם עַל־כׇּל־גּוֹיִ֥ם ׀ יְהֹוָ֑ה עַ֖ל הַשָּׁמַ֣יִם כְּבוֹדֽוֹ׃ יְהֹוָ֥ה אֲדֹנֵ֑ינוּ מָה־אַדִּ֥יר שִׁ֝מְךָ֗ בְּכׇל־הָאָֽרֶץ׃<br><br><small>ואחר כך אומר הש''צ חצי קדיש</small>",
      ),
    );
  }
  parts.push(hr);
  ...
  if (isTachanunDay) {
    ...
    vd.push(
      p(
        '<small>בתעניות ציבור אומרים כאן תחנונים נוספים שנמצאים ב"תעניות ואבילות"</small> ',
      ),
    );
    if (!isMonThu) {
      vd.push(
        p(
          "<small>בימים שני וחמישי אין אומרים את הקדיש הזה וממשיכים באל מלך</small>",
        ),
      );
      vd.push(
        p(
      ...
      vd.push(
        p(
          "<b>אֶל מֶלֶךְ</b> יוֹשֵׁב עַל כִּסֵּא רַחֲמִים וּמִתְנַהֵג בַּחֲסִידוּת, מוֹחֵל עֲוֹנוֹת עַמּוֹ, מַעֲבִיר רִאשׁוֹן רִאשׁוֹן. מַרְבֶּה מְחִילָה לַחַטָּאִים, וּסְלִיחָה לַפּוֹשְׁעִים, עוֹשֶׂה צְדָקוֹת עִם כָּל בָּשָׂר וְרוּחַ, לֹא כְרָעָתָם לָהֶם גּוֹמֵל, אֶל, הוֹרֵתָנוּ לוֹמַר מִדּוֹת שְׁלֹשׁ עֶשְׂרֵה, זְכוֹר לָנוּ הַיּוֹם בְּרִית שְׁלֹשׁ עֶשְׂרֵה כְּמוֹ שֶׁהוֹדַעְתָּ לֶעָנָו מִקֶּדֶם, וְכֵן כָּתוּב בְּתוֹרָתָךְ וַיֵּ֤רֶד יְהֹוָה֙ בֶּֽעָנָ֔ן וַיִּתְיַצֵּ֥ב עִמּ֖וֹ שָׁ֑ם וַיִּקְרָ֥א בְשֵׁ֖ם יְהֹוָֽה: וְשָׁם נֶאֱמַר:  ",
        ),
      );
      vd.push(
        p(
      ...
      vd.push(
        p(
          "<b>יְהִי</b> רָצוֹן מִלְּפָנֶיךָ יְהֹוָה אֱלֹהֵינוּ וֵאלֹהֵי אֲבוֹתֵינוּ, שֶׁתִּמְחֹל לָנוּ אֶת-כָּל-חַטֹּאתֵינוּ, וּתְכַפֵּר לָנוּ אֶת-כָּל-עֲוֹנוֹתֵינוּ, וְתִמְחֹל וְתִסְלַח לְכָל פְּשָׁעֵינוּ, וְסָלַחְתָּ֛ לַעֲוֺנֵ֥נוּ וּלְחַטָּאתֵ֖נוּ וּנְחַלְתָּֽנוּ:  ",
        ),
      );
      vd.push(
        p(
          "<b>סְלַח</b> לָנוּ אָבִינוּ כִּי חָטָאנוּ, מְחֹל לָנוּ מַלְכֵּנוּ כִּי פָשָׁעְנוּ: כִּֽי־אַתָּ֣ה אֲ֭דֹנָי ט֣וֹב וְסַלָּ֑ח וְרַב־חֶ֗֝סֶד לְכׇל־קֹֽרְאֶֽיךָ׃ לְמַֽעַן־שִׁמְךָ֥ יְהֹוָ֑ה וְֽסָלַחְתָּ֥ לַ֝עֲוֺנִ֗י כִּ֣י רַב־הֽוּא׃ לְמַעַן־שִׁמְךָ֣ יְהֹוָ֣ה תְּחַיֵּ֑נִי בְּצִדְקָתְךָ֓ ׀ תּוֹצִ֖יא מִצָּרָ֣ה נַפְשִֽׁי׃ יְהֹוָ֣ה צְבָא֣וֹת עִמָּ֑נוּ מִשְׂגָּֽב־לָ֨נוּ אֱלֹהֵ֖י יַֽעֲקֹ֣ב סֶֽלָה׃ יְהֹוָ֥ה צְבָא֑וֹת אַֽשְׁרֵ֥י אָ֝דָ֗ם בֹּטֵ֥חַ בָּֽךְ׃ יְהֹוָ֥ה הוֹשִׁ֑יעָה הַ֝מֶּ֗לֶךְ יַעֲנֵ֥נוּ בְיוֹם־קׇרְאֵֽנוּ: הֲשִׁיבֵ֨נוּ יְהֹוָ֤ה ׀ אֵלֶ֙יךָ֙ ונשוב [וְֽנָשׁ֔וּבָה] חַדֵּ֥שׁ יָמֵ֖ינוּ כְּקֶֽדֶם׃",
        ),
      );
      vd.push(
        p(
      ...
      vd.push(
        p(
          "<b>אֵין כָּמוֹךָ</b> חַנּוּן וְרַחוּם אֱלֹהֵינוּ. אֵין כָּמוֹךָ אֵל אֶרֶךְ אַפַּיִם וְרַב חֶסֶד וֶאֱמֶת. הושִׁיעֵנוּ וְרַחֲמֵנוּ מֵרַעַשׁ וּמֵרֹגֶז הַצִּילֵנוּ: זְכֹר֙ לַעֲבָדֶ֔יךָ לְאַבְרָהָ֥ם לְיִצְחָ֖ק וּֽלְיַעֲקֹ֑ב אַל־תֵּ֗פֶן אֶל־קְשִׁי֙ הָעָ֣ם הַזֶּ֔ה וְאֶל־רִשְׁע֖וֹ וְאֶל־חַטָּאתֽוֹ׃ שׁ֚וּב מֵחֲר֣וֹן אַפֶּ֔ךָ וְהִנָּחֵ֥ם עַל־הָרָעָ֖ה לְעַמֶּֽךָ׃ וְהָסֵר מִמֶּנּוּ מַכַּת הַמָּוֶת כִּי רַחוּם אָתָּה. כִּי כֵן דַּרְכְּךָ לַעֲשׂוֹת חֶסֶד חִנָּם בְּכָל דּוֹר וָדוֹר: אָנָּ֣א יְ֭הֹוָה הוֹשִׁ֘יעָ֥ה נָּ֑א אָנָּ֥א יְ֝הֹוָ֗ה הַצְלִ֘יחָ֥ה נָֽא׃ אָנָּא יְהֹוָה עֲנֵנוּ בְיוֹם קָרְאֵנוּ: לְךָ יְהֹוָה קִוִּינוּ. לְךָ יְהֹוָה חִכִּינוּ. לְךָ יְהֹוָה נְיַחֵל אַל תֶּחֱשֶׁה וּתְעַנֵּנוּ. כִּי נָאֲמוּ גוֹיִם אָבְדָה תִקְוָתָם. כָּל בֶּרֶךְ לְךָ תִכְרַע וְכָל קוֹמָה לְפָנֶיךָ תִשְׁתַּחֲוֶה:  ",
        ),
      );
      vd.push(
        p(
          "<b>הַפּוֹתֵחַ</b> יָד בִּתְשׁוּבָה לְקַבֵּל פּוֹשְׁעִים וְחַטָּאִים. נִבְהֲלָה נַפְשֵׁנוּ מֵרוֹב עִצְּבוֹנֵנוּ. אַל תִּשְׁכָּחֵנוּ נֶצַח קוּמָה וְהוֹשִׁיעֵנוּ. אַל תִּשְׁפּוֹךְ חֲרוֹנְךָ עָלֵינוּ כִּי עַמְּךָ אֲנַחְנוּ בְּנֵי בְרִיתֶךָ. אֵל הַבִּיטָה. דַּל כְּבוֹדֵנוּ בַגּוֹיִם וְשִׁקְּצוּנוּ כְּטֻמְאַת הַנִּדָּה. עַד מָתַי עֻזְּךָ בַּשְּׁבִי וְתִפְאַרְתְּךָ בְּיַד צָר. הֵמָּה יִרְאוּ וְיֵבשׁוּ וְיֵחַתּוּ מִגְּבוּרָתָם. עוֹרְרָה גְבוּרָתְךָ וְהוֹשִׁיעֵנוּ לְמַעַן שְׁמֶךָ. אַל יִמְעֲטוּ לְפָנֶיךָ תְּלָאוֹתֵינוּ מַהֵר יְקַדְּמוּנוּ רַחֲמֶיךָ בְּעֵת צָרוֹתֵנוּ. לֹא לְמַעֲנֵנוּ אֶלָּא לְמַעֲנָךְ פְּעֹל וְאַל תַּשְׁחֵת אֶת זֵכֶר שְׁאֵרִיתֵנוּ. כִּי לְךָ מְיַחֲלוֹת עֵינֵינוּ כִּי אֵל מֶלֶךְ חַנּוּן וְרַחוּם אָתָּה. וּזְכוֹר עֵדוּתֵינוּ בְּכָל יוֹם תָּמִיד אוֹמְרִים פַּעֲמַיִם בְּאַהֲבָה: שְׁמַ֖ע יִשְׂרָאֵ֑ל יְהֹוָ֥ה אֱלֹהֵ֖ינוּ יְהֹוָ֥ה ׀ אֶחָֽד׃",
        ),
      );
    }
    vd.push(
  ...
  if (isTorahReadingDay) {
    ...
    tr.push(
      p(
        "<b>אֶל</b> אֶרֶךְ אַפַּיִם וְרַב חֶסֶד וֶאֱמֶת, אֶל בְּאַפְּךָ תוֹכִיחֵנוּ, חוּסָה יְהֹוָה עַל יִשְׂרָאֵל עַמֶּךָ, וְהוֹשִׁיעֵנוּ מִכָּל רָע, חָטָאנוּ לְךָ, אָדוֹן סְלַח נָא, כְּרֹב רַחֲמֶיךָ אֶל:",
      ),
    );
    tr.push(
      p(
        "<b>אֶל</b> אֶרֶךְ אַפַּיִם וּמַלֵּא רַחֲמִים, אֶל תַּסְתֵּר פָּנֶיךָ מִמֶּנּוּ, חוּסָה יְהֹוָה עַל שְׁאֵרִית יִשְׂרָאֵל עַמֶּךָ, וְהַצִּילֵנוּ מִכָּל רָע, חָטָאנוּ לְךָ, אֲדוֹן סְלַח נָא, כְּרֹב רַחֲמֶיךָ אֶל:  ",
      ),
    );
    tr.push(p("<small>ביום שאין בו תחנון אומרים:</small> "));
    tr.push(
      p(
        "<b>יְהִ֨י</b> יְהֹוָ֤ה אֱלֹהֵ֙ינוּ֙ עִמָּ֔נוּ כַּאֲשֶׁ֥ר הָיָ֖ה עִם־אֲבֹתֵ֑ינוּ אַל־יַעַזְבֵ֖נוּ וְאַֽל־יִטְּשֵֽׁנוּ׃ הוֹשִׁ֤יעָה ׀ אֶת־עַמֶּ֗ךָ וּבָרֵ֥ךְ אֶת־נַחֲלָתֶ֑ךָ וּֽרְעֵ֥ם וְ֝נַשְּׂאֵ֗ם עַד־הָעוֹלָֽם׃ בַּֽ֭עֲבוּר דָּוִ֣ד עַבְדֶּ֑ךָ אַל־תָּ֝שֵׁ֗ב פְּנֵ֣י מְשִׁיחֶֽךָ׃",
      ),
    );
    tr.push(
      p(
        "<small>שמוציאים ספר תורה אומרים</small><br><b>בָּרוּךְ</b> הַמָּקוֹם שֶׁנָּתַן תּוֹרָה לְעַמּוֹ יִשְׂרָאֵל, בָּרוּךְ הוּא: אַשְׁרֵ֣י הָ֭עָם שֶׁכָּ֣כָה לּ֑וֹ אַֽשְׁרֵ֥י הָ֝עָ֗ם שֱׁיְהֹוָ֥ה אֱלֹהָֽיו׃ גַּדְּל֣וּ לַיהֹוָ֣ה אִתִּ֑י וּנְרוֹמְמָ֖ה שְׁמ֣וֹ יַחְדָּֽו׃ רוֹמְמ֡וּ יְהֹ֘וָ֤ה אֱלֹהֵ֗ינוּ וְֽ֭הִשְׁתַּחֲווּ לַהֲדֹ֥ם רַגְלָ֗יו קָד֥וֹשׁ הֽוּא׃ רוֹמְמ֡וּ יְהֹ֘וָ֤ה אֱלֹהֵ֗ינוּ וְֽ֭הִשְׁתַּחֲווּ לְהַ֣ר קׇדְשׁ֑וֹ כִּי־קָ֝ד֗וֹשׁ יְהֹוָ֥ה אֱלֹהֵֽינוּ׃ אֵין־קָד֥וֹשׁ כַּיהֹוָ֖ה כִּ֣י אֵ֣ין בִּלְתֶּ֑ךָ וְאֵ֥ין צ֖וּר כֵּאלֹהֵֽינוּ׃ כִּ֤י מִ֣י אֱ֭לוֹהַּ מִבַּלְעֲדֵ֣י יְהֹוָ֑ה וּמִ֥י צ֝֗וּר זוּלָתִ֥י אֱלֹהֵֽינוּ׃ תּוֹרָ֥ה צִוָּה־לָ֖נוּ מֹשֶׁ֑ה מוֹרָשָׁ֖ה קְהִלַּ֥ת יַעֲקֹֽב׃ עֵץ־חַיִּ֣ים הִ֭יא לַמַּחֲזִיקִ֣ים בָּ֑הּ וְֽתֹמְכֶ֥יהָ מְאֻשָּֽׁר׃ דְּרָכֶ֥יהָ דַרְכֵי־נֹ֑עַם וְֽכׇל־נְתִ֖יבוֹתֶ֣יהָ שָׁלֽוֹם׃ שָׁל֣וֹם רָ֭ב לְאֹהֲבֵ֣י תוֹרָתֶ֑ךָ וְאֵֽין־לָ֥מוֹ מִכְשֽׁוֹל׃ יְֽהֹוָ֗ה עֹ֭ז לְעַמּ֣וֹ יִתֵּ֑ן יְהֹוָ֓ה ׀ יְבָרֵ֖ךְ אֶת־עַמּ֣וֹ בַשָּׁלֽוֹם׃ ",
      ),
    );
    tr.push(
      p(
        "<small>כשמגיע לדוכן, מגביה את ספר התורה ומראה הכתב לקהל ואומרים</small> ",
      ),
    );
    tr.push(
      p(
        "<b>וְזֹ֖את </b> הַתּוֹרָ֑ה אֲשֶׁר־שָׂ֣ם מֹשֶׁ֔ה לִפְנֵ֖י בְּנֵ֥י יִשְׂרָאֵֽל׃ תּוֹרָ֥ה צִוָּה־לָ֖נוּ מֹשֶׁ֑ה מוֹרָשָׁ֖ה קְהִלַּ֥ת יַעֲקֹֽב׃ הָאֵל֮ תָּמִ֢ים דַּ֫רְכּ֥וֹ אִמְרַֽת־יְהֹוָ֥ה צְרוּפָ֑ה מָגֵ֥ן ה֝֗וּא לְכֹ֤ל ׀ הַחֹסִ֬ים בּֽוֹ׃",
      ),
    );
    tr.push(p("<small>ואומר העולה: הַשֵּׁם עִמָּכֶם:</small> "));
    tr.push(p("<small>ועונים הקהל: יְבָרֶכְךָ הַשֵּׁם:</small> "));
    tr.push(
      p("<small>ואומר העולה:</small> בָּרְכוּ אֶת יְהֹוָה הַמְּבֹרָךְ: "),
    );
    tr.push(
      p(
        "<small>ועונים הקהל:</small> בָּרוּךְ יְהֹוָה הַמְּבֹרָךְ לְעוֹלָם וָעֶד: ",
      ),
    );
    tr.push(
      p(
        "<small>וחוזר העולה:</small> בָּרוּךְ יְהֹוָה הַמְּבֹרָךְ לְעוֹלָם וָעֶד: ",
      ),
    );
    tr.push(p("<small>ומברך העולה לפני הקריאה</small>  "));
    tr.push(
      p(
        "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵינוּ  מֶלֶךְ הָעוֹלָם, אֲשֶׁר בָּחַר בָּנוּ מִכָּל הָעַמִּים וְנָתַן לָנוּ אֶת תּוֹרָתוֹ. בָּרוּךְ אַתָּה יְהֹוָה, נוֹתֵן הַתּוֹרָה: <small>[אמן]</small> ",
      ),
    );
    tr.push(p("<small>אחר הקריאה מברך העולה</small> "));
    tr.push(
      p(
        "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵינוּ  מֶלֶךְ הָעוֹלָם, אֲשֶׁר נָתַן לָנוּ אֶת תּוֹרָתוֹ תּוֹרַת אֱמֶת, וְחַיֵּי עוֹלָם נָטַע בְּתוֹכֵנוּ. בָּרוּךְ אַתָּה יְהֹוָה, נוֹתֵן הַתּוֹרָה: <small>[אמן]</small> ",
      ),
    );
  ...
  parts.push(
    p(
      "<b>יְהִי־חַסְדְּךָ֣</b> יְהֹוָ֣ה עָלֵ֑ינוּ כַּ֝אֲשֶׁ֗ר יִחַ֥לְנוּ לָֽךְ׃",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<small><b>תִּתְקַבַּל</b> צְלוֹתָֽנָא וּבָֽעוּתָֽנָא, עִם צְלֽוֹתְהוֹן וּבָעֽוּתְהוֹן דְּכָל-בֵּית יִשְׂרָאֵל, קֳדָם אֲבוּנָא דְּבִשְׁמַיָּא וְאַרְעָא, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>  ",
    ),
  );
  parts.push(
    p(
      "<small><b>יְהֵא</b> שְׁלָמָא רַבָּא מִן שְׁמַיָּא. חַיִּים וְשָׂבָע וִישׁוּעָה וְנֶחָמָה. וְשֵׁיזָבָא וּרְפוּאָה וּגְאוּלָה וּסְלִיחָה וְכַפָּרָה וְרֶוַח וְהַצָּלָה לָנוּ וּלְכָל עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>  ",
    ),
  );
  parts.push(
    p(
      "<small>יפסע שלש פסיעות לאחור<br><b>עוֹשֶׂה</b> שָׁלוֹם בִּמְרוֹמָיו. הוּא בְּרַחֲמָיו יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>   ",
    ),
  );
  ...
  parts.push(
    p(
      "<small><b>יְהֵא</b> שְׁלָמָא רַבָּא מִן שְׁמַיָּא. חַיִּים וְשָׂבָע וִישׁוּעָה וְנֶחָמָה. וְשֵׁיזָבָא וּרְפוּאָה וּגְאוּלָה וּסְלִיחָה וְכַפָּרָה וְרֶוַח וְהַצָּלָה לָנוּ וּלְכָל עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small><br><b>עוֹשֶׂה</b> שָׁלוֹם בִּמְרוֹמָיו. הוּא בְּרַחֲמָיו יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>   ",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p(
      "<b>קַוֵּ֗ה</b> אֶל־יְהֹ֫וָ֥ה חֲ֭זַק וְיַאֲמֵ֣ץ לִבֶּ֑ךָ וְ֝קַוֵּ֗ה אֶל־יְהֹוָֽה: אֵין־קָד֥וֹשׁ כַּיהֹוָ֖ה כִּ֣י אֵ֣ין בִּלְתֶּ֑ךָ וְאֵ֥ין צ֖וּר כֵּאלֹהֵֽינוּ׃ כִּ֤י מִ֣י אֱ֭לוֹהַּ מִבַּלְעֲדֵ֣י יְהֹוָ֑ה וּמִ֥י צ֝֗וּר זוּלָתִ֥י אֱלֹהֵֽינוּ׃",
    ),
  );
  parts.push(
    p(
      "<b>אֵין</b> כֵּאלֹהֵֽינוּ, אֵין כַּאדוֹנֵֽנוּ, אֵין כְּמַלְכֵּֽנוּ, אֵין כְּמוֹשִׁיעֵֽנוּ: מִי כֵּאלֹהֵֽינוּ, מִי כַּאדוֹנֵֽנוּ, מִי כְּמַלְכֵּֽנוּ, מִי כְּמוֹשִׁיעֵֽנוּ: נוֹדֶה לֵאלֹהֵֽינוּ, נוֹדֶה לַאדוֹנֵֽנוּ, נוֹדֶה לְמַלְכֵּֽנוּ, נוֹדֶה לְמוֹשִׁיעֵֽנוּ: בָּרוּךְ אֱלֹהֵֽינוּ, בָּרוּךְ אֲדוֹנֵֽנוּ, בָּרוּךְ מַלְכֵּֽנוּ, בָּרוּךְ מוֹשִׁיעֵֽנוּ: אַתָּה הוּא אֱלֹהֵֽינוּ, אַתָּה הוּא אֲדוֹנֵֽנוּ, אַתָּה הוּא מַלְכֵּֽנוּ, אַתָּה הוּא מוֹשִׁיעֵֽנוּ: אַתָּה תוֹשִׁיעֵֽנוּ, אַתָּ֣ה תָ֭קוּם תְּרַחֵ֣ם צִיּ֑וֹן כִּי־עֵ֥ת לְ֝חֶֽנְנָ֗הּ כִּי־בָ֥א מוֹעֵֽד: ",
    ),
  );
  parts.push(p("<big><b>פטום הקטורת</b></big>"));
  parts.push(
    p(
      "<b>אַתָּה</b> הוּא יְהֹוָה אֱלֹהֵֽינוּ, שֶׁהִקְטִֽירוּ אֲבוֹתֵֽינוּ לְפָנֶֽיךָ אֶת קְטֹֽרֶת הַסַּמִּים, בִּזְמַן שֶׁבֵּית הַמִּקְדָשׁ קַיָּם, כַּאֲשֶׁר צִוִּֽיתָ אוֹתָם עַל־יַד מֹשֶׁה נְבִיאָךְ, כַּכָּתוּב בְּתֽוֹרָתָךְ:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>תָּנוּ</b> רַבָּנָן: פִּטּוּם הַקְּטֹֽרֶת כֵּיצַד? שְׁלֹשׁ מֵאוֹת וְשִׁשִּׁים וּשְׁמוֹנָה מָנִים הָיוּ בָהּ. שְׁלֹשׁ מֵאוֹת וְשִׁשִּׁים וַֽחֲמִשָּׁה כְּמִנְיַן יְמוֹת הַחַמָּה, מָנֶה בְּכָל־יוֹם, מַֽחֲצִיתוֹ בַּבֹּֽקֶר וּמַֽחֲצִיתוֹ בָּעֶרֶב. וּשְׁלֹשָׁה מָנִים יְתֵרִים, שֶׁמֵּהֶם מַכְנִיס כֹּהֵן גָּדוֹל, וְנוֹטֵל מֵהֶם מְלֹא חָפְנָיו בְּיוֹם הַכִּפּוּרִים, וּמַֽחֲזִירָן לְמַכְתֶּֽשֶׁת בְּעֶֽרֶב יוֹם הַכִּפּוּרִים, כְּדֵי לְקַיֵּם מִצְוַת דַקָּה מִן הַדַּקָּה. וְאַחַד־עָשָׂר סַמָּנִים הָיוּ בָהּ, וְאֵֽלּוּ הֵן: ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<small><b>יְהֵא</b> שְׁלָמָא רַבָּא מִן שְׁמַיָּא. חַיִּים וְשָׂבָע וִישׁוּעָה וְנֶחָמָה. וְשֵׁיזָבָא וּרְפוּאָה וּגְאוּלָה וּסְלִיחָה וְכַפָּרָה וְרֶוַח וְהַצָּלָה לָנוּ וּלְכָל עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small><br><br><b>עוֹשֶׂה</b> שָׁלוֹם בִּמְרוֹמָיו. הוּא בְּרַחֲמָיו יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל־עַמּוֹ יִשְׂרָאֵל. וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>",
    ),
  );
  parts.push(
    p(
      "<small>ואומר החזן:</small> (רַבָּנָן) בָּרְכוּ אֶת יְהֹוָה הַמְּבֹרָךְ:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "(ב) הֲרֵינִי זוֹכֵר אֶת הַשַּׁבָּת, כְּמוֹ שֶׁנֶּאֱמַר <small>(שמות כ ז)</small> זָכ֛וֹר אֶת־י֥וֹם הַשַּׁבָּ֖ת לְקַדְּשֽׁוֹ׃ ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "(ז) וַהֲרֵינִי זוֹכֵר מַה שֶׁעָשָׂה הַקָּדוֹשׁ בָּרוּךְ הוּא לְמִרְיָם הַנְּבִיאָה, כְּמוֹ שֶׁנֶּאֱמַר <small>(דברים כד ט)</small> זָכ֕וֹר אֵ֧ת אֲשֶׁר־עָשָׂ֛ה יְהֹוָ֥ה אֱלֹהֶ֖יךָ לְמִרְיָ֑ם בַּדֶּ֖רֶךְ בְּצֵאתְכֶ֥ם מִמִּצְרָֽיִם׃ ",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "(י) הֲרֵינִי זוֹכֵר אֶת יְרוּשָׁלַיִם, כְּמוֹ שֶׁנֶּאֱמַר <small>(תהלים קלז ה)</small> אִֽם־אֶשְׁכָּחֵ֥ךְ יְֽרוּשָׁלָ֗‍ִם תִּשְׁכַּ֥ח יְמִינִֽי׃ תִּדְבַּֽק־לְשׁוֹנִ֨י ׀ לְחִכִּי֮ אִם־לֹ֢א אֶ֫זְכְּרֵ֥כִי אִם־לֹ֣א אַ֭עֲלֶה אֶת־יְרוּשָׁלַ֑‍ִם עַ֗֝ל רֹ֣אשׁ שִׂמְחָתִֽי׃",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<small>כשיוצא מבית הכנסת אומר: <br>יְהֹוָ֤ה ׀ נְחֵ֬נִי בְצִדְקָתֶ֗ךָ לְמַ֥עַן שׁוֹרְרָ֑י הושר [הַיְשַׁ֖ר] לְפָנַ֣י דַּרְכֶּֽךָ׃ וְיַעֲקֹ֖ב הָלַ֣ךְ לְדַרְכּ֑וֹ וַיִּפְגְּעוּ־ב֖וֹ מַלְאֲכֵ֥י אֱלֹהִֽים׃ וַיֹּ֤אמֶר יַעֲקֹב֙ כַּאֲשֶׁ֣ר רָאָ֔ם מַחֲנֵ֥ה אֱלֹהִ֖ים זֶ֑ה וַיִּקְרָ֛א שֵֽׁם־הַמָּק֥וֹם הַה֖וּא מַֽחֲנָֽיִם׃</small>",
    ),
  );
  parts.push(hr);
  ...
  if (isHallelDay) {
    ...
    hl.push(
      p(
        "<small>בִּרְשׁוּת מוֹרַי וְרַבּוֹתַי. <small>והציבור עונים:</small> שָׁמַיִם.</small>",
      ),
    );
    hl.push(p("<small>והחזן מברך ואחר כך הציבור מברכים</small>"));
    hl.push(
      p(
        "<small><b>בָּרוּךְ</b> אַתָּה יְהֹוָה, אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם, אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו, וְצִוָּנוּ לִגְמוֹר אֶת הַהַלֵּל:</small>",
      ),
    );
    hl.push(
      p(
        "<b>הַ֥לְלוּיָ֨הּ</b> ׀ <small>(תהילים קי״ג:א׳-ב׳)</small> הַ֭לְלוּ עַבְדֵ֣י יְהֹוָ֑ה הַֽ֝לְל֗וּ אֶת־שֵׁ֥ם יְהֹוָֽה׃ יְהִ֤י שֵׁ֣ם יְהֹוָ֣ה מְבֹרָ֑ךְ מֵ֝עַתָּ֗ה וְעַד־עוֹלָֽם׃ מִמִּזְרַח־שֶׁ֥מֶשׁ עַד־מְבוֹא֑וֹ מְ֝הֻלָּ֗ל שֵׁ֣ם יְהֹוָֽה׃ רָ֖ם עַל־כׇּל־גּוֹיִ֥ם ׀ יְהֹוָ֑ה עַ֖ל הַשָּׁמַ֣יִם כְּבוֹדֽוֹ׃ מִ֭י כַּיהֹוָ֣ה אֱלֹהֵ֑ינוּ הַֽמַּגְבִּיהִ֥י לָשָֽׁבֶת׃ הַֽמַּשְׁפִּילִ֥י לִרְא֑וֹת בַּשָּׁמַ֥יִם וּבָאָֽרֶץ׃ מְקִ֥ימִ֣י מֵעָפָ֣ר דָּ֑ל מֵ֝אַשְׁפֹּ֗ת יָרִ֥ים אֶבְיֽוֹן׃ לְהוֹשִׁיבִ֥י עִם־נְדִיבִ֑ים עִ֗֝ם נְדִיבֵ֥י עַמּֽוֹ׃ מֽוֹשִׁיבִ֨י ׀ עֲקֶ֬רֶת הַבַּ֗יִת אֵֽם־הַבָּנִ֥ים שְׂמֵחָ֗ה הַֽלְלוּיָֽהּ׃",
      ),
    );
    hl.push(
      p(
        "<b>בְּצֵ֣את</b> <small>(תהילים קי״ד:א׳-ב׳)</small> יִ֭שְׂרָאֵל מִמִּצְרָ֑יִם בֵּ֥ית יַ֝עֲקֹ֗ב מֵעַ֥ם לֹעֵֽז׃ הָיְתָ֣ה יְהוּדָ֣ה לְקׇדְשׁ֑וֹ יִ֝שְׂרָאֵ֗ל מַמְשְׁלוֹתָֽיו׃ הַיָּ֣ם רָ֭אָה וַיָּנֹ֑ס הַ֝יַּרְדֵּ֗ן יִסֹּ֥ב לְאָחֽוֹר׃ הֶ֭הָרִים רָקְד֣וּ כְאֵילִ֑ים גְּ֝בָע֗וֹת כִּבְנֵי־צֹֽאן׃ מַה־לְּךָ֣ הַ֭יָּם כִּ֣י תָנ֑וּס הַ֝יַּרְדֵּ֗ן תִּסֹּ֥ב לְאָחֽוֹר׃ הֶ֭הָרִים תִּרְקְד֣וּ כְאֵילִ֑ים גְּ֝בָע֗וֹת כִּבְנֵי־צֹֽאן׃ מִלִּפְנֵ֣י אָ֭דוֹן ח֣וּלִי אָ֑רֶץ מִ֝לִּפְנֵ֗י אֱל֣וֹהַּ יַעֲקֹֽב׃ הַהֹפְכִ֣י הַצּ֣וּר אֲגַם־מָ֑יִם חַ֝לָּמִ֗ישׁ לְמַעְיְנוֹ־מָֽיִם׃",
      ),
    );
    if (!isFullHallel) {
...
function buildShacharitAshkenazPayload(context) {
  ...
  parts.push(
    p(
      "<b>מוֹדֶה אֲנִי</b> לְפָנֶֽיךָ מֶֽלֶךְ חַי וְקַיָּם שֶׁהֶחֱזַֽרְתָּ בִּי נִשְׁמָתִי בְּחֶמְלָה, רַבָּה אֱמוּנָתֶֽךָ: <b>רֵאשִׁית חָכְמָה</b> יִרְאַת יְהֹוָה, שֵֽׂכֶל טוֹב לְכָל־עֹשֵׂיהֶם, תְּהִלָּתוֹ עוֹמֶֽדֶת לָעַד: בָּרוּךְ שֵׁם כְּבוֹד מַלְכוּתוֹ לְעוֹלָם וָעֶד:",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר נָתַן לַשֶּֽׂכְוִי בִינָה לְהַבְחִין בֵּין יוֹם וּבֵין לָֽיְלָה:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁלֹּא עָשַֽׂנִי גּוֹי:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁלֹּא עָשַֽׂנִי עָֽבֶד:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁלֹּא עָשַֽׂנִי אִשָּׁה:",
    ),
  );
  parts.push(
    sup(
      "(<small>נשים אומרות:</small> בָּרוּךְ אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁעָשַֽׂנִי כִּרְצוֹנוֹ:)",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם פּוֹקֵֽחַ עִוְרִים:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם מַלְבִּישׁ עֲרֻמִּים:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם מַתִּיר אֲסוּרִים:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם זוֹקֵף כְּפוּפִים:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם רוֹקַע הָאָֽרֶץ עַל הַמָּֽיִם:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁעָשָׂה לִי כָּל־צָרְכִּי:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם הַמֵּכִין מִצְעֲדֵי גָֽבֶר:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אוֹזֵר יִשְׂרָאֵל בִּגְבוּרָה:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם עוֹטֵר יִשְׂרָאֵל בְּתִפְאָרָה:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם הַנּוֹתֵן לַיָּעֵף כֹּֽחַ:",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם הַמַּעֲבִיר שֵׁנָה מֵעֵינָי וּתְנוּמָה מֵעַפְעַפָּי:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>בָּרְ֒כִי נַפְשִׁי</b> אֶת־יְהֹוָה, יְהֹוָה אֱלֹהַי גָּדַֽלְתָּ מְאֹד, הוֹד וְהָדָר לָבָֽשְׁתָּ: עֹֽטֶה־אוֹר כַּשַּׂלְמָה, נוֹטֶה שָׁמַֽיִם כַּיְרִיעָה:",
    ),
  );
  parts.push(
    sup(
  ...
  parts.push(sup("<small>בשעה שמניח אותו על ראשו להתעטף בו, יאמר:</small>"));
  parts.push(
    p(
  ...
  parts.push(
    sup("<small>יניח התפילין של יד על קיבורת ידו, וקודם שיהדק יברך:</small>"),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר קִדְּ֒שָֽׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לְהָנִיחַ תְּפִלִּין:",
    ),
  );
  parts.push(
    sup(
      "<small>ויכרוך שבע כריכות סביב זרועו, ואחר כך יניח של ראש בגובה ראשו כנגד בין עיניו, וקודם שמהדקן יברך:</small>",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר קִדְּ֒שָֽׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ עַל מִצְוַת תְּפִלִּין:",
    ),
  );
  parts.push(sup("<small>ואחר שיהדק הרצועה על ראשו יאמר:</small>"));
  parts.push(
    p(
  ...
  // Rule 1: opening instruction → sup()
  parts.push(
    sup(
      '<small>המתפלל צריך שיכוין בליבו פירוש המילות שהוא מוציא בשפתיו. ואם אינו יכול לכוין פירוש המילות לכל הפחות צריך שיחשוב בשעת התפילה בדברים המכניעים את הלב ומכוונים את לבו לאביו שבשמים. יכוין רגליו זה אצל זה כאילו אינן אלא אחת. ויזהר להתפלל בלחש. ולא ירמוז בעיניו ולא יקרוץ בשפתיו ולא יראה באצבעותיו ואינו פוסק אפילו לקדיש וקדושה וברכו (קיצשו"ע יח)</small>',
    ),
  );
  // Avot blessing
  parts.push(
    p(
      "אֲדֹנָי שְׂפָתַי תִּפְתָּח וּפִי יַגִּיד תְּהִלָּתֶֽךָ: <b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ אֱלֹהֵי אַבְרָהָם אֱלֹהֵי יִצְחָק וֵאלֹהֵי יַעֲקֹב הָאֵל הַגָּדוֹל הַגִּבּוֹר וְהַנּוֹרָא אֵל עֶלְיוֹן גּוֹמֵל חֲסָדִים טוֹבִים וְקוֹנֵה הַכֹּל וְזוֹכֵר חַסְדֵי אָבוֹת וּמֵבִיא גוֹאֵל לִבְנֵי בְנֵיהֶם לְמַֽעַן שְׁמוֹ בְּאַהֲבָה: מֶֽלֶךְ עוֹזֵר וּמוֹשִֽׁיעַ וּמָגֵן: בָּרוּךְ אַתָּה יְהֹוָה מָגֵן אַבְרָהָם:",
    ),
  );
  ...
  parts.push(
    p(
      "<b>אַתָּה גִבּוֹר</b> לְעוֹלָם אֲדֹנָי מְחַיֵּה מֵתִים אַתָּה רַב לְהוֹשִֽׁיעַ:",
    ),
  );
  ...
  // Rule 5: "בחזרת הש"ץ" header → sup()
  parts.push(sup('<small>בחזרת הש"ץ אומרים כאן קדושה:</small>'));
  // Kedushah — already in sup()
  parts.push(
    sup(
      '<small>קהל וחזן:</small> <b>נְקַדֵּשׁ</b> אֶת־שִׁמְךָ בָּעוֹלָם כְּשֵׁם שֶׁמַּקְדִּישִׁים אוֹתוֹ בִּשְׁמֵי מָרוֹם. כַּכָּתוּב עַל־יַד נְבִיאֶֽךָ וְקָרָא זֶה אֶל־זֶה וְאָמַר: <small>קהל וחזן:</small> <b>קָדוֹשׁ קָדוֹשׁ קָדוֹשׁ יְהֹוָה צְבָאוֹת מְלֹא כָל הָאָֽרֶץ כְּבוֹדוֹ:</b> <small>שליח ציבור:</small> לְעֻמָּתָם בָּרוּךְ יֹאמֵֽרוּ: <small>קהל וחזן:</small> <b>בָּרוּךְ כְּבוֹד־יְהֹוָה מִמְּ֒קוֹמוֹ:</b> <small>שליח ציבור:</small> וּבְדִבְרֵי קָדְשְׁךָ כָּתוּב לֵאמֹר: <small>קהל וחזן:</small> <b>יִמְלֹךְ</b> יְהֹוָה לְעוֹלָם אֱלֹהַֽיִךְ צִיּוֹן לְדֹר וָדֹר הַלְ֒לוּיָהּ: <small>שליח ציבור:</small> <b>לְדוֹר</b> וָדוֹר נַגִּיד גָּדְלֶֽךָ וּלְנֵֽצַח נְצָחִים קְדֻשָּׁתְךָ נַקְדִּישׁ וְשִׁבְחֲךָ אֱלֹהֵֽינוּ מִפִּֽינוּ לֹא יָמוּשׁ לְעוֹלָם וָעֶד כִּי אֵל מֶֽלֶךְ גָּדוֹל וְקָדוֹשׁ אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה הָאֵל הַקָּדוֹשׁ: <small>בעשי"ת מסיים: בָּרוּךְ אַתָּה יְהֹוָה הַמֶּֽלֶךְ הַקָּדוֹשׁ:</small>',
    ),
  );
  // Atah Kadosh
  parts.push(
    p(
      "<b>אַתָּה קָדוֹשׁ</b> וְשִׁמְךָ קָדוֹשׁ וּקְדוֹשִׁים בְּכָל־יוֹם יְהַלְ֒לֽוּךָ סֶּֽלָה. בָּרוּךְ אַתָּה יְהֹוָה הָאֵל הַקָּדוֹשׁ:",
    ),
  );
  ...
  parts.push(
    p(
      "<b>אַתָּה חוֹנֵן</b> לְאָדָם דַּֽעַת וּמְלַמֵּד לֶאֱנוֹשׁ בִּינָה: חָנֵּֽנוּ מֵאִתְּ֒ךָ דֵּעָה בִּינָה וְהַשְׂכֵּל: בָּרוּךְ אַתָּה יְהֹוָה חוֹנֵן הַדָּֽעַת:",
    ),
  );
  parts.push(
    p(
      "<b>הֲשִׁיבֵֽנוּ</b> אָבִֽינוּ לְתוֹרָתֶֽךָ וְקָרְ֒בֵֽנוּ מַלְכֵּֽנוּ לַעֲבוֹדָתֶֽךָ וְהַחֲזִירֵֽנוּ בִּתְשׁוּבָה שְׁלֵמָה לְפָנֶֽיךָ: בָּרוּךְ אַתָּה יְהֹוָה הָרוֹצֶה בִּתְשׁוּבָה:",
    ),
  );
  parts.push(
    p(
      "<b>סְלַח לָֽנוּ</b> אָבִֽינוּ כִּי חָטָֽאנוּ מְחַל לָֽנוּ מַלְכֵּֽנוּ כִּי פָשָֽׁעְנוּ כִּי מוֹחֵל וְסוֹלֵֽחַ אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה חַנּוּן הַמַּרְבֶּה לִסְלֽוֹחַ:",
    ),
  );
  parts.push(
    p(
      "<b>רְאֵה</b> בְעָנְיֵֽנוּ וְרִיבָה רִיבֵֽנוּ וּגְאָלֵֽנוּ מְהֵרָה לְמַֽעַן שְׁמֶֽךָ כִּי גּוֹאֵל חָזָק אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה גּוֹאֵל יִשְׂרָאֵל:",
    ),
  );
  // Rule 6: עננו → sup(), public fast only
  if (context.isFastDay) {
    parts.push(
      sup('<small>בתענית ציבור אומר כאן הש"ץ <b>עֲנֵֽנוּ</b></small>'),
    );
  }
  // Rule 7: prayer for the sick → sup()
  parts.push(
    p(
      "<b>רְפָאֵֽנוּ</b> יְהֹוָה וְנֵרָפֵא הוֹשִׁיעֵֽנוּ וְנִוָּשֵֽׁעָה כִּי תְהִלָּתֵֽנוּ אָֽתָּה וְהַעֲלֵה רְפוּאָה שְׁלֵמָה לְכָל מַכּוֹתֵֽינוּ כִּי אֵל מֶֽלֶךְ רוֹפֵא נֶאֱמָן וְרַחֲמָן אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה רוֹפֵא חוֹלֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  parts.push(
    sup(
      "<small>מי שרוצה להתפלל על החולה יאמר כאן תחנה זו:</small> <small>יְהִי רָצוֹן מִלְּ֒פָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁתִּשְׁלַח מְהֵרָה רְפוּאָה שְׁלֵמָה מִן הַשָּׁמַֽיִם, רְפוּאַת הַנֶּֽפֶשׁ וּרְפוּאַת הַגּוּף, לַחוֹלֶה (פלוני/פלונית) בֶּן/בַּת (פלונית) בְּתוֹךְ שְׁאָר חוֹלֵי יִשְׂרָאֵל:</small>",
    ),
  );
  // Rule 8: ברכת השנים → dynamic; error note for winter → sup()
  if (isSummer) {
    parts.push(
      p(
  ...
  parts.push(
    p(
      "<b>תְּקַע</b> בְּשׁוֹפָר גָּדוֹל לְחֵרוּתֵֽנוּ וְשָׂא נֵס לְקַבֵּץ גָּלֻיּוֹתֵֽינוּ וְקַבְּ֒צֵֽנוּ יַֽחַד מֵאַרְבַּע כַּנְפוֹת הָאָֽרֶץ: בָּרוּךְ אַתָּה יְהֹוָה מְקַבֵּץ נִדְחֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  // Rule 9: המלך המשפט → sup(), Ten Days only
  parts.push(
    p(
      "<b>הָשִֽׁיבָה</b> שׁוֹפְ֒טֵֽינוּ כְּבָרִאשׁוֹנָה וְיוֹעֲצֵֽינוּ כְּבַתְּ֒חִלָּה וְהָסֵר מִמֶּֽנּוּ יָגוֹן וַאֲנָחָה וּמְלוֹךְ עָלֵֽינוּ אַתָּה יְהֹוָה לְבַדְּ֒ךָ בְּחֶֽסֶד וּבְרַחֲמִים וְצַדְּ֒קֵֽנוּ בַּמִשְׁפָּט: בָּרוּךְ אַתָּה יְהֹוָה מֶֽלֶךְ אֹהֵב צְדָקָה וּמִשְׁפָּט:",
    ),
  );
  if (context.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p(
      "<b>וְלַמַּלְשִׁינִים</b> אַל תְּהִי תִקְוָה וְכָל הָרִשְׁעָה כְּרֶֽגַע תֹּאבֵד וְכָל אֹיְבֶֽיךָ מְהֵרָה יִכָּרֵֽתוּ וְהַזֵּדִים מְהֵרָה תְעַקֵּר וּתְשַׁבֵּר וּתְמַגֵּר וְתַכְנִֽיעַ בִּמְהֵרָה בְיָמֵֽינוּ: בָּרוּךְ אַתָּה יְהֹוָה שׁוֹבֵר אֹיְ֒בִים וּמַכְנִֽיעַ זֵדִים:",
    ),
  );
  parts.push(
    p(
      "<b>עַל־הַצַּדִּיקִים</b> וְעַל־הַחֲסִידִים וְעַל־זִקְנֵי עַמְּ֒ךָ בֵּית יִשְׂרָאֵל וְעַל פְּלֵיטַת סוֹפְ֒רֵיהֶם וְעַל גֵּרֵי הַצֶּֽדֶק וְעָלֵֽינוּ יֶהֱמוּ רַחֲמֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וְתֵן שָׂכָר טוֹב לְכָל הַבּוֹטְ֒חִים בְּשִׁמְךָ בֶּאֱמֶת וְשִׂים חֶלְקֵֽנוּ עִמָּהֶם לְעוֹלָם וְלֹא נֵבוֹשׁ כִּי בְךָ בָּטָֽחְנוּ: בָּרוּךְ אַתָּה יְהֹוָה מִשְׁעָן וּמִבְטָח לַצַּדִּיקִים:",
    ),
  );
  parts.push(
    p(
      "<b>וְלִירוּשָׁלַֽיִם</b> עִירְ֒ךָ בְּרַחֲמִים תָּשׁוּב וְתִשְׁכּוֹן בְּתוֹכָהּ כַּאֲשֶׁר דִּבַּֽרְתָּ וּבְנֵה אוֹתָהּ בְּקָרוֹב בְּיָמֵֽינוּ בִּנְיַן עוֹלָם וְכִסֵּא דָוִד מְהֵרָה לְתוֹכָהּ תָּכִין: בָּרוּךְ אַתָּה יְהֹוָה בּוֹנֵה יְרוּשָׁלָֽיִם:",
    ),
  );
  parts.push(
    p(
      "<b>אֶת־צֶֽמַח</b> דָּוִד עַבְדְּ֒ךָ מְהֵרָה תַצְמִֽיחַ וְקַרְנוֹ תָּרוּם בִּישׁוּעָתֶֽךָ כִּי לִישׁוּעָתְ֒ךָ קִוִּֽינוּ כָּל הַיּוֹם: בָּרוּךְ אַתָּה יְהֹוָה מַצְמִֽיחַ קֶֽרֶן יְשׁוּעָה:",
    ),
  );
  parts.push(
    p(
      "<b>שְׁמַע קוֹלֵֽנוּ</b> יְהֹוָה אֱלֹהֵֽינוּ חוּס וְרַחֵם עָלֵֽינוּ וְקַבֵּל בְּרַחֲמִים וּבְרָצוֹן אֶת־תְּפִלָּתֵֽנוּ כִּי אֵל שׁוֹמֵֽעַ תְּפִלּוֹת וְתַחֲנוּנִים אָֽתָּה וּמִלְּ֒פָנֶֽיךָ מַלְכֵּֽנוּ רֵיקָם אַל־תְּשִׁיבֵֽנוּ כִּי אַתָּה שׁוֹמֵֽעַ תְּפִלַּת עַמְּ֒ךָ יִשְׂרָאֵל בְּרַחֲמִים: בָּרוּךְ אַתָּה יְהֹוָה שׁוֹמֵֽעַ תְּפִלָּה:",
    ),
  );
  // Retzeh base
  parts.push(
    p(
      "<b>רְצֵה</b> יְהֹוָה אֱלֹהֵֽינוּ בְּעַמְּ֒ךָ יִשְׂרָאֵל וּבִתְפִלָּתָם וְהָשֵׁב אֶת הָעֲבוֹדָה לִדְבִיר בֵּיתֶֽךָ וְאִשֵּׁי יִשְׂרָאֵל וּתְפִלָּתָם בְּאַהֲבָה תְקַבֵּל בְּרָצוֹן וּתְהִי לְרָצוֹן תָּמִיד עֲבוֹדַת יִשְׂרָאֵל עַמֶּֽךָ:",
    ),
  );
  // Rule 10: יעלה ויבוא → sup(), RC/Chol HaMoed only, dynamic feast name
  if (context.isRoshChodesh || context.isCholHamoed) {
    var yaalehDay = context.isRoshChodesh
    ...
    parts.push(
      sup(
        "<b>אֱלֹהֵֽינוּ</b> וֵאלֹהֵי אֲבוֹתֵֽינוּ <b>יַעֲלֶה וְיָבֹא</b> וְיַגִּֽיעַ וְיֵרָאֶה וְיֵרָצֶה וְיִשָּׁמַע וְיִפָּקֵד וְיִזָּכֵר זִכְרוֹנֵֽנוּ וּפִקְדוֹנֵֽנוּ וְזִכְרוֹן אֲבוֹתֵֽינוּ. וְזִכְרוֹן מָשִֽׁיחַ בֶּן דָּוִד עַבְדֶּֽךָ. וְזִכְרוֹן יְרוּשָׁלַֽיִם עִיר קָדְשֶֽׁךָ. וְזִכְרוֹן כָּל עַמְּ֒ךָ בֵּית יִשְׂרָאֵל לְפָנֶֽיךָ. לִפְלֵיטָה לְטוֹבָה לְחֵן וּלְחֶֽסֶד וּלְרַחֲמִים לְחַיִּים וּלְשָׁלוֹם. בְּיוֹם " +
          yaalehDay +
          " זָכְרֵֽנוּ יְהֹוָה אֱלֹהֵֽינוּ בּוֹ לְטוֹבָה. וּפָקְדֵֽנוּ בוֹ לִבְרָכָה. וְהוֹשִׁיעֵֽנוּ בוֹ לְחַיִּים. וּבִדְבַר יְשׁוּעָה וְרַחֲמִים חוּס וְחָנֵּֽנוּ וְרַחֵם עָלֵֽינוּ וְהוֹשִׁיעֵֽנוּ. כִּי אֵלֶֽיךָ עֵינֵֽינוּ כִּי אֵל מֶֽלֶךְ חַנּוּן וְרַחוּם אָֽתָּה: <small>שכח ולא אמר יעלה ויבא אם נזכר קודם שאמר יהיו לרצון חוזר ומתחיל רצה. ואפלו אם נזכר קודם שהתחיל מודים כיון שסים ברכת המחזיר שכינתו לציון צריך להתחיל רצה. אך אם נזכר קודם ברכת המחזיר שכינתו לציון אומרו שם ומסים ותחזינה עינינו וכו'. ואם לא נזכר עד לאחר יהיו לרצון וגו' חוזר לראש התפלה.</small>",
      ),
    );
  }
  parts.push(
    p(
      "<b>וְתֶחֱזֶֽינָה</b> עֵינֵֽינוּ בְּשׁוּבְ֒ךָ לְצִיּוֹן בְּרַחֲמִים: בָּרוּךְ אַתָּה יְהֹוָה הַמַּחֲזִיר שְׁכִינָתוֹ לְצִיּוֹן:",
    ),
  );
  // Rule 11: מודים kneeling → sup()
  parts.push(
    sup(
      '<small>כשאומר מודים כורע ראשו וגופו כאגמון עד שיתפקקו כל חוליות שבשדרה. וכשהוא כורע יכרע במהירות בפעם אחת וכשהוא זוקף זוקף בנחת ראשו תחלה ואחר כך גופו שלא תהא עליו כמשאוי (שו"ע או"ח סי\' קיג)</small>',
    ),
  );
  // Modim base
  parts.push(
    p(
      "<b>מוֹדִים</b> אֲנַֽחְנוּ לָךְ שָׁאַתָּה הוּא יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ לְעוֹלָם וָעֶד צוּר חַיֵּֽינוּ מָגֵן יִשְׁעֵֽנוּ אַתָּה הוּא לְדוֹר וָדוֹר נֽוֹדֶה לְּךָ וּנְסַפֵּר תְּהִלָּתֶֽךָ עַל־חַיֵּֽינוּ הַמְּ֒סוּרִים בְּיָדֶֽךָ וְעַל נִשְׁמוֹתֵֽינוּ הַפְּ֒קוּדוֹת לָךְ וְעַל נִסֶּֽיךָ שֶׁבְּכָל יוֹם עִמָּֽנוּ וְעַל נִפְלְ֒אוֹתֶֽיךָ וְטוֹבוֹתֶֽיךָ שֶׁבְּ֒כָל עֵת עֶֽרֶב וָבֹֽקֶר וְצָהֳרָֽיִם הַטּוֹב כִּי לֹא כָלוּ רַחֲמֶֽיךָ וְהַמְ֒רַחֵם כִּי לֹא תַֽמּוּ חֲסָדֶֽיךָ מֵעוֹלָם קִוִּֽינוּ לָךְ:",
    ),
  );
  // Rule 12: מודים דרבנן → sup()
  parts.push(
    sup(
      "<small>כשיגיע שליח צבור למודים וכורע כל העם שוחין ואומרין הודאה קטנה המתחלת כמו כן במודים. שאין דרך העבד להודות לרבו ולומר לו אדוני אתה על ידי שליח אלא כל אדם צריך לקבל בפיו עול מלכות שמים ואם יקבל על ידי שליח אינה קבלה גמורה שיוכל להכחיש ולומר לא שלחתיו. (אבודרהם)</small> <b>מוֹדִים</b> אֲנַֽחְנוּ לָךְ שָׁאַתָּה הוּא יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ אֱלֹהֵי כָל בָּשָׂר יוֹצְ֒רֵֽנוּ יוֹצֵר בְּרֵאשִׁית בְּרָכוֹת וְהוֹדָאוֹת לְשִׁמְךָ הַגָּדוֹל וְהַקָּדוֹשׁ עַל שֶׁהֶחֱיִיתָֽנוּ וְקִיַּמְתָּֽנוּ כֵּן תְּחַיֵּֽינוּ וּתְקַיְּ֒מֵֽנוּ וְתֶאֱסוֹף גָּלֻיּוֹתֵֽינוּ לְחַצְרוֹת קָדְשֶֽׁךָ לִשְׁמוֹר חֻקֶּֽיךָ וְלַעֲשׂוֹת רְצוֹנֶֽךָ וּלְעָבְדְּ֒ךָ בְּלֵבָב שָׁלֵם עַל שֶׁאֲנַֽחְנוּ מוֹדִים לָךְ. בָּרוּךְ אֵל הַהוֹדָאוֹת:",
    ),
  );
  ...
  parts.push(
    p(
      "<b>וְעַל־כֻּלָּם</b> יִתְבָּרַךְ וְיִתְרוֹמַם שִׁמְךָ מַלְכֵּֽנוּ תָּמִיד לְעוֹלָם וָעֶד:",
    ),
  );
  ...
  parts.push(
    p(
      "<b>וְכֹל הַחַיִּים</b> יוֹדֽוּךָ סֶּֽלָה וִיהַלְ֒לוּ אֶת־שִׁמְךָ בֶּאֱמֶת הָאֵל יְשׁוּעָתֵֽנוּ וְעֶזְרָתֵֽנוּ סֶֽלָה: בָּרוּךְ אַתָּה יְהֹוָה הַטּוֹב שִׁמְךָ וּלְךָ נָאֶה לְהוֹדוֹת:",
    ),
  );
  // Rule 15: ברכת כהנים — already in sup()
  parts.push(
    sup(
      "<small>ברכת כהנים</small> <small>שליח ציבור:</small> אֱלֹהֵינוּ וֵאלֹהֵי אֲבוֹתֵינוּ בָּרְכֵנוּ בַּבְּרָכָה הַמְּשֻׁלֶּשֶׁת בַּתּוֹרָה הַכְּתוּבָה עַל יְדֵי מֹשֶׁה עַבְדֶּךָ, הָאֲמוּרָה מִפִּי אַהֲרֹן וּבָנָיו כֹּהֲנִים עַם קְדוֹשֶׁךָ כָּאָמוּר: <small>כהנים אומרים:</small> יְבָרֶכְךָ יְהֹוָה וְיִשְׁמְרֶךָ: יָאֵר יְהֹוָה פָּנָיו אֵלֶיךָ וִיחֻנֶּךָּ: יִשָּׂא יְהֹוָה פָּנָיו אֵלֶיךָ וְיָשֵׂם לְךָ שָׁלוֹם: <small>אם אין כהנים אומר השליח ציבור:</small> <small>אֱלֹהֵינוּ וֵאלֹהֵי אֲבוֹתֵינוּ בָּרְכֵנוּ בַּבְּרָכָה הַמְּשֻׁלֶּשֶׁת בַּתּוֹרָה הַכְּתוּבָה עַל יְדֵי מֹשֶׁה עַבְדֶּךָ, הָאֲמוּרָה מִפִּי אַהֲרֹן וּבָנָיו כֹּהֲנִים עַם קְדוֹשֶׁךָ כָּאָמוּר: יְבָרֶכְךָ וְיִשְׁמְרֶךָ יָאֵר וִיחֻנֶּךָּ יִשָּׂא וְיָשֵׂם לְךָ שָׁלוֹם:</small>",
    ),
  );
  // Rule 16: שים שלום stays p(); בספר חיים → sup(), Ten Days only
  parts.push(
    p(
      "<b>שִׂים שָׁלוֹם</b> טוֹבָה וּבְרָכָה חֵן וָחֶֽסֶד וְרַחֲמִים עָלֵֽינוּ וְעַל כָּל־יִשְׂרָאֵל עַמֶּֽךָ. בָּרְ֒כֵֽנוּ אָבִֽינוּ כֻּלָּֽנוּ כְּאֶחָד בְּאוֹר פָּנֶֽיךָ. כִּי בְאוֹר פָּנֶֽיךָ נָתַֽתָּ לָּֽנוּ יְהֹוָה אֱלֹהֵֽינוּ תּוֹרַת חַיִּים וְאַהֲבַת חֶֽסֶד וּצְדָקָה וּבְרָכָה וְרַחֲמִים וְחַיִּים וְשָׁלוֹם. וְטוֹב בְּעֵינֶֽיךָ לְבָרֵךְ אֶת עַמְּ֒ךָ יִשְׂרָאֵל בְּכָל־עֵת וּבְכָל־שָׁעָה בִּשְׁלוֹמֶֽךָ:",
    ),
  );
  if (context.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p("בָּרוּךְ אַתָּה יְהֹוָה הַמְ֒בָרֵךְ אֶת־עַמּוֹ יִשְׂרָאֵל בַּשָּׁלוֹם:"),
  );
  // Rule 17: end section — all stays p()
  parts.push(
    p(
      'יִהְיוּ לְרָצוֹן אִמְרֵי פִי וְהֶגְיוֹן לִבִּי לְפָנֶֽיךָ יְהֹוָה צוּרִי וְגוֹאֲלִי: <b>אֱלֹהַי</b> נְצוֹר לְשׁוֹנִי מֵרָע וּשְׂפָתַי מִדַּבֵּר מִרְמָה. וְלִמְקַלְ֒לַי נַפְשִׁי תִדּוֹם וְנַפְשִׁי כֶּעָפָר לַכֹּל תִּהְיֶה. פְּתַח לִבִּי בְּתוֹרָתֶֽךָ וּבְמִצְוֹתֶֽיךָ תִּרְדֹּף נַפְשִׁי. וְכֹל הַחוֹשְׁ֒בִים עָלַי רָעָה מְהֵרָה הָפֵר עֲצָתָם וְקַלְקֵל מַחֲשַׁבְתָּם: עֲשֵׂה לְמַֽעַן שְׁמֶֽךָ עֲשֵׂה לְמַֽעַן יְמִינֶֽךָ עֲשֵׂה לְמַֽעַן קְדֻשָּׁתֶֽךָ עֲשֵׂה לְמַֽעַן תּוֹרָתֶֽךָ. לְמַֽעַן יֵחָלְ֒צוּן יְדִידֶֽיךָ הוֹשִֽׁיעָה יְמִינְ֒ךָ וַעֲנֵֽנִי: יִהְיוּ לְרָצוֹן אִמְרֵי פִי וְהֶגְיוֹן לִבִּי לְפָנֶֽיךָ יְהֹוָה צוּרִי וְגוֹאֲלִי: עֹשֶׂה (<small>בעשי"ת</small> הַשָּׁלוֹם) שָׁלוֹם בִּמְרוֹמָיו הוּא יַעֲשֶׂה שָׁלוֹם עָלֵֽינוּ וְעַל כָּל־יִשְׂרָאֵל וְאִמְרוּ אָמֵן: <b>יְהִי רָצוֹן</b> מִלְּ֒פָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁיִּבָּנֶה בֵּית הַמִּקְדָּשׁ בִּמְהֵרָה בְיָמֵֽינוּ וְתֵן חֶלְקֵֽנוּ בְּתוֹרָתֶֽךָ: וְשָׁם נַעֲבָדְךָ בְּיִרְאָה כִּימֵי עוֹלָם וּכְשָׁנִים קַדְמוֹנִיּוֹת: וְעָרְ֒בָה לַיהוָֹה מִנְחַת יְהוּדָה וִירוּשָׁלָֽםִ כִּימֵי עוֹלָם וּכְשָׁנִים קַדְמוֹנִיּוֹת:',
    ),
  );
  // Rule 18: end note → sup()
  parts.push(
    sup(
      '<small>בר"ח וחולו של מועד וחנוכה אומרים כאן הלל לאחר חזרת הש"ץ. בת"צ אומרים סליחות. בימים שאין אומרים בהם תחנון יאמר הש"ץ חצי קדיש.</small>',
    ),
  );
  parts.push(hr);
  ...
  if (isHallelDay) {
    var hl = [];
    hl.push(p("<big><b>הַלֵּל</b></big>"));
    hl.push(
      p(
        "<small>הלל שלם: מברכים ואומרים הכל. חצי הלל: מברכים ומדלגים לֹא לָנוּ וְאָהַבְתִּי. בראש חודש ובשביעי שמיני של פסח אין מברכים לפני הלל למנהג אשכנז.</small>",
      ),
    );
    if (isFullHallel) {
      hl.push(
        p(
          "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לִגְמֹר אֶת הַהַלֵּל:",
        ),
      );
    }
    hl.push(
      p(
        "<b>הַלְלוּיָהּ</b> הַלְלוּ עַבְדֵי יְהֹוָה הַלְלוּ אֶת שֵׁם יְהֹוָה: יְהִי שֵׁם יְהֹוָה מְבֹרָךְ מֵעַתָּה וְעַד עוֹלָם: מִמִּזְרַח שֶׁמֶשׁ עַד מְבוֹאוֹ מְהֻלָּל שֵׁם יְהֹוָה: רָם עַל כָּל גּוֹיִם יְהֹוָה עַל הַשָּׁמַיִם כְּבוֹדוֹ: מִי כַּיְהֹוָה אֱלֹהֵינוּ הַמַּגְבִּיהִי לָשָׁבֶת: הַמַּשְׁפִּילִי לִרְאוֹת בַּשָּׁמַיִם וּבָאָרֶץ: מְקִימִי מֵעָפָר דָּל מֵאַשְׁפֹּת יָרִים אֶבְיוֹן: לְהוֹשִׁיבִי עִם נְדִיבִים עִם נְדִיבֵי עַמּוֹ: מוֹשִׁיבִי עֲקֶרֶת הַבַּיִת אֵם הַבָּנִים שְׂמֵחָה הַלְלוּיָהּ:",
      ),
    );
    hl.push(
      p(
        "<b>בְּצֵאת</b> יִשְׂרָאֵל מִמִּצְרָיִם בֵּית יַעֲקֹב מֵעַם לֹעֵז: הָיְתָה יְהוּדָה לְקָדְשׁוֹ יִשְׂרָאֵל מַמְשְׁלוֹתָיו: הַיָּם רָאָה וַיָּנֹס הַיַּרְדֵּן יִסֹּב לְאָחוֹר: הֶהָרִים רָקְדוּ כְאֵילִים גְּבָעוֹת כִּבְנֵי צֹאן: מַה לְּךָ הַיָּם כִּי תָנוּס הַיַּרְדֵּן תִּסֹּב לְאָחוֹר: הֶהָרִים תִּרְקְדוּ כְאֵילִים גְּבָעוֹת כִּבְנֵי צֹאן: מִלִּפְנֵי אָדוֹן חוּלִי אָרֶץ מִלִּפְנֵי אֱלוֹהַּ יַעֲקֹב: הַהֹפְכִי הַצּוּר אֲגַם מָיִם חַלָּמִישׁ לְמַעְיְנוֹ מָיִם:",
      ),
    );
    if (!isFullHallel) {
      hl.push(
        p("<small>בחצי הלל מדלגים מכאן עד יְהֹוָה זְכָרָנוּ יְבָרֵךְ</small>"),
      );
    } else {
      hl.push(
        p(
          "<b>לֹא לָנוּ</b> יְהֹוָה לֹא לָנוּ כִּי לְשִׁמְךָ תֵּן כָּבוֹד עַל חַסְדְּךָ עַל אֲמִתֶּךָ: לָמָּה יֹאמְרוּ הַגּוֹיִם אַיֵּה נָא אֱלֹהֵיהֶם: וֵאלֹהֵינוּ בַשָּׁמָיִם כֹּל אֲשֶׁר חָפֵץ עָשָׂה: עֲצַבֵּיהֶם כֶּסֶף וְזָהָב מַעֲשֵׂה יְדֵי אָדָם: פֶּה לָהֶם וְלֹא יְדַבֵּרוּ עֵינַיִם לָהֶם וְלֹא יִרְאוּ: אָזְנַיִם לָהֶם וְלֹא יִשְׁמָעוּ אַף לָהֶם וְלֹא יְרִיחוּן: יְדֵיהֶם וְלֹא יְמִישׁוּן רַגְלֵיהֶם וְלֹא יְהַלֵּכוּ לֹא יֶהְגּוּ בִּגְרוֹנָם: כְּמוֹהֶם יִהְיוּ עֹשֵׂיהֶם כֹּל אֲשֶׁר בֹּטֵחַ בָּהֶם: יִשְׂרָאֵל בְּטַח בַּיְהֹוָה עֶזְרָם וּמָגִנָּם הוּא: בֵּית אַהֲרֹן בִּטְחוּ בַיְהֹוָה עֶזְרָם וּמָגִנָּם הוּא: יִרְאֵי יְהֹוָה בִּטְחוּ בַיְהֹוָה עֶזְרָם וּמָגִנָּם הוּא:",
        ),
      );
    }
    hl.push(
      p(
        "<b>יְהֹוָה זְכָרָנוּ יְבָרֵךְ</b> יְבָרֵךְ אֶת בֵּית יִשְׂרָאֵל יְבָרֵךְ אֶת בֵּית אַהֲרֹן: יְבָרֵךְ יִרְאֵי יְהֹוָה הַקְּטַנִּים עִם הַגְּדֹלִים: יֹסֵף יְהֹוָה עֲלֵיכֶם עֲלֵיכֶם וְעַל בְּנֵיכֶם: בְּרוּכִים אַתֶּם לַיְהֹוָה עֹשֵׂה שָׁמַיִם וָאָרֶץ: הַשָּׁמַיִם שָׁמַיִם לַיְהֹוָה וְהָאָרֶץ נָתַן לִבְנֵי אָדָם: לֹא הַמֵּתִים יְהַלְלוּ יָהּ וְלֹא כָּל יֹרְדֵי דוּמָה: וַאֲנַחְנוּ נְבָרֵךְ יָהּ מֵעַתָּה וְעַד עוֹלָם הַלְלוּיָהּ:",
      ),
    );
    if (isFullHallel) {
      hl.push(
        p(
          "<b>אָהַבְתִּי</b> כִּי יִשְׁמַע יְהֹוָה אֶת קוֹלִי תַּחֲנוּנָי: כִּי הִטָּה אָזְנוֹ לִי וּבְיָמַי אֶקְרָא: אֲפָפוּנִי חֶבְלֵי מָוֶת וּמְצָרֵי שְׁאוֹל מְצָאוּנִי צָרָה וְיָגוֹן אֶמְצָא: וּבְשֵׁם יְהֹוָה אֶקְרָא אָנָּה יְהֹוָה מַלְּטָה נַפְשִׁי: חַנּוּן יְהֹוָה וְצַדִּיק וֵאלֹהֵינוּ מְרַחֵם: שֹׁמֵר פְּתָאִים יְהֹוָה דַּלֹּתִי וְלִי יְהוֹשִׁיעַ: שׁוּבִי נַפְשִׁי לִמְנוּחָיְכִי כִּי יְהֹוָה גָּמַל עָלָיְכִי: כִּי חִלַּצְתָּ נַפְשִׁי מִמָּוֶת אֶת עֵינִי מִן דִּמְעָה אֶת רַגְלִי מִדֶּחִי: אֶתְהַלֵּךְ לִפְנֵי יְהֹוָה בְּאַרְצוֹת הַחַיִּים: הֶאֱמַנְתִּי כִּי אֲדַבֵּר אֲנִי עָנִיתִי מְאֹד: אֲנִי אָמַרְתִּי בְחָפְזִי כָּל הָאָדָם כֹּזֵב:",
        ),
      );
    }
    hl.push(
      p(
        "<b>מָה אָשִׁיב לַיְהֹוָה</b> כָּל תַּגְמוּלוֹהִי עָלָי: כּוֹס יְשׁוּעוֹת אֶשָּׂא וּבְשֵׁם יְהֹוָה אֶקְרָא: נְדָרַי לַיְהֹוָה אֲשַׁלֵּם נֶגְדָה נָּא לְכָל עַמּוֹ: יָקָר בְּעֵינֵי יְהֹוָה הַמָּוְתָה לַחֲסִידָיו: אָנָּה יְהֹוָה כִּי אֲנִי עַבְדֶּךָ אֲנִי עַבְדְּךָ בֶּן אֲמָתֶךָ פִּתַּחְתָּ לְמוֹסֵרָי: לְךָ אֶזְבַּח זֶבַח תּוֹדָה וּבְשֵׁם יְהֹוָה אֶקְרָא: נְדָרַי לַיְהֹוָה אֲשַׁלֵּם נֶגְדָה נָּא לְכָל עַמּוֹ: בְּחַצְרוֹת בֵּית יְהֹוָה בְּתוֹכֵכִי יְרוּשָׁלָיִם הַלְלוּיָהּ:",
      ),
    );
    hl.push(
      p(
        "<b>הַלְלוּ אֶת יְהֹוָה כָּל גּוֹיִם</b> שַׁבְּחוּהוּ כָּל הָאֻמִּים: כִּי גָבַר עָלֵינוּ חַסְדּוֹ וֶאֱמֶת יְהֹוָה לְעוֹלָם הַלְלוּיָהּ:",
      ),
    );
    hl.push(p("<b>הוֹדוּ לַיְהֹוָה כִּי טוֹב</b> כִּי לְעוֹלָם חַסְדּוֹ:"));
    hl.push(p("יֹאמַר נָא יִשְׂרָאֵל כִּי לְעוֹלָם חַסְדּוֹ:"));
    hl.push(p("יֹאמְרוּ נָא בֵית אַהֲרֹן כִּי לְעוֹלָם חַסְדּוֹ:"));
    hl.push(p("יֹאמְרוּ נָא יִרְאֵי יְהֹוָה כִּי לְעוֹלָם חַסְדּוֹ:"));
    hl.push(
      p(
        "<b>מִן הַמֵּצַר</b> קָרָאתִי יָהּ עָנָנִי בַמֶּרְחָב יָהּ: יְהֹוָה לִי לֹא אִירָא מַה יַּעֲשֶׂה לִי אָדָם: יְהֹוָה לִי בְּעֹזְרָי וַאֲנִי אֶרְאֶה בְשֹׂנְאָי: טוֹב לַחֲסוֹת בַּיְהֹוָה מִבְּטֹחַ בָּאָדָם: טוֹב לַחֲסוֹת בַּיְהֹוָה מִבְּטֹחַ בִּנְדִיבִים: כָּל גּוֹיִם סְבָבוּנִי בְּשֵׁם יְהֹוָה כִּי אֲמִילַם: סַבּוּנִי גַם סְבָבוּנִי בְּשֵׁם יְהֹוָה כִּי אֲמִילַם: סַבּוּנִי כִדְבוֹרִים דֹּעֲכוּ כְּאֵשׁ קוֹצִים בְּשֵׁם יְהֹוָה כִּי אֲמִילַם: דַּחֹה דְחִיתַנִי לִנְפֹּל וַיְהֹוָה עֲזָרָנִי: עָזִּי וְזִמְרָת יָהּ וַיְהִי לִי לִישׁוּעָה: קוֹל רִנָּה וִישׁוּעָה בְּאֹהֳלֵי צַדִּיקִים יְמִין יְהֹוָה עֹשָׂה חָיִל: יְמִין יְהֹוָה רוֹמֵמָה יְמִין יְהֹוָה עֹשָׂה חָיִל: לֹא אָמוּת כִּי אֶחְיֶה וַאֲסַפֵּר מַעֲשֵׂי יָהּ: יַסֹּר יִסְּרַנִּי יָהּ וְלַמָּוֶת לֹא נְתָנָנִי: פִּתְחוּ לִי שַׁעֲרֵי צֶדֶק אָבֹא בָם אוֹדֶה יָהּ: זֶה הַשַּׁעַר לַיְהֹוָה צַדִּיקִים יָבֹאוּ בוֹ:",
      ),
    );
    hl.push(
      p(
        "<b>אוֹדְךָ</b> כִּי עֲנִיתָנִי וַתְּהִי לִי לִישׁוּעָה: <small>אוֹדְךָ כִּי עֲנִיתָנִי וַתְּהִי לִי לִישׁוּעָה:</small> אֶבֶן מָאֲסוּ הַבּוֹנִים הָיְתָה לְרֹאשׁ פִּנָּה: <small>אֶבֶן מָאֲסוּ הַבּוֹנִים הָיְתָה לְרֹאשׁ פִּנָּה:</small> מֵאֵת יְהֹוָה הָיְתָה זֹּאת הִיא נִפְלָאת בְּעֵינֵינוּ: <small>מֵאֵת יְהֹוָה הָיְתָה זֹּאת הִיא נִפְלָאת בְּעֵינֵינוּ:</small> זֶה הַיּוֹם עָשָׂה יְהֹוָה נָגִילָה וְנִשְׂמְחָה בוֹ: <small>זֶה הַיּוֹם עָשָׂה יְהֹוָה נָגִילָה וְנִשְׂמְחָה בוֹ:</small>",
      ),
    );
    hl.push(
      p(
        "<b>אָנָּא יְהֹוָה הוֹשִׁיעָה נָּא</b>: אָנָּא יְהֹוָה הוֹשִׁיעָה נָּא:",
      ),
    );
    hl.push(
      p(
        "<b>אָנָּא יְהֹוָה הַצְלִיחָה נָּא</b>: אָנָּא יְהֹוָה הַצְלִיחָה נָּא:",
      ),
    );
    hl.push(
      p(
        "<b>בָּרוּךְ הַבָּא</b> בְּשֵׁם יְהֹוָה בֵּרַכְנוּכֶם מִבֵּית יְהֹוָה: <small>בָּרוּךְ הַבָּא בְּשֵׁם יְהֹוָה בֵּרַכְנוּכֶם מִבֵּית יְהֹוָה:</small> אֵל יְהֹוָה וַיָּאֶר לָנוּ אִסְרוּ חַג בַּעֲבֹתִים עַד קַרְנוֹת הַמִּזְבֵּחַ: <small>אֵל יְהֹוָה וַיָּאֶר לָנוּ אִסְרוּ חַג בַּעֲבֹתִים עַד קַרְנוֹת הַמִּזְבֵּחַ:</small> אֵלִי אַתָּה וְאוֹדֶךָּ אֱלֹהַי אֲרוֹמְמֶךָּ: <small>אֵלִי אַתָּה וְאוֹדֶךָּ אֱלֹהַי אֲרוֹמְמֶךָּ:</small> הוֹדוּ לַיְהֹוָה כִּי טוֹב כִּי לְעוֹלָם חַסְדּוֹ: <small>הוֹדוּ לַיְהֹוָה כִּי טוֹב כִּי לְעוֹלָם חַסְדּוֹ:</small>",
      ),
    );
    hl.push(
      p(
        "<small><b>יְהַלְלוּךָ</b> יְהֹוָה אֱלֹהֵינוּ כָּל מַעֲשֶׂיךָ וַחֲסִידֶיךָ צַדִּיקִים עוֹשֵׂי רְצוֹנֶךָ וְכָל עַמְּךָ בֵּית יִשְׂרָאֵל בְּרִנָּה יוֹדוּ וִיבָרְכוּ וִישַׁבְּחוּ וִיפָאֲרוּ וִירוֹמְמוּ וְיַעֲרִיצוּ וְיַקְדִּישׁוּ וְיַמְלִיכוּ אֶת שִׁמְךָ מַלְכֵּנוּ כִּי לְךָ טוֹב לְהוֹדוֹת וּלְשִׁמְךָ נָאֶה לְזַמֵּר כִּי מֵעוֹלָם וְעַד עוֹלָם אַתָּה אֵל: בָּרוּךְ אַתָּה יְהֹוָה מֶלֶךְ מְהֻלָּל בַּתִּשְׁבָּחוֹת:</small>",
      ),
    );
    hl.push(
      p(
        "<small><b>יִתְגַּדַּל</b> וְיִתְקַדַּשׁ שְׁמֵהּ רַבָּא בְּעָלְמָא דִּי בְרָא כִרְעוּתֵהּ וְיַמְלִיךְ מַלְכוּתֵהּ בְּחַיֵּיכוֹן וּבְיוֹמֵיכוֹן וּבְחַיֵּי דְכָל בֵּית יִשְׂרָאֵל בַּעֲגָלָא וּבִזְמַן קָרִיב וְאִמְרוּ אָמֵן: יְהֵא שְׁמֵהּ רַבָּא מְבָרַךְ לְעָלַם וּלְעָלְמֵי עָלְמַיָּא: עֹשֶׂה שָׁלוֹם בִּמְרוֹמָיו הוּא יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל יִשְׂרָאֵל וְאִמְרוּ אָמֵן:</small>",
      ),
    );
    parts.push(sup(hl.join("")));
  }
  ...
  // ─── אין כאלוהינו ───
  parts.push(p("<big><b>אֵין כֵּאלֹהֵֽינוּ</b></big>"));
  parts.push(
    p(
      "<b>קַוֵּ֗ה</b> אֶל־יְהֹ֫וָ֥ה חֲ֭זַק וְיַאֲמֵ֣ץ לִבֶּ֑ךָ וְ֝קַוֵּ֗ה אֶל־יְהֹוָֽה: אֵין־קָד֥וֹשׁ כַּיהֹוָ֖ה כִּ֣י אֵ֣ין בִּלְתֶּ֑ךָ וְאֵ֥ין צ֖וּר כֵּאלֹהֵֽינוּ׃ כִּ֤י מִ֣י אֱ֭לוֹהַּ מִבַּלְעֲדֵ֣י יְהֹוָ֑ה וּמִ֥י צ֝֗וּר זוּלָתִ֥י אֱלֹהֵֽינוּ׃",
    ),
  ...
  parts.push(
    p(
      "<b>אֵין</b> כֵּאלֹהֵֽינוּ, אֵין כַּאדוֹנֵֽינוּ, אֵין כְּמַלְכֵּֽינוּ, אֵין כְּמוֹשִׁיעֵֽינוּ: מִי כֵּאלֹהֵֽינוּ, מִי כַּאדוֹנֵֽינוּ, מִי כְּמַלְכֵּֽינוּ, מִי כְּמוֹשִׁיעֵֽינוּ: נוֹדֶה לֵאלֹהֵֽינוּ, נוֹדֶה לַאדוֹנֵֽינוּ, נוֹדֶה לְמַלְכֵּֽינוּ, נוֹדֶה לְמוֹשִׁיעֵֽינוּ: בָּרוּךְ אֱלֹהֵֽינוּ, בָּרוּךְ אֲדוֹנֵֽינוּ, בָּרוּךְ מַלְכֵּֽינוּ, בָּרוּךְ מוֹשִׁיעֵֽינוּ: אַתָּה הוּא אֱלֹהֵֽינוּ, אַתָּה הוּא אֲדוֹנֵֽינוּ, אַתָּה הוּא מַלְכֵּֽינוּ, אַתָּה הוּא מוֹשִׁיעֵֽינוּ:",
    ),
  );
  parts.push(hr);

  // ─── פיטום הקטורת ───
  parts.push(p("<big><b>פִּטּוּם הַקְּטֹֽרֶת</b></big>"));
  parts.push(
    p(
      "<b>אַתָּה</b> הוּא יְהֹוָה אֱלֹהֵֽינוּ, שֶׁהִקְטִֽירוּ אֲבוֹתֵֽינוּ לְפָנֶֽיךָ אֶת קְטֹֽרֶת הַסַּמִּים, בִּזְמַן שֶׁבֵּית הַמִּקְדָּשׁ קַיָּם, כַּאֲשֶׁר צִוִּֽיתָ אוֹתָם עַל יַד מֹשֶׁה נְבִיאָךְ, כַּכָּתוּב בְּתֽוֹרָתָךְ:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>תָּנֵי</b> בַר קַפָּרָא: אַחַת לְשִׁשִּׁים אוֹ לְשִׁבְעִים שָׁנָה הָֽיְתָה בָֽאָה שֶׁל שִׁירַֽיִם לַֽחֲצָאִין. וְעוֹד תָּנֵי בַר קַפָּרָא: אִלּוּ הָיָה נוֹתֵן בָּהּ קָרְטוֹב שֶׁל דְּבַשׁ, אֵין אָדָם יָכוֹל לַֽעֲמֹד מִפְּנֵי רֵיחָהּ. וְלָֽמָּה אֵין מְעָֽרְבִין בָּהּ דְּבַשׁ? מִפְּנֵי שֶׁהַתּוֹרָה אָֽמְרָה: כִּי כָל שְׂאֹר וְכָל דְּבַשׁ לֹא תַקְטִֽירוּ מִמֶּנּוּ אִשֶּׁה לַֽיהֹוָֽה:",
    ),
  );
  parts.push(
    p(
      "<b>תָּנָא</b> דְבֵי אֵלִיָּהוּ: כָּל הַשּׁוֹנֶה הֲלָכוֹת בְּכָל יוֹם, מֻבְטָח לוֹ שֶׁהוּא בֶן הָעוֹלָם הַבָּא, שֶׁנֶּאֱמַר: הֲלִיכוֹת עוֹלָם לוֹ. אַל תִּקְרֵי הֲלִיכוֹת אֶלָּא הֲלָכוֹת. אָמַר רִבִּי אֶלְעָזָר אָמַר רִבִּי חֲנִינָא: תַּלְמִידֵי חֲכָמִים מַרְבִּים שָׁלוֹם בָּעוֹלָם, שֶׁנֶּאֱמַר: וְכָל בָּנַיִךְ לִמּוּדֵי יְהֹוָה וְרַב שְׁלוֹם בָּנָֽיִךְ. אַל תִּקְרֵי בָּנָֽיִךְ אֶלָּא בּוֹנָֽיִךְ: יְהִי שָׁלוֹם בְּחֵילֵֽךְ שַׁלְוָה בְּאַרְמְנוֹתָֽיִךְ: לְמַעַן אַחַי וְרֵעָי אֲדַבְּרָה נָּא שָׁלוֹם בָּֽךְ: לְמַעַן בֵּית יְהֹוָה אֱלֹהֵֽינוּ אֲבַקְשָׁה טוֹב לָֽךְ: וּרְאֵה בָנִים לְבָנֶֽיךָ שָׁלוֹם עַל יִשְׂרָאֵֽל: שָׁלוֹם רָב לְאֹהֲבֵי תוֹרָתֶֽךָ וְאֵין לָמוֹ מִכְשׁוֹל: יְהֹוָה עֹז לְעַמּוֹ יִתֵּן יְהֹוָה יְבָרֵךְ אֶת עַמּוֹ בַשָּׁלוֹם:",
    ),
  );
  parts.push(sup('<small>ואומרים קדיש "על ישראל"</small>'));
  parts.push(
    p(
      "<small><b>יִתְגַּדַּל</b> וְיִתְקַדַּשׁ שְׁמֵהּ רַבָּא. <small>[אָמֵן]</small> בְּעָלְמָא דִּי בְרָא כִּרְעוּתֵהּ, וְיַמְלִיךְ מַלְכוּתֵהּ, וְיַצְמַח פֻּרְקָנֵהּ, וִיקָרֵב מְשִׁיחֵהּ. <small>[אָמֵן]</small> בְּחַיֵּיכוֹן וּבְיוֹמֵיכוֹן וּבְחַיֵּי דְכָל בֵּית יִשְׂרָאֵל, בַּעֲגָלָא וּבִזְמַן קָרִיב, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small> יְהֵא שְׁמֵהּ רַבָּא מְבָרַךְ לְעָלַם וּלְעָלְמֵי עָלְמַיָּא יִתְבָּרַךְ וְיִשְׁתַּבַּח וְיִתְפָּאַר וְיִתְרוֹמַם וְיִתְנַשֵּׂא וְיִתְהַדָּר וְיִתְעַלֶּה וְיִתְהַלָּל שְׁמֵהּ דְּקֻדְשָׁא בְּרִיךְ הוּא. <small>[אָמֵן]</small> לְעֵלָּא מִן כָּל בִּרְכָתָא שִׁירָתָא תֻּשְׁבְּחָתָא וְנֶחֱמָתָא דַּאֲמִירָן בְּעָלְמָא, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>",
    ),
  );
  parts.push(
    p(
      "<small><b>עַל</b> יִשְׂרָאֵל וְעַל רַבָּנָן וְעַל תַּלְמִידֵיהוֹן וְעַל כָּל תַּלְמִידֵי תַלְמִידֵיהוֹן, דְּעָסְקִין בְּאוֹרַיְתָא קַדִּישְׁתָּא, דִּי בְאַתְרָא הָדֵין וְדִי בְכָל אֲתַר וַאֲתַר. יְהֵא לָנָא וּלְהוֹן וּלְכוֹן חִנָּא וְחִסְדָּא וְרַחֲמֵי, מִן קֳדָם מָרֵה דִשְׁמַיָּא וְאַרְעָא, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>",
    ),
  );
  parts.push(
    p(
      "<small><b>יְהֵא</b> שְׁלָמָא רַבָּא מִן שְׁמַיָּא, חַיִּים טוֹבִים וְשָׂבָע וִישׁוּעָה וְנֶחָמָה וְשֵׁיזָבָא וּרְפוּאָה וּגְאֻלָּה וּסְלִיחָה וְכַפָּרָה וְרֶוַח וְהַצָּלָה לָנוּ וּלְכָל עַמּוֹ יִשְׂרָאֵל, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small><br><b>עוֹשֶׂה</b> שָׁלוֹם בִּמְרוֹמָיו הוּא בְּרַחֲמָיו יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל יִשְׂרָאֵל, וְאִמְרוּ אָמֵן. <small>[אָמֵן]</small></small>",
    ),
  );
  parts.push(hr);
...
function appendSfaradTallitTefillinSections(parts) {
  parts.push(
    '<p><big><b>הַלְבָּשַׁת טַלִּית</b></big></p><p><b>בָּרְ֒כִי נַפְשִׁי</b> אֶת־יְהֹוָה, יְהֹוָה אֱלֹהַי גָּדַֽלְתָּ מְאֹד, הוֹד וְהָדָר לָבָֽשְׁתָּ: עֹֽטֶה־אוֹר כַּשַּׂלְמָה, נוֹטֶה שָׁמַֽיִם כַּיְרִיעָה: לְשֵׁם יִחוּד קוּדְשָׁא בְּרִיךְ הוּא וּשְׁכִינְתֵּהּ בִּדְחִילוּ וּרְחִימוּ לְיַחֵד שֵׁם י"ה בו"ה בְּיִחוּדָא שְׁלִים בְּשֵׁם כָּל־יִשְׂרָאֵל הֲרֵינִי מִתְעַטֵּף גּוּפִי בַּצִּיצִית כֵּן תִּתְעַטֵּף נִשְׁמָתִי וּרְמַ"ח אֵיבָרַי וּשְׁסָ"ה גִידַי בְּאוֹר הַצִּיצִית הָעוֹלֶה תַּרְיַ"ג. וּכְשֵׁם שֶׁאֲנִי מִתְכַּסֶּה בַּטַּלִּית בָּעוֹלָם הַזֶּה כַּךְ אֶזְכֶּה לַחֲלוּקָא דְרַבָּנָן וּלְטַלִּית נָאָה לָעוֹלָם הַבָּא בְּגַן עֵֽדֶן. וְעַל יְדֵי מִצְוַת צִיצִית תִּנָּצֵל נַפְשִׁי רוּחִי וְנִשְׁמָתִי וּתְפִלָּתִי מִן הַחִיצוֹנִים וְהַטַּלִּית יִפְרֹשׂ כְּנָפָיו עֲלֵיהֶם וְיַצִּילֵם כְּנֶֽשֶׁר יָעִיר קִנּוֹ עַל גּוֹזָלָיו יְרַחֵף. וּתְהֵא חֲשׁוּבָה מִצְוַת צִיצִית לִפְנֵי הַקָּדוֹשׁ בָּרוּךְ הוּא כְּאִלּוּ קִיַּמְתִּֽיהָ בְּכָל פְּרָטֶֽיהָ וְדִקְדּוּקֶֽיהָ וְכַוָּנוֹתֶֽיהָ וְתַרְיַ"ג מִצְווֹת הַתְּ֒לוּיִם בָּהּ אָמֵן סֶֽלָה: <small>בשעה שמניח אותו על ראשו להתעטף בו, יאמר:</small> <b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר קִדְּ֒שָֽׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לְהִתְעַטֵּף בַּצִּיצִית: מַה־יָּקָר חַסְדְּ֒ךָ אֱלֹהִים וּבְנֵי אָדָם בְּצֵל כְּנָפֶֽיךָ יֶחֱסָֽיוּן: יִרְוְיֻן מִדֶּֽשֶׁן בֵּיתֶֽךָ וְנַֽחַל עֲדָנֶֽיךָ תַשְׁקֵם: כִּי־עִמְּ֒ךָ מְקוֹר חַיִּים, בְּאוֹרְ֒ךָ נִרְאֶה־אוֹר: מְשֹׁךְ חַסְדְּ֒ךָ לְיֹדְ֒עֶֽיךָ וְצִדְקָתְ֒ךָ לְיִשְׁרֵי־לֵב:</p><hr class="prayer-divider"><p><big><b>הַנָּחַת תְּפִלִּין</b></big></p><p>לְשֵׁם יִחוּד קוּדְשָׁא בְּרִיךְ הוּא וּשְׁכִינְתֵּהּ בִּדְחִילוּ וּרְחִימוּ, לְיַחֵד שֵׁם י"ה בו"ה בְּיִחוּדָא שְׁלִים בְּשֵׁם כָּל יִשְׂרָאֵל, הִנְנִי מְכַוֵּן בַּהֲנָחַת תְּפִלִּין לְקַיֵּם מִצְוַת בּוֹרְאִי שֶׁצִּוָּֽנוּ לְהָנִֽיחַ תְּפִלִּין כַּכָּתוּב בְּתוֹרָתוֹ וּקְשַׁרְתָּם לְאוֹת עַל־יָדֶֽךָ וְהָיוּ לְטֹטָפוֹת בֵּין עֵינֶֽיךָ וְהֵם אַרְבַּע פָּרָשִׁיּוֹת אֵֽלּוּ: שְׁמַע. וְהָיָה אִם־שָׁמֹֽעַ. קַדֶּשׁ־לִי. וְהָיָה כִּי־יְבִאֲךָ. שֶׁיֵּשׁ בָּהֶן יִחוּדוֹ וְאַחְדּוּתוֹ יִתְבָּרַךְ שְׁמוֹ בָּעוֹלָם וְשֶׁנִּזְכֹּר נִסִּים וְנִפְלָאוֹת שֶׁעָשָׂה עִמָּֽנוּ בְּהוֹצִיאָֽנוּ מִמִּצְרָֽיִם וַאֲשֶׁר־לוֹ הַכֹּֽחַ וְהַמֶּמְשָׁלָה בָּעֶלְיוֹנִים וּבַתַּחְתּוֹנִים לַעֲשׂוֹת בָּהֶם כִּרְצוֹנוֹ. וְצִוָּֽנוּ לְהָנִֽיחַ עַל־הַיָּד לְזִכְרוֹן זְרֽוֹעַ הַנְּ֒טוּיָה וְשֶׁהִיא נֶֽגֶד הַלֵּב לְשַׁעְבֵּד בָּזֶה תַּאֲווֹת וּמַחְשְׁבוֹת לִבֵּֽנוּ לַעֲבוֹדָתוֹ יִתְבָּרַךְ שְׁמוֹ. וְעַל־הָרֹאשׁ נֶֽגֶד הַמֹּֽחַ שֶׁהַנְּ֒שָׁמָה שֶׁבְּ֒מֹחִי עִם שְׁאָר חוּשַׁי וְכֹחוֹתַי כֻּלָּם יִהְיוּ מְשֻׁעְבָּדִים לַעֲבוֹדָתוֹ יִתְבָּרַךְ שְׁמוֹ. וּמִשֶּֽׁפַע מִצְוַת תְּפִלִּין יִתְמַשֵּׁךְ עָלַי לִהְיוֹת לִי חַיִּים אֲרֻכִּים וְשֶֽׁפַע קֹֽדֶשׁ וּמַחֲשָׁבוֹת קְדוֹשׁוֹת בְּלִי הִרְהוּר חֵטְא וְעָוֹן כְּלָל וְשֶׁלֹּא יְפַתֵּֽנוּ וְלֹא יִתְגָּרֶה־בָֽנוּ יֵֽצֶר הָרָע וְיַנִּיחֵֽנוּ לַעֲבֹד אֶת־יְיָ כַּאֲשֶׁר עִם־לְבָבֵֽנוּ. וִיהִי רָצוֹן לְפָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁתְּהֵא חֲשׁוּבָה מִצְוַת הֲנָחַת תְּפִלִּין לִפְנֵי הַקָּדוֹשׁ בָּרוּךְ הוּא כְּאִלּוּ קִיַּמְתִּֽיהָ בְּכָל־פְּרָטֶֽיהָ וְדִקְדּוּקֶֽיהָ וְכַוָּנוֹתֶֽיהָ וְתַרְיַ"ג מִצְוֹת הַתְּ֒לוּיוֹת בָּהּ אָמֵן סֶֽלָה: <small>יניח התפילין של יד על קיבורת ידו, וקודם שיהדק יברך:</small> <b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר קִדְּ֒שָֽׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לְהָנִיחַ תְּפִלִּין: <small>ויכרוך שבע כריכות סביב זרועו, ואחר כך יניח של ראש בגובה ראשו כנגד בין עיניו, וקודם שמהדקן יברך:</small> <b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם אֲשֶׁר קִדְּ֒שָֽׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ עַל מִצְוַת תְּפִלִּין: <small>ואחר שיהדק הרצועה על ראשו יאמר:</small> בָּרוּךְ שֵׁם כְּבוֹד מַלְכוּתוֹ לְעוֹלָם וָעֶד: <b>וּמֵחָכְמָתְ֒ךָ</b> אֵל עֶלְיוֹן תַּאֲצִיל עָלַי וּמִבִּינָתְ֒ךָ תְּבִינֵֽנִי וּבְחַסְדְּ֒ךָ תַּגְדִּיל עָלַי וּבִגְבוּרָתְ֒ךָ תַּצְמִית אוֹיְ֒בַי וְקָמַי וְשֶׁמֶן הַטּוֹב תָּרִיק עַל שִׁבְעָה קְנֵי הַמְּ֒נוֹרָה לְהַשְׁפִּֽיעַ טוּבְ֒ךָ לִבְרִיּוֹתֶֽיךָ: פּוֹתֵחַ אֶת־יָדֶֽךָ וּמַשְׂבִּֽיעַ לְכָל־חַי רָצוֹן: <small>ואח״כ כורך הרצועה של יד על אצבעו, ואומר:</small> <b>וְאֵרַשְׂתִּיךְ</b> לִי לְעוֹלָם, וְאֵרַשְׂתִּיךְ לִי בְּצֶֽדֶק וּבְמִשְׁפָּט וּבְחֶֽסֶד וּבְרַחֲמִים: וְאֵרַשְׂתִּיךְ לִי בֶּאֱמוּנָה, וְיָדַֽעַתְּ אֶת־יְהֹוָה: <small>אחר שיניח תפילין יאמר פרשיות \'קדש\' \'והיה כי יביאך\' כדי להשלים ד\' פרשיות בכל יום:</small> <b>וַיְדַבֵּר</b> יְהֹוָה אֶל־משֶׁה לֵּאמֹר: קַדֶּשׁ־לִי כָל־בְּכוֹר פֶּֽטֶר כָּל־רֶֽחֶם בִּבְנֵי יִשְׂרָאֵל בָּאָדָם וּבַבְּ֒הֵמָה לִי הוּא: וַיֹּֽאמֶר משֶׁה אֶל־הָעָם זָכוֹר אֶת־הַיּוֹם הַזֶּה אֲשֶׁר יְצָאתֶם מִמִּצְרַֽיִם מִבֵּית עֲבָדִים כִּי בְּחֹֽזֶק יָד הוֹצִיא יְהֹוָה אֶתְכֶם מִזֶּה וְלֹא יֵאָכֵל חָמֵץ: הַיּוֹם אַתֶּם יֹצְ֒אִים בְּחֹֽדֶשׁ הָאָבִיב: וְהָיָה כִי־יְבִיאֲךָ יְהֹוָה אֶל־אֶֽרֶץ הַכְּ֒נַעֲנִי וְהַחִתִּי וְהָאֱמֹרִי וְהַחִוִּי וְהַיְבוּסִי אֲשֶׁר נִשְׁבַּע לַאֲבֹתֶֽיךָ לָֽתֶת לָךְ אֶֽרֶץ זָבַת חָלָב וּדְבָשׁ וְעָבַדְתָּ אֶת־הָעֲבֹדָה הַזֹּאת בַּחֹֽדֶשׁ הַזֶּה: שִׁבְעַת יָמִים תֹּאכַל מַצֹּת וּבַיּוֹם הַשְּׁ֒בִיעִי חַג לַיהוָֹה: מַצּוֹת יֵאָכֵל אֵת שִׁבְעַת הַיָּמִים וְלֹא־יֵרָאֶה לְךָ חָמֵץ וְלֹא־יֵרָאֶה לְךָ שְׂאֹר בְּכָל־גְּבֻלֶֽךָ: וְהִגַּדְתָּ לְבִנְךָ בַּיּוֹם הַהוּא לֵאמֹר בַּֽעֲבוּר זֶה עָשָׂה יְהֹוָה לִי בְּצֵאתִי מִמִּצְרָֽיִם: וְהָיָה לְךָ לְאוֹת עַל־יָדְ֒ךָ וּלְזִכָּרוֹן בֵּין עֵינֶֽיךָ לְמַֽעַן תִּֽהְיֶה תּוֹרַת יְהֹוָה בְּפִֽיךָ כִּי בְּיָד חֲזָקָה הוֹצִאֲךָ יְהֹוָה מִמִּצְרָֽיִם: וְשָׁמַרְתָּ אֶת־הַחֻקָּה הַזֹּאת לְמוֹעֲדָהּ מִיָּמִים יָמִֽימָה: <b>וְהָיָה כִּי־יְבִאֲךָ</b> יְהֹוָה אֶל־אֶֽרֶץ הַכְּ֒נַעֲנִי כַּאֲשֶׁר נִשְׁבַּע לְךָ וְלַאֲבֹתֶֽיךָ וּנְתָנָהּ לָךְ: וְהַעֲבַרְתָּ כָל־פֶּֽטֶר־רֶֽחֶם לַֽיהוָֹה וְכָל פֶּֽטֶר שֶֽׁגֶר בְּהֵמָה אֲשֶׁר יִהְיֶה לְךָ הַזְּ֒כָרִים לַיהוָֹה: וְכָל־פֶּֽטֶר חֲמֹר תִּפְדֶּה בְשֶׂה וְאִם־לֹא תִפְדֶּה וַעֲרַפְתּוֹ וְכֹל בְּכוֹר אָדָם בְּבָנֶֽיךָ תִּפְדֶּה: וְהָיָה כִּי־יִשְׁאָלְ֒ךָ בִנְךָ מָחָר לֵאמֹר מַה־זֹּאת וְאָמַרְתָּ אֵלָיו בְּחֹֽזֶק יָד הוֹצִיאָֽנוּ יְהֹוָה מִמִּצְרַֽיִם מִבֵּית עֲבָדִים: וַיְהִי כִּי־הִקְשָׁה פַרְעֹה לְשַׁלְּ֒חֵנוּ וַיַּהֲרֹג יְהֹוָה כָּל־בְּכוֹר בְּאֶֽרֶץ מִצְרַֽיִם מִבְּֽ֒כֹר אָדָם וְעַד־בְּכוֹר בְּהֵמָה עַל־כֵּן אֲנִי זֹבֵֽחַ לַֽיהוָֹה כָּל־פֶּֽטֶר רֶֽחֶם הַזְּ֒כָרִים וְכָל־בְּכוֹר בָּנַי אֶפְדֶּה: וְהָיָה לְאוֹת עַל־יָדְ֒כָה וּלְטוֹטָפֹת בֵּין עֵינֶֽיךָ כִּי בְּחֹֽזֶק יָד הוֹצִיאָֽנוּ יְהֹוָה מִמִּצְרָֽיִם:</p><hr class="prayer-divider">',
  );
}

function appendSfaradPostShacharitToratEmetSections(parts) {
  parts.push("<p><big><b>" + "שש זכירות" + "</b></big></p>");
  parts.push(
    "<p>" +
      "כתבו המקובלים מצות עשה להזכיר בפה אלו הזכירות והנזהר בהם לאומרם מובטח לו שהוא בן עולם הבא זכירת יציאת מצרים: לְמַעַן תִּזְכֹּר אֶת יוֹם צֵאתְךָ מֵאֶרֶץ מִצְרַיִם כֹּל יְמֵי חַיֶּיךָ: זכירת מעמד הר סיני: רַק הִשָּׁמֶר לְךָ וּשְׁמֹר נַפְשְׁךָ מְאֹד פֶּן תִּשְׁכַּח אֶת הַדְּבָרִים אֲשֶׁר רָאוּ עֵינֶיךָ וּפֶן יָסוּרוּ מִלְּבָבְךָ כֹּל יְמֵי חַיֶּיךָ וְהוֹדַעְתָּם לְבָנֶיךָ וְלִבְנֵי בָנֶיךָ: יוֹם אֲשֶׁר עָמַדְתָּ לִפְנֵי יְהֹוָה אֱלֹהֶיךָ בְּחֹרֵב: זכירת מעשה עמלק ומחייתו: זָכוֹר אֵת אֲשֶׁר עָשָׂה לְךָ עֲמָלֵק בַּדֶּרֶךְ בְּצֵאתְכֶם מִמִּצְרָיִם: אֲשֶׁר קָרְךָ בַּדֶּרֶךְ וַיְזַנֵּב בְּךָ כָּל הַנֶּחֱשָׁלִים אַחַרֶיךָ וְאַתָּה עָיֵף וְיָגֵעַ וְלֹא יָרֵא אֱלֹהִים: וְהָיָה בְּהָנִיחַ יְהֹוָה אֱלֹהֶיךָ לְךָ מִכָּל אֹיְבֶיךָ מִסָּבִיב בָּאָרֶץ אֲשֶׁר יְהֹוָה אֱלֹהֶיךָ נֹתֵן לְךָ נַחֲלָה לְרִשְׁתָּהּ תִּמְחֶה אֶת זֵכֶר עֲמָלֵק מִתָּחַת הַשָּׁמָיִם לֹא תִּשְׁכָּח: זכירת מעשה העגל: זְכֹר אַל תִּשְׁכַּח אֵת אֲשֶׁר הִקְצַפְתָּ אֶת יְהֹוָה אֱלֹהֶיךָ בַּמִּדְבָּר: זכירת מעשה מרים: זָכוֹר אֵת אֲשֶׁר עָשָׂה יְהֹוָה אֱלֹהֶיךָ לְמִרְיָם בַּדֶּרֶךְ בְּצֵאתְכֶם מִמִּצְרָיִם: זכירת שבת: זָכוֹר אֶת יוֹם הַשַּׁבָּת לְקַדְּשׁוֹ:" +
      "</p>",
  );
  parts.push('<hr class="prayer-divider">');
  parts.push("<p><big><b>" + "שלשה עשר עיקרים" + "</b></big></p>");
  parts.push(
    "<p>" +
      "אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ הוּא בּוֹרֵא וּמַנְהִיג לְכָל הַבְּרוּאִים, וְהוּא לְבַדּו עָשָׂה וְעוֹשֶׂה וְיַעֲשֶׂה לְכָל הַמַּעֲשִׂים. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ הוּא יָחִיד וְאֵין יְחִידוּת כָּמוֹהוּ בְּשׁוּם פָּנִים, וְהוּא לְבַדּוֹ אֱלֹהֵינוּ, הָיָה הוֶה וְיִהְיֶה. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ אֵינוֹ גוּף, וְלֹא יַשִּׂיגוּהוּ מַשִּׂיגֵי הַגּוּף, וְאֵין לוֹ שׁוּם דִּמְיוֹן כְּלָל. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ הוּא רִאשׁוֹן וְהוּא אַחֲרוֹן. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ לוֹ לְבַדּוֹ רָאוּי לְהִתְפַּלֵּל, וְאֵין רָאוּי לְהִתְפַּלֵּל לְזוּלָתוֹ. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁכָּל דִּבְרֵי נְבִיאִים אֱמֶת. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁנְּבוּאַת מֹשֶׁה רַבֵּנוּ עָלָיו הַשָּׁלוֹם הָיְתָה אֲמִתִּית, וְשֶׁהוּא הָיָה אָב לַנְּבִיאִים, לַקּוֹדְמִים לְפָנָיו וְלַבָּאִים אַחֲרָיו. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁכָּל הַתּוֹרָה הַמְּצוּיָה עַתָּה בְיָדֵינוּ הִיא הַנְּתוּנָה לְמֹשֶׁה רַבֵּנוּ עָלָיו הַשָּׁלוֹם. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁזֹּאת הַתּוֹרָה לֹא תְהֵא מֻחְלֶפֶת וְלֹא תְהֵא תוֹרָה אַחֶרֶת מֵאֵת הַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ יוֹדֵעַ כָּל מַעֲשֵׂה בְנֵי אָדָם וְכָל מַחְשְׁבוֹתָם,שֶׁנֶּאֱמַר הַיֹּצֵר יַחַד לִבָּם הַמֵּבִין אֶל כָּל מַעֲשֵׂיהֶם. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁהַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ גּוֹמֵל טוֹב לְשׁוֹמְרֵי מִצְוֹתָיו וּמַעֲנִישׁ לְעוֹבְרֵי מִצְוֹתָיו. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, בְּבִיאַת הַמָּשִׁיחַ, וְאַף עַל פִּי שֶׁיִּתְמַהְמֵהַּ, עִם כָּל זֶה אֲחַכֶּה לּוֹ בְּכָל יוֹם שֶׁיָּבוֹא. אֲנִי מַאֲמִין בֶּאֱמוּנָה שְׁלֵמָה, שֶׁתִּהְיֶה תְּחִיַּת הַמֵּתִים בְּעֵת שֶׁיַעֲלֶה רָצוֹן מֵאֵת הַבּוֹרֵא יִתְבָּרַךְ שְׁמוֹ וְיִתְעַלֶּה זִכְרוֹ לָעַד וּלְנֵצַח נְצָחִים. לִישׁוּעָתְךָ קִוִיתִי יְהֹוָה: קִוִיתִי יְהֹוָה לִישׁוּעָתְךָ: יְהֹוָה לִישׁוּעָתְךָ קִוִיתִי: לְפוּרְקָנָךְ סַבָּרִית יְהֹוָה: סַבָּרִית יְהֹוָה לְפוּרְקָנָךְ: יְהֹוָה לְפוּרְקָנָךְ סַבָּרִית: אֲדוֹן עוֹלָם אֲשֶׁר מָלַךְ, בְּטֶרֶם כָּל יְצִיר נִבְרָא: לְעֵת נַעֲשָׂה בְחֶפְצו כֹּל, אֲזַי מֶלֶךְ שְׁמוֹ נִקְרָא: וְאַחֲרֵי כִּכְלוֹת הַכֹּל, לְבַדּוֹ יִמְלֹךְ נוֹרָא: וְהוּא הָיָה, וְהוּא הֹוֶה, וְהוּא יִהְיֶה בְּתִפְאָרָה: וְהוּא אֶחָד וְאֵין שֵׁנִי, לְהַמְשִׁיל לוֹ לְהַחְבִּירָה: בְּלִי רֵאשִׁית בְּלִי תַכְלִית, וְלוֹ הָעֹז וְהַמִּשְׂרָה: וְהוּא אֵלִי וְחַי גוֹאֲלִי, וְצוּר חֶבְלִי בְּעֵת צָרָה: וְהוּא נִסִּי וּמָנוֹס לִי, מְנָת כּוֹסִי בְּיום אֶקְרָא: בְּיָדוֹ אַפְקִיד רוּחִי, בְּעֵת אִישָׁן וְאָעִירָה: וְעִם רוּחִי גְּוִיָּתִי, יְהֹוָה לִי וְלֹא אִירָא:" +
      "</p>",
  );
  parts.push('<hr class="prayer-divider">');
}
function buildShacharitSfaradPayload(context) {
  ...
  var isTachanunDay =
    ...
    !context2.isShavuot &&
    !context2.isSukkot &&
    !context2.isRoshHaShana &&
    !context2.isYomKippur &&
    !context2.isShabbat &&
  ...
  var mon = displayDate.getMonth();
  var isSummer =
    context2.isPesach || (mon >= 3 && mon <= 8 && !context2.isSukkot);
  var parts = [];
  ...
  parts.push(
    p(
      "<b>מוֹדֶה אֲנִי</b> לְפָנֶֽיךָ מֶֽלֶךְ חַי וְקַיָּם שֶׁהֶחֱזַֽרְתָּ בִּי נִשְׁמָתִי בְּחֶמְלָה, רַבָּה אֱמוּנָתֶֽךָ:",
    ),
  );
  parts.push(hr);
  ...
  parts.push(
    p(
      "<b>וַיְדַבֵּר</b> יְהֹוָה אֶל מֹשֶׁה לֵאמֹר דַבֵּר אֶל אַהֲרֹן וְאֶל בָּנָיו לֵאמֹר כֹּה תְבָרְכוּ אֶת בְּנֵי יִשְׂרָאֵל אָמוֹר לָהֶם:",
    ),
  );
  parts.push(
    p(
      "<b>יְבָרֶכְ֒ךָ</b> יְהֹוָה וְיִשְׁמְרֶֽךָ: יָאֵר יְהֹוָה פָּנָיו אֵלֶֽיךָ וִיחֻנֶּֽךָּ: יִשָּׂא יְהֹוָה פָּנָיו אֵלֶֽיךָ וְיָשֵׂם לְךָ שָׁלוֹם:",
    ),
  );
  parts.push(
    p("<b>וְשָׂמוּ</b> אֶת שְׁמִי עַל בְּנֵי יִשְׂרָאֵל וַאֲנִי אַבָרְכֵם:"),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם הַּנוֹתֵן לַשֶּֽׂכְוִי בִינָה לְהַבְחִין בֵּין יוֹם וּבֵין לָֽיְלָה:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁלֹּא עָשַֽׂנִי אִשָּׁה: <small>נשים אומרות:</small>",
    ),
  );
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם שֶׁעָשַֽׂנִי כִּרְצוֹנוֹ:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵֽינוּ מֶֽלֶךְ הָעוֹלָם הַמֵּכִין מִצְעֲדֵי גָֽבֶר: <small>בט' אב ויוה\"כ אין אומרים ברכה זו:</small>",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>וְשָׁחַט</b> אֹתוֹ עַל יֶֽרֶךְ הַמִּזְבֵּֽחַ צָפֹֽנָה לִפְנֵי יְהֹוָה וְזָרְקוּ בְּנֵי אַהֲרֹן הַכֹּהֲנִים אֶת־דָּמוֹ עַל־הַמִּזְבֵּֽחַ סָבִיב: יְהִי רָצוֹן מִלְּ֒פָנֶֽיךָ, יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ, שֶׁתְּ֒הֵא אֲמִירָה זוּ, חֲשׁוּבָה וּמְקֻבֶּֽלֶת, וּמְרֻצָּה לְפָנֶֽיךָ, כְּאִלּוּ הִקְרַֽבְנוּ קָרְבַּן הַתָּמִיד בְּמוֹעֲדוֹ, וּבִמְ֒קוֹמוֹ וּכְהִלְכָתוֹ:",
    ),
  );
  parts.push(
    p(
      "<b>אַתָּה הוּא</b> יְהֹוָה אֱלֹהֵֽינוּ שֶׁהִקְטִֽירוּ אֲבוֹתֵֽינוּ לְפָנֶֽיךָ אֶת קְטֹֽרֶת הַסַּמִּים בִּזְמַן שֶׁבֵּית הַמִּקְדָּשׁ הָיָה קַיָּם כַּאֲשֶׁר צִוִּיתָ אוֹתָם עַל יְדֵי משֶׁה נְבִיאֶֽךָ כַּכָּתוּב בְּתוֹרָתֶֽךָ:",
    ),
  );
  parts.push(
    p(
      "<b>וַיֹּֽאמֶר</b> יְהֹוָה אֶל־משֶׁה קַח־לְךָ סַמִּים נָטָף וּשְׁחֵֽלֶת וְחֶלְבְּנָה סַמִּים וּלְבֹנָה זַכָּה בַּד בְּבַד יִהְיֶה: וְעָשִֽׂיתָ אֹתָהּ קְטֹֽרֶת רֹֽקַח מַעֲשֵׂה רוֹקֵֽחַ מְמֻלָּח טָהוֹר קֹֽדֶשׁ: וְשָׁחַקְתָּ מִמֶּֽנָּה הָדֵק וְנָתַתָּה מִמֶּֽנָּה לִפְנֵי הָעֵדֻת בְּאֹֽהֶל מוֹעֵד אֲשֶׁר אִוָּעֵד לְךָ שָֽׁמָּה קֹֽדֶשׁ קָדָשִׁים תִּהְיֶה לָכֶם: וְנֶאֱמַר <b>וְהִקְטִיר</b> עָלָיו אַהֲרֹן קְטֹֽרֶת סַמִּים בַּבֹּֽקֶר בַּבֹּֽקֶר בְּהֵיטִיבוֹ אֶת־הַנֵּרֹת יַקְטִירֶֽנָּה: וּבְהַעֲלֹת אַהֲרֹן אֶת־הַנֵּרֹת בֵּין הָֽעַרְבַּֽיִם יַקְטִירֶֽנָּה קְטֹֽרֶת תָּמִיד לִפְנֵי יְהֹוָה לְדֹרֹתֵיכֶם:",
    ),
  );
  parts.push(
    p(
      "<b>תָּנוּ רַבָּנָן</b> פִּטּוּם הַקְּ֒טֹֽרֶת כֵּיצַד שְׁלֹש מֵאוֹת וְשִׁשִּׁים וּשְׁמוֹנָה מָנִים הָיוּ בָהּ, שְׁלֹש מֵאוֹת וְשִׁשִּׁים וַחֲמִשָּׁה כְּמִנְיַן יְמוֹת הַחַמָּה מָנֶה לְכָל יוֹם, פְּרַס בַּשַּׁחֲרִית וּפְרַס בֵּין הָעַרְבָּֽיִם, וּשְׁלשָׁה מָנִים יְתֵרִים שֶׁמֵּהֶם מַכְנִיס כֹּהֵן גָּדוֹל מְלֹא חָפְנָיו בְּיוֹם הַכִּפּוּרִים, וּמַחֲזִירָן לְמַכְתֶּֽשֶׁת בְּעֶֽרֶב יוֹם הַכִּפּוּרִים, וְשׁוֹחֲקָן יָפֶה יָפֶה כְּדֵי שֶׁתְּ֒הֵא דַקָּה מִן הַדַּקָּה, וְאַחַד עָשָׂר סַמָּנִים הָֽיוּ בָהּ, וְאֵֽלּוּ הֵן הַצֳּרִי וְהַצִּפֹּֽרֶן הַחֶלְבְּנָה וְהַלְּ֒בוֹנָה מִשְׁקַל שִׁבְעִים שִׁבְעִים מָנֶה, מוֹר וּקְצִיעָה שִׁבֹּֽלֶת נֵרְדְּ וְכַרְכֹּם מִשְׁקַל שִׁשָּׁה עָשָׂר שִׁשָּׁה עָשָׂר מָנֶה, הַקּשְׁטְ שְׁנֵים עָשָׂר וְקִלּוּפָה שְׁלשָׁה וְקִנָּמוֹן תִּשְׁעָה, בֹּרִית כַּרְשִׁינָה תִּשְׁעָה קַבִּין, יֵין קַפְרִיסִין סְאִין תְּלָתָא וְקַבִּין תְּלָתָא, וְאִם אֵין לוֹ יֵין קַפְרִיסִין מֵבִיא חֲמַר חִוַּרְיָן עַתִּיק, מֶֽלַח סְדוֹמִית רֹֽבַע הַקָּב, מַעֲלֶה עָשָׁן כָּל שֶׁהוּא, רַבִּי נָתָן הַבַּבְלִי אוֹמֵר אַף כִּפַּת הַיַּרְדֵּן כָּל שֶׁהוּא, וְאִם נָֽתַן בָּהּ דְּבַשׁ פְּסָלָהּ וְאִם חִסַּר אַחַת מִכָּל סַמָּנֶֽיהָ חַיָּב מִיתָה:",
    ),
  );
  parts.push(
    p(
      "<b>רַבָּן</b> שִׁמְעוֹן בֶּן גַּמְלִיאֵל אוֹמֵר הַצֳּרִי אֵינוֹ אֶלָּא שְׂרָף הַנּוֹטֵף מֵעֲצֵי הַקְּ֒טָף, בֹּרִית כַּרְשִׁינָה שֶׁשָׁפִין בָּה אֶת הַצִפֹּרֶן כִּדֵי שֶׁתְּ֒הֵא נָאָה יֵין קַפְרִיסִין שֶׁשׁוֹרִין בּוֹ אֶת הַצִפֹּרֶן כְּדֵי שֶׁתְּ֒הֵא עַזָּה, וַהֲלֹא מֵי רַגְלַֽיִם יָפִין לָהּ אֶלָּא שֶׁאֵין מַכְנִיסִין מֵי רַגְלַֽיִם בַּמִּקְדָּשׁ מִפְּנֵי הַכָּבוֹד:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>תַּנְיָא</b> בַּר קַפָּרָא אוֹמֵר אַחַת לְשִׁשִּׁים אוֹ לְשִׁבְעִים שָׁנָה הָיְ֒תָה בָאָה שֶׁל שִׁירַֽיִם לַחֲצָאִין: וְעוֹד תָּנֵי בַּר קַפָּרָא אִלּוּ הָיָה נוֹתֵן בָּהּ קוֹרְ֒טוֹב שֶׁל דְּבַשׁ אֵין אָדָם יָכוֹל לַעֲמֹד מִפְּ֒נֵי רֵיחָהּ וְלָֽמָּה אֵין מְעָרְ֒בִין בָּהּ דְּבַשׁ מִפְּ֒נֵי שֶׁהַתּוֹרָה אָמְ֒רָה כִּי כָל־שְׂאֹר וְכָל־דְּבַשׁ לֹא־תַקְטִֽירוּ מִמֶּֽנּוּ אִשֶּׁה לַיהוָֹה: <small>אומר ג' פעמים:</small> יְהֹוָה צְבָאוֹת עִמָּֽנוּ מִשְׂגַּב־לָֽנוּ אֱלֹהֵי יַעֲקֹב סֶֽלָה: יְהֹוָה צְבָאוֹת אַשְׁרֵי אָדָם בֹּטֵֽחַ בָּךְ: יְהֹוָה הוֹשִֽׁיעָה הַמֶּֽלֶךְ יַעֲנֵֽנוּ בְיוֹם־קָרְאֵֽנוּ: אַתָּה סֵֽתֶר לִי מִצַּר תִּצְּ֒רֵֽנִי רָנֵּי פַלֵּט תְּסוֹבְ֒בֵֽנִי סֶֽלָה: וְעָרְ֒בָה לַיהוָֹה מִנְחַת יְהוּדָה וִירוּשָׁלָֽםִ כִּימֵי עוֹלָם וּכְשָׁנִים קַדְמֹנִיּוֹת:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>אָֽנָּא</b> בְּכֹֽחַ גְּדֻלַּת יְמִינְ֒ךָ תַּתִּיר צְרוּרָה: קַבֵּל רִנַּת עַמְּ֒ךָ שַׂגְּ֒בֵֽנוּ טַהֲרֵֽנוּ נוֹרָא: נָא גִבּוֹר דּוֹרְ֒שֵׁי יִחוּדְ֒ךָ כְּבָבַת שָׁמְ֒רֵם: בָּרְ֒כֵם טַהֲרֵם רַחֲמֵם צִדְקָתְ֒ךָ תָּמִיד גָּמְ֒לֵם: חֲסִין קָדוֹשׁ בְּרֹב טוּבְ֒ךָ נַהֵל עֲדָתֶֽךָ: יָחִיד גֵּאֶה לְעַמְּ֒ךָ פְּנֵה, זוֹכְ֒רֵי קְדֻשָּׁתֶֽךָ: שַׁוְעָתֵֽנוּ קַבֵּל וּשְׁמַע צַעֲקָתֵֽנוּ יוֹדֵֽעַ תַּעֲלוּמוֹת: בָּרוּךְ שֵׁם כְּבוֹד מַלְכוּתוֹ לְעוֹלָם וָעֶד:",
    ),
  );
  parts.push(
    p(
      "<b>רִבּוֹן הָעוֹלָמִים</b> אַתָּה צִוִּיתָֽנוּ לְהַקְרִיב קָרְבַּן הַתָּמִיד בְּמוֹעֲדוֹ וְלִהְיוֹת כֹּהֲנִים בַּעֲבוֹדָתָם וּלְוִיִּם בְּדוּכָנָם וְיִשְׂרָאֵל בְּמַעֲמָדָם, וְעַתָּה בַּעֲוֹנוֹתֵֽינוּ חָרַב בֵּית הַמִּקְדָּשׁ וּבֻטַּל הַתָּמִיד וְאֵין לָֽנוּ לֹא כֹהֵן בַּעֲבוֹדָתוֹ וְלֹא לֵוִי בְּדוּכָנוֹ וְלֹא יִשְׂרָאֵל בְּמַעֲמָדוֹ. לָכֵן יְהִי רָצוֹן מִלְּ֒פָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁיְּ֒הֵא שִֽׂיחַ שִׂפְתוֹתֵֽינוּ חָשׁוּב וּמְקֻבָּל וּמְרֻצֶּה לְפָנֶֽיךָ כְּאִלּוּ הִקְרַֽבְנוּ קָרְבַּן הַתָּמִיד בְּמוֹעֲדוֹ וְעָמַֽדְנוּ עַל מַעֲמָדוֹ. כְּמָה שֶׁנֶּאֱמַר וּנְשַׁלְּ֒מָה פָרִים שְׂפָתֵֽינוּ: וְנֶאֶמַר וְשָׁחַט אֹתוֹ עַל יֶֽרֶךְ הַמִּזְבֵּֽחַ צָפֹֽנָה לִפְנֵי יְהֹוָה וְזָרְ֒קוּ בְּנֵי אַהֲרֹן הַכֹּהֲנִים אֶת־דָּמוֹ עַל־הַמִּזְבֵּֽחַ סָבִיב: וְנֶאֱמַר זֹאת הַתּוֹרָה לָעֹלָה לַמִּנְחָה וְלַחַטָּאת וְלָאָשָׁם וְלַמִּלּוּאִים וּלְזֶֽבַח הַשְּׁ֒לָמִים: <small>בראש חודש מוסיפים:</small>",
    ),
  );
  parts.push(
    p(
      '<b>וּבְרָאשֵׁי חָדְשֵׁיכֶם</b> תַּקְרִֽיבוּ עֹלָה לַיהוָֹה פָּרִים בְּנֵי־בָקָר שְׁנַֽיִם וְאַֽיִל אֶחָד כְּבָשִׂים בְּנֵי־שָׁנָה שִׁבְעָה תְּמִימִם: וּשְׁלשָׁה עֶשְׂרֹנִים סֹֽלֶת מִנְחָה בְּלוּלָה בַשֶּֽׁמֶן לַפָּר הָאֶחָד וּשְׁ֒נֵי עֶשְׂרֹנִים סֹֽלֶת מִנְחָה בְּלוּלָה בַשֶּֽׁמֶן לָאַֽיִל הָאֶחָד: וְעִשָּׂרֹן עִשָּׂרוֹן סֹֽלֶת מִנְחָה בְּלוּלָה בַשֶּֽׁמֶן לַכֶּֽבֶשׂ הָאֶחָד עֹלָה רֵֽיחַ נִיחֹֽחַ אִשֶּׁה לַיהוָֹה: וְנִסְכֵּיהֶם חֲצִי הַהִין יִהְיֶה לַפָּר וּשְׁלִישִׁת הַהִין לָאַֽיִל וּרְבִיעִת הַהִין לַכֶּֽבֶשׂ יָֽיִן זֹאת עֹלַת חֹֽדֶשׁ בְּחָדְשׁוֹ לְחָדְשֵׁי הַשָּׁנָה: וּשְׂעִיר עִזִּים אֶחָד לְחַטָּאת לַיהוָֹה עַל עֹלַת הַתָּמִיד יֵעָשֶׂה וְנִסְכּוֹ: <small>קבעו לשנות אחר פרשת התמיד פרק איזהו מקומן וברייתא דרבי ישמעאל כדי שיזכה כל אדם ללמוד בכל יום מקרא משנה וגמרא. דברייתא דרבי ישמעאל הוי במקום גמרא, שהמדרש כגמרא: (שו"ע או"ח נ, א)</small> <small>הא דתקינו לשנות פרק \'איזהו מקומן\', לפי שאין בכל אותו פרק מחלוקת והיא משנה ברורה למשה מסיני. (ב"י)</small> <small>משנה א</small>',
    ),
  );
  parts.push(
    p(
      "<b>אֵיזֶהוּ מְקוֹמָן</b> שֶׁל זְבָחִים קָדְשֵׁי קָדָשִׁים שְׁחִיטָתָן בַּצָּפוֹן פָּר וְשָׂעִיר שֶׁל יוֹם הַכִּפּוּרִים שְׁחִיטָתָן בַּצָּפוֹן וְקִבּוּל דָּמָן בִּכְלִי שָׁרֵת בַּצָּפוֹן וְדָמָן טָעוּן הַזָּיָה עַל בֵּין הַבַּדִּים וְעַל הַפָּרֹֽכֶת וְעַל מִזְבַּח הַזָּהָב מַתָּנָה אַחַת מֵהֶן מְעַכָּֽבֶת: שְׁיָרֵי הַדָּם הָיָה שׁוֹפֵךְ עַל יְסוֹד מַעֲרָבִי שֶׁל מִזְבֵּֽחַ הַחִיצוֹן אִם לֹא נָתַן לֹא עִכֵּב: <small>משנה ב</small>",
    ),
  );
  parts.push(
    p(
      "<b>פָּרִים</b> הַנִּשְׂרָפִים וּשְׂעִירִים הַנִּשְׂרָפִים שְׁחִיטָתָן בַּצָּפוֹן וְקִבּוּל דָּמָן בִּכְלִי שָׁרֵת בַּצָּפוֹן וְדָמָן טָעוּן הַזָּיָה עַל הַפָּרֹֽכֶת וְעַל מִזְבַּח הַזָּהָב מַתָּנָה אַחַת מֵהֶן מְעַכָּֽבֶת: שְׁיָרֵי הַדָּם הָיָה שׁוֹפֵךְ עַל יְסוֹד מַעֲרָבִי שֶׁל מִזְבֵּֽחַ הַחִיצוֹן אִם לֹא נָתַן לֹא עִכֵּב אֵֽלּוּ וָאֵֽלּוּ נִשְׂרָפִין בְּבֵית הַדָּֽשֶׁן: <small>משנה ג</small>",
    ),
  );
  parts.push(
    p(
      "<b>חַטֹּאת</b> הַצִּבּוּר וְהַיָּחִיד אֵֽלּוּ הֵן חַטֹּאת הַצִּבּוּר שְׂעִירֵי רָאשֵׁי חֳדָשִׁים וְשֶׁל מוֹעֲדוֹת שְׁחִיטָתָן בַּצָּפוֹן וְקִבּוּל דָּמָן בִּכְלִי שָׁרֵת בַּצָּפוֹן וְדָמָן טָעוּן אַרְבַּע מַתָּנוֹת עַל אַרְבַּע קְרָנוֹת, כֵּיצַד עָלָה בַכֶּֽבֶשׁ וּפָנָה לַסּוֹבֵב וּבָא לוֹ לְקֶֽרֶן דְּרוֹמִית מִזְרָחִית, מִזְרָחִית צְפוֹנִית, צְפוֹנִית מַעֲרָבִית, מַעֲרָבִית דְּרוֹמִית, שְׁיָרֵי הַדָּם הָיָה שׁוֹפֵךְ עַל יְסוֹד דְּרוֹמִי וְנֶאֱכָלִין לִפְנִים מִן הַקְּ֒לָעִים לְזִכְרֵי כְהֻנָּה בְּכָל מַאֲכָל לְיוֹם וָלַֽיְלָה עַד חֲצוֹת:",
    ),
  );
  parts.push(
    p(
      "<b>יְהִי רָצוֹן</b> מִלְּ֒פָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ אִם נִתְחַיַּֽבְתִּי חַטָּאת שֶּׁתְּ֒הֵא אֲמִֽירָה זוּ מְרֻצָּה לְפָנֶֽיךָ כְּאִֽלּוּ הִקְרַֽבְתִּי חַטָּאת: <small>משנה ד</small>",
    ),
  );
  parts.push(
    p(
      "<b>הָעוֹלָה</b> קֹֽדֶשׁ קָדָשִׁים שְׁחִיטָתָהּ בַּצָּפוֹן וְקִבּוּל דָּמָהּ בִּכְלִי שָׁרֵת בַּצָּפוֹן וְדָמָהּ טָעוּן שְׁתֵּי מַתָּנוֹת שֶׁהֵן אַרְבַּע וּטְעוּנָה הֶפְשֵׁט וְנִתּֽוּחַ וְכָלִיל לָאִשִּׁים:",
    ),
  );
  parts.push(
    p(
      "<b>יְהִי רָצוֹן</b> כְּאִֽלּוּ הִקְרַֽבְתִּי עוֹלָה: <small>משנה ה</small>",
    ),
  );
  parts.push(
    p(
      "<b>זִבְחֵי</b> שַׁלְמֵי צִבּוּר וַאֲשָׁמוֹת, אֵֽלּוּ הֵן אֲשָׁמוֹת אֲשַׁם גְּזֵלוֹת אֲשַׁם מְעִילוֹת אֲשַׁם שִׁפְחָה חֲרוּפָה אֲשַׁם נָזִיר אֲשַׁם מְצוֹרָע אָשָׁם תָּלוּי, שְׁחִיטָתָן בַּצָּפוֹן וְקִבּוּל דָּמָן בִּכְלִי שָׁרֵת בַּצָּפוֹן, וְדָמָן טָעוּן שְׁתֵּי מַתָּנוֹת שֶׁהֵן אַרְבַּע, וְנֶאֱכָלִין לִפְנִים מִן הַקְּ֒לָעִים לְזִכְרֵי כְהֻנָּה בְּכָל מַאֲכָל לְיוֹם וָלַֽיְלָה עַד חֲצוֹת:",
    ),
  );
  parts.push(
    p(
      "<b>יְהִי רָצוֹן</b> מִלְּ֒פָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ אִם נִתְחַיַּֽבְתִּי אָשָׁם שֶּׁתְּ֒הֵא אֲמִֽירָה זוּ מְרֻצָּה לְפָנֶֽיךָ כְּאִֽלּוּ הִקְרַֽבְתִּי אָשָׁם: <small>משנה ו</small>",
    ),
  );
  parts.push(
    p(
      "<b>הַתּוֹדָה</b> וְאֵיל נָזִיר קָדָשִׁים קַלִּים שְׁחִיטָתָן בְּכָל־מָקוֹם בָּעֲזָרָה וְדָמָן טָעוּן שְׁתֵּי מַתָּנוֹת שֶׁהֵן אַרְבַּע, וְנֶאֱכָלִין בְּכָל־הָעִיר לְכָל־אָדָם בְּכָל־מַאֲכָל לְיוֹם וָלַֽיְלָה עַד־חֲצוֹת, הַמּוּרָם מֵהֶם כַּיּוֹצֵא בָהֶם אֶלָּא שֶׁהַמּוּרָם נֶאֱכָל לַכֹּהֲנִים לִנְשֵׁיהֶם וְלִבְנֵיהֶם וּלְעַבְדֵיהֶם:",
    ),
  );
  parts.push(
    p(
      "<b>יְהִי רָצוֹן</b> כְּאִֽלּוּ הִקְרַֽבְתִּי תּוֹדָה: <small>משנה ז</small>",
    ),
  );
  parts.push(
    p(
      "<b>שְׁלָמִים</b> קָדָשִׁים קַלִּים שְׁחִיטָתָן בְּכָל מָקוֹם בָּעֲזָרָה וְדָמָן טָעוּן שְׁתֵּי מַתָּנוֹת שֶׁהֵן אַרְבַּע, וְנֶאֱכָלִין בְּכָל הָעִיר לְכָל אָדָם בְּכָל מַאֲכָל לִשְׁנֵי יָמִים וְלַֽיְלָה אֶחָד: הַמּוּרָם מֵהֶם כַּיּוֹצֵא בָהֶם אֶלָּא שֶׁהַמּוּרָם נֶאֱכָל לַכֹּהֲנִים לִנְשֵׁיהֶם וְלִבְנֵיהֶם וּלְעַבְדֵיהֶם:",
    ),
  );
  parts.push(
    p(
      "<b>יְהִי רָצוֹן</b> כְּאִֽלּוּ הִקְרַֽבְתִּי שְׁלָמִים: <small>משנה ח</small>",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<small>המנהג הפשוט בכל תפוצות ישראל לומר ברכו לפני פריסת שמע. ואין לאומרו בפחות מעשרה שכל דבר שבקדושה אינו בפחות מעשרה (אבודרהם).</small> <small>ואומר שליח צבור:</small>",
    ),
  );
  parts.push(
    p("<b>בָּרְ֒כוּ אֶת יְהֹוָה הַמְּ֒בֹרָךְ:</b> <small>ועונין הקהל:</small>"),
  );
  parts.push(
    p(
      '<b>בָּרוּךְ</b> יְהֹוָה הַמְּ֒בֹרָךְ לְעוֹלָם וָעֶד: <small>אין להפסיק כלל בין ברכו ליוצר אור אפילו לדבר מצוה (לבוש). איתא בכתבים בתחלת יוצר אור צריך למשמש בתפילין. לכן כשיאמר יוצר אור ימשמש בשל יד. ובורא חושך ימשמש בשל ראש (פע"ח).</small>',
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "מֶֽלֶךְ עוֹזֵר וּמוֹשִֽׁיעַ וּמָגֵן: בָּרוּךְ אַתָּה יְהֹוָה מָגֵן אַבְרָהָם: <b>גבורות</b> <b>אַתָּה גִבּוֹר</b> לְעוֹלָם אֲדֹנָי מְחַיֶּה מֵתִים אַתָּה רַב לְהוֹשִֽׁיעַ:",
    ),
  );
  if (isSummer) {
    parts.push(p("מוֹרִיד הַטָּל"));
  ...
  parts.push(
    sup(
      '<small>בחזרת הש"ץ אומרים כאן קדושה:</small> <b>קדושה</b> <small>קהל וחזן:</small> <b>נַקְדִישָׁךְ</b> וְנַעֲרִיצָךְ כְּנֹֽעַם שִֽׂיחַ סוֹד שַׂרְפֵי קֽוֹדֶשׁ הַמְּ֒שַׁלְּ֒שִׁים לְךָ קְדֻשָּׁה כַּכָּתוּב עַל יַד נְבִיאֶֽךָ וְקָרָא זֶה אֶל זֶה וְאָמַר: <small>קהל וחזן:</small> <b>קָדוֹשׁ קָדוֹשׁ קָדוֹשׁ יְהֹוָה צְבָאוֹת מְלֹא כָל הָאָֽרֶץ כְּבוֹדוֹ:</b> <small>שליח ציבור:</small> לְעֻמָּתָם מְשַׁבְּ֒חִים וְאוֹמְ֒רִים: <small>קהל וחזן:</small> <b>בָּרוּךְ כְּבוֹד יְהֹוָה מִמְּ֒קוֹמוֹ:</b> <small>שליח ציבור:</small> וּבְדִבְרֵי קָדְשְׁךָ כָּתוּב לֵאמֹר: <small>קהל וחזן:</small> <b>יִמְלֹךְ יְהֹוָה לְעוֹלָם אֱלֹהַֽיִךְ צִיּוֹן לְדֹר וָדֹר:</b> הַלְ֒לוּיָהּ:',
    ),
  );
  parts.push(
    p(
      "<b>קדושת השם</b> <b>אַתָּה קָדוֹשׁ</b> וְשִׁמְךָ קָדוֹשׁ וּקְדוֹשִׁים בְּכָל יוֹם יְהַלְ֒לֽוּךָ סֶּֽלָה: כִּי אֵל מֶֽלֶךְ גָּדוֹל וְקָדוֹשׁ אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה הָאֵל הַקָּדוֹשׁ:",
    ),
  );
  if (context2.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p(
      "<b>בינה</b> <b>אַתָּה חוֹנֵן</b> לְאָדָם דַּֽעַת וּמְלַמֵּד לֶאֱנוֹשׁ בִּינָה: חָנֵּֽנוּ מֵאִתְּ֒ךָ חָכְמָה בִּינָה וָדָּעַת: בָּרוּךְ אַתָּה יְהֹוָה חוֹנֵן הַדָּֽעַת:",
    ),
  );
  parts.push(
    p(
      "<b>תשובה</b> <b>הֲשִׁיבֵֽנוּ</b> אָבִֽינוּ לְתוֹרָתֶֽךָ וְקָרְ֒בֵֽנוּ מַלְכֵּֽנוּ לַעֲבוֹדָתֶֽךָ וְהַחֲזִירֵֽנוּ בִּתְשׁוּבָה שְׁלֵמָה לְפָנֶֽיךָ: בָּרוּךְ אַתָּה יְהֹוָה הָרוֹצֶה בִּתְשׁוּבָה:",
    ),
  );
  parts.push(
    p(
      "<b>סליחה</b> <b>סְלַח לָֽנוּ</b> אָבִֽינוּ כִּי חָטָֽאנוּ מְחַל לָֽנוּ מַלְכֵּֽנוּ כִּי פָשָֽׁעְנוּ כִּי אֵל טוֹב וְסַלָח אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה חַנּוּן הַמַּרְבֶּה לִסְלֽוֹחַ:",
    ),
  );
  parts.push(
    p(
      "<b>גאולה</b> <b>רְאֵה</b> נָא בְעָנְיֵֽנוּ וְרִֽיבָה רִיבֵֽנוּ וּגְאָלֵֽנוּ גְאוּלָה שְׁלֵמָה מְהֵרָה לְמַֽעַן שְׁמֶֽךָ כִּי אֵל גּוֹאֵל חָזָק אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה גּוֹאֵל יִשְׂרָאֵל:",
    ),
  );
  ...
  parts.push(
    p(
      "<b>רפואה</b> <b>רְפָאֵֽנוּ</b> יְהֹוָה וְנֵרָפֵא הוֹשִׁיעֵֽנוּ וְנִוָּשֵֽׁעָה כִּי תְהִלָּתֵֽנוּ אָֽתָּה וְהַעֲלֵה אֲרוּכָה וּמַרְפֵּא לְכָל תַּחֲלוּאֵֽינוּ וּלְכָל מַכְאוֹבֵֽינוּ וּלְכָל מַכּוֹתֵֽינוּ",
    ),
  );
  parts.push(sup("<small>מי שרוצה להתפלל על החולה יאמר כאן תחנה זו:</small>"));
  parts.push(
    p(
      "<small>יְהִי רָצוֹן מִלְפָנֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ שֶׁתִּשְׁלַח מְהֵרָה רְפוּאָה שְׁלֵמָה מִן הַשָּׁמַֽיִם, רְפוּאַת הַנֶּֽפֶשׁ וּרְפוּאַת הַגּוּף, לַחוֹלֶה (פלוני/פלונית) בֶּן/בַּת (פלונית) בְּתוֹךְ שְׁאָר חוֹלֵי יִשְׂרָאֵל:</small> כִּי אֵל מֶֽלֶךְ רוֹפֵא נֶאֱמָן וְרַחֲמָן אָֽתָּה: בָּרוּךְ אַתָּה יְהֹוָה רוֹפֵא חוֹלֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  parts.push(
    p(
      "<b>ברכת השנים</b> <b>בָּרֵךְ</b> עָלֵֽינוּ יְהֹוָה אֱלֹהֵֽינוּ אֶת־הַשָּׁנָה הַזֹּאת וְאֶת־כָּל־מִינֵי תְבוּאָתָהּ לְטוֹבָה, וְתֵן",
    ),
  );
  if (isSummer) {
  ...
  parts.push(
    p(
      "עַל־פְּנֵי הָאֲדָמָה וְשַׂבְּ֒עֵֽנוּ מִטּוּבֶֽךָ וּבָרֵךְ שְׁנוֹתֵנוּ כַּשָּׁנִים הַטּוֹבוֹת לִבְרָכָה כִּי אֵל טוֹב וּמֵטִיב אַתָּה וּמְבָרֵךְ הַשָּׁנִים: בָּרוּךְ אַתָּה יְהֹוָה מְבָרֵךְ הַשָּׁנִים:",
    ),
  );
  if (!isSummer) {
    parts.push(
  ...
  parts.push(
    p(
      "<b>קיבוץ גליות</b> <b>תְּקַע</b> בְּשׁוֹפָר גָּדוֹל לְחֵרוּתֵֽנוּ וְשָׂא נֵס לְקַבֵּץ גָּלֻיּוֹתֵֽינוּ וְקַבְּ֒צֵֽנוּ יַֽחַד מְהֵרָה מֵאַרְבַּע כַּנְפוֹת הָאָֽרֶץ לְאַרְצֵֽנוּ: בָּרוּךְ אַתָּה יְהֹוָה מְקַבֵּץ נִדְחֵי עַמּוֹ יִשְׂרָאֵל:",
    ),
  );
  parts.push(
    p(
      "<b>דין</b> <b>הָשִֽׁיבָה</b> שׁוֹפְ֒טֵֽינוּ כְּבָרִאשׁוֹנָה וְיוֹעֲצֵֽינוּ כְּבַתְּ֒חִלָּה וְהָסֵר מִמֶּֽנּוּ יָגוֹן וַאֲנָחָה וּמְלוֹךְ עָלֵֽינוּ מְהֵרָה אַתָּה יְהֹוָה לְבַדְּ֒ךָ בְּחֶֽסֶד וּבְרַחֲמִים וְצַדְּ֒קֵֽנוּ בְּצֶֽדֶק וּבְמִשְׁפָּט: בָּרוּךְ אַתָּה יְהֹוָה מֶֽלֶךְ אֹהֵב צְדָקָה וּמִשְׁפָּט:",
    ),
  );
  if (context2.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p(
      "<b>ברכת המינים</b> <b>וְלַמַּלְשִׁינִים</b> אַל תְּהִי תִקְוָה וְכָל הָרִשְׁעָה כְּרֶגַע תֺּאבֵד וְכָל־אֹיְ֒בֶֽיךָ מְהֵרָה יִכָּרֵֽתוּ, וְהַזֵּדִים מְהֵרָה תְעַקֵּר וּתְשַׁבֵּר וּתְמַגֵּר וּתְכַלֵם וְתַשְׁפִּילֵם וְתַכְנִיעֵם בִּמְהֵרָה בְיָמֵֽינוּ: בָּרוּךְ אַתָּה יְהֹוָה שׁוֹבֵר אֹיְ֒בִים וּמַכְנִֽיעַ זֵדִים:",
    ),
  );
  parts.push(
    p(
      "<b>צדיקים</b> <b>עַל־הַצַּדִּיקִים</b> וְעַל־הַחֲסִידִים וְעַל־זִקְנֵי עַמְּ֒ךָ בֵּית יִשְׂרָאֵל וְעַל־פְּלֵיטַת (בֵּית) סוֹפְ֒רֵיהֶם וְעַל־גֵּרֵי הַצֶּֽדֶק וְעָלֵֽינוּ יֶהֱמוּ נָא רַחֲמֶֽיךָ יְהֹוָה אֱלֹהֵֽינוּ וְתֵן שָׂכָר טוֹב לְכָל־הַבּוֹטְ֒חִים בְּשִׁמְךָ בֶּאֱמֶת וְשִׂים חֶלְקֵֽנוּ עִמָּהֶם וּלְעוֹלָם לֹא נֵבוֹשׁ כִּי בְךָ בָטָֽחְנוּ: בָּרוּךְ אַתָּה יְהֹוָה מִשְׁעָן וּמִבְטָח לַצַּדִּיקִים:",
    ),
  );
  parts.push(
    p(
      "<b>בנין ירושלים</b> <b>וְלִירוּשָׁלַֽיִם</b> עִירְ֒ךָ בְּרַחֲמִים תָּשׁוּב וְתִשְׁכּוֹן בְּתוֹכָהּ כַּאֲשֶׁר דִּבַּֽרְתָּ וּבְנֵה אוֹתָהּ בְּקָרוֹב בְּיָמֵֽינוּ בִּנְיַן עוֹלָם וְכִסֵּא דָוִד עַבְדֶּךָ, מְהֵרָה לְתוֹכָהּ תָּכִין: בָּרוּךְ אַתָּה יְהֹוָה בּוֹנֵה יְרוּשָׁלָֽיִם:",
    ),
  );
  parts.push(
    p(
      "<b>מלכות בית דוד</b> <b>אֶת־צֶֽמַח</b> דָּוִד עַבְדְּ֒ךָ מְהֵרָה תַצְמִֽיחַ וְקַרְנוֹ תָּרוּם בִּישׁוּעָתֶֽךָ, כִּי לִישׁוּעָתְ֒ךָ קִוִּינוּ כָּל הַיּוֹם וּמְצַפִּים לִישׁוּעָה: בָּרוּךְ אַתָּה יְהֹוָה מַצְמִֽיחַ קֶֽרֶן יְשׁוּעָה:",
    ),
  );
  parts.push(
    p(
      "<b>קבלת תפלה</b> <b>אָב הָרַחֲמָן</b> שְׁמַע קוֹלֵֽנוּ יְהֹוָה אֱלֹהֵֽינוּ חוּס וְרַחֵם עָלֵֽינוּ וְקַבֵּל בְּרַחֲמִים וּבְרָצוֹן אֶת תְּפִלָּתֵֽנוּ, כִּי אֵל שׁוֹמֵֽעַ תְּפִלּוֹת וְתַחֲנוּנִים אָֽתָּה וּמִלְּ֒פָנֶֽיךָ מַלְכֵּֽנוּ רֵיקָם אַל תְּשִׁיבֵֽנוּ, חָנֵּֽנוּ וַעֲנֵֽנוּ וּשְׁמַע תְּפִלָתֵֽנוּ* כִּי אַתָּה שׁוֹמֵֽעַ תְּפִלַּת כָּל פֶּה עַמְּ֒ךָ יִשְׂרָאֵל בְּרַחֲמִים: בָּרוּךְ אַתָּה יְהֹוָה שׁוֹמֵֽעַ תְּפִלָּה:",
    ),
  );
  parts.push(
    sup(
      "<small>* תפלה שיתפלל על מזונו קודם כִּי אַתָּה:</small> <small>אַתָּה הוּא יְהֺוָה הָאֱלֹהִים הַזָּן וּמְפַרְנֵס וּמְכַלְכֵּל מִקַּרְנֵי רְאֵמִים עַד בֵּיצֵי כִנִּים. הַטְרִפֵֽנִי לֶֽחֶם חֻקִּי וְהַמְצֵא לִי וּלְכָל בְּנֵי בֵיתִי מְזוֹנוֹתַי קֽוֹדֶם שֶׁאֶצְטָרֵךְ לָהֶם, בְּנַֽחַת וְלֹא בְצַֽעַר, בְּהֶתֵּר וְלֹא בְּאִסּוּר, בְּכָבוֹד וְלֹא בְּבִזָּיוֹן, לְחַיִּים וּלְשָׁלוֹם, מִשֶּֽׁפַע בְּרָכָה וְהַצְלָחָה, וּמִשֶּֽׁפַע בְּרֵכָה עֶלְיוֹנָה, כְּדֵי שֶׁאוּכַל לַעֲשׂוֹת רְצוֹנֶֽךָ וְלַעֲסוֹק בְּתוֹרָתֶֽךָ וּלְקַיֵּם מִצְוֺתֶֽיךָ. וְאַל תַּצְרִיכֵֽנִי לִידֵי מַתְּ֒נַת בָּשָׂר וָדָם. וִיקֻיַּם בִּי מִקְרָא שֶׁכָּתוּב פּוֹתֵֽחַ אֶת יָדֶֽךָ וּמַשְׂבִּיעַ לְכָל חַי רָצוֹן. וְכָתוּב הַשְׁלֵךְ עַל יְהֺוָה יְהָבְ֒ךָ וְהוּא יְכַלכְּךָ:</small>",
    ),
  );
  parts.push(p(""));
  parts.push(
    p(
      "<b>עבודה</b> <b>רְצֵה</b> יְהֹוָה אֱלֹהֵֽינוּ בְּעַמְּ֒ךָ יִשְׂרָאֵל וְלִתְפִלָּתָם שְׁעֵה, וְהָשֵׁב אֶת הָעֲבוֹדָה לִדְבִיר בֵּיתֶֽךָ וְאִשֵּׁי יִשְׂרָאֵל, וּתְפִלָּתָם בְּאַהֲבָה תְקַבֵּל בְּרָצוֹן וּתְהִי לְרָצוֹן תָּמִיד עֲבוֹדַת יִשְׂרָאֵל עַמֶּֽךָ:",
    ),
  );
  if (context2.isRoshChodesh || context2.isCholHamoed) {
    var sfaradYaalehFestival = context2.isRoshChodesh
      ? '<small>לר"ח:</small> רֹאשׁ הַחֹֽדֶשׁ הַזֶּה'
      : context2.isPesach
        ? "<small>לפסח:</small> חַג הַמַּצּוֹת הַזֶּה"
        : context2.isSukkot
          ? "<small>לסכות:</small> חַג הַסֻּכּוֹת הַזֶּה"
          : "";
    parts.push(
      sup(
        "<small>בראש חדש ובחול המועד אומרים זה:</small> <b>אֱלֹהֵֽינוּ</b> וֵאלֹהֵי אֲבוֹתֵֽינוּ יַעֲלֶה וְיָבֹא וְיַגִּֽיעַ וְיֵרָאֶה וְיֵרָצֶה וְיִשָּׁמַע וְיִפָּקֵד וְיִזָּכֵר זִכְרוֹנֵֽנוּ וּפִקְדוֹנֵֽנוּ וְזִכְרוֹן אֲבוֹתֵֽינוּ וְזִכְרוֹן מָשִֽׁיחַ בֶּן דָּוִד עַבְדֶּֽךָ וְזִכְרוֹן יְרוּשָׁלַֽיִם עִיר קָדְשֶֽׁךָ וְזִכְרוֹן כָּל עַמְּ֒ךָ בֵּית יִשְׂרָאֵל לְפָנֶֽיךָ, לִפְלֵיטָה לְטוֹבָה לְחֵן וּלְחֶֽסֶד וּלְרַחֲמִים וּלְחַיִּים טוֹבִים וּלְשָׁלוֹם בְּיוֹם" +
          " " +
          sfaradYaalehFestival +
          " " +
          "זָכְרֵֽנוּ יְהֹוָה אֱלֹהֵֽינוּ בּוֹ לְטוֹבָה, וּפָקְדֵֽנוּ בוֹ לִבְרָכָה, וְהוֹשִׁיעֵֽנוּ בוֹ לְחַיִּים טוֹבִים, וּבִדְבַר יְשׁוּעָה וְרַחֲמִים חוּס וְחָנֵּֽנוּ, וְרַחֵם עָלֵֽינוּ וְהוֹשִׁיעֵֽנוּ, כִּי אֵלֶֽיךָ עֵינֵֽינוּ, כִּי אֵל מֶֽלֶךְ חַנּוּן וְרַחוּם אָֽתָּה:" +
          " " +
          "<small>שכח ולא אמר יעלה ויבא אם נזכר קודם שאמר יהיו לרצון חוזר ומתחיל רצה. ואפלו אם נזכר קודם שהתחיל מודים כיון שסים ברכת המחזיר שכינתו לציון צריך להתחיל רצה. אך אם נזכר קודם ברכת המחזיר שכינתו לציון אומרו שם ומסים ותחזינה עינינו וכו'. ואם לא נזכר עד לאחר יהיו לרצון וגו' חוזר לראש התפלה.</small>",
      ),
    );
  }
  parts.push(
    p(
      "<b>וְתֶחֱזֶֽינָה</b> עֵינֵֽינוּ בְּשׁוּבְ֒ךָ לְצִיּוֹן בְּרַחֲמִים: בָּרוּךְ אַתָּה יְהֹוָה הַמַּחֲזִיר שְׁכִינָתוֹ לְצִיּוֹן: <b>הודאה</b>",
    ),
  );
  parts.push(
    sup(
  ...
  parts.push(
    p(
      "<b>וְעַל־כֻּלָּם</b> יִתְבָּרַךְ וְיִתְרוֹמַם וְיִתְנַשֵׂא שִׁמְךָ מַלְכֵּֽנוּ תָּמִיד לְעוֹלָם וָעֶד:",
    ),
  );
  if (context2.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p(
      "<b>וְכֹל הַחַיִּים</b> יוֹדֽוּךָ סֶּֽלָה וִיהַלְ֒לוּ וְיבָרְ֒כוּ אֶת־שִׁמְךָ הַגָּדוֹל בְּאֱמֶת לְעוֹלָם כִּי טוֹב הָאֵל יְשׁוּעָתֵֽנוּ וְעֶזְרָתֵֽנוּ סֶֽלָה הָאֵל הַטּוֹב: בָּרוּךְ אַתָּה יְהֹוָה הַטּוֹב שִׁמְךָ וּלְךָ נָאֶה לְהוֹדוֹת:",
    ),
  );
  parts.push(
    sup(
      "<small>ברכת כהנים</small> <small>שליח ציבור:</small> אֱלֹהֵֽינוּ וֵאלֹהֵי אֲבוֹתֵֽינוּ בָּרְ֒כֵֽנוּ בַבְּ֒רָכָה הַמְּ֒שֻׁלֶּֽשֶׁת בַּתּוֹרָה הַכְּ֒תוּבָה עַל יְדֵי משֶׁה עַבְדֶּֽךָ, הָאֲמוּרָה מִפִּי אַהֲרֹן וּבָנָיו כֹּהֲנִים עַם קְדוֹשֶֽׁךָ כָּאָמוּר: יְבָרֶכְ֒ךָ יְהֹוָה וְיִשְׁמְ֒רֶֽךָ: יָאֵר יְהֹוָה פָּנָיו אֵלֶֽיךָ וִיחֻנֶּֽךָּ: יִשָּׂא יְהֹוָה פָּנָיו אֵלֶֽיךָ וְיָשֵׂם לְךָ שָׁלוֹם: <small>לאחר ברכת כהנים יאמרו הציבור:</small> <small>אַדִּיר בַּמָּרוֹם שׁוֹכֵן בִּגְ֒בוּרָה, אַתָּה שָׁלוֹם וְשִׁמְךָ שָׁלוֹם. יְהִי רָצוֹן שֶׁתָּשִׂים עָלֵֽינוּ וְעַל כָּל עַמְּ֒ךָ בֵּית יִשְׂרָאֵל חַיִּים וּבְרָכָה לְמִשְׁמֶֽרֶת שָׁלוֹם:</small>",
    ),
  );
  parts.push(
    p(
      "<b>שלום</b> <b>שִׂים שָׁלוֹם</b> טוֹבָה וּבְרָכָה חַיִים חֵן וָחֶֽסֶד וְרַחֲמִים עָלֵֽינוּ וְעַל כָּל־יִשְׂרָאֵל עַמֶּֽךָ, בָּרְ֒כֵֽנוּ אָבִֽינוּ כֻּלָּֽנוּ כְּאֶחָד בְּאוֹר פָּנֶֽיךָ כִּי בְאוֹר פָּנֶֽיךָ נָתַֽתָּ לָּֽנוּ יְהֹוָה אֱלֹהֵֽינוּ תּוֹרַת חַיִּים וְאַהֲבַת חֶֽסֶד וּצְדָקָה וּבְרָכָה וְרַחֲמִים וְחַיִּים וְשָׁלוֹם, וְטוֹב יִהְיֶה בְּעֵינֶֽיךָ לְבָרְ֒כֵֽנוּ וּלְבָרֵךְ אֶת־כָּל־עַמְּ֒ךָ יִשְׂרָאֵל בְּכָל־עֵת וּבְכָל־שָׁעָה בִּשְׁלוֹמֶֽךָ (בְּרוֹב עוֹז וְשָׁלוֹם).",
    ),
  );
  if (context2.isAseretYemeiTeshuva) {
    parts.push(
  ...
  parts.push(
    p(
      "בָּרוּךְ אַתָּה יְהֹוָה הַמְבָרֵךְ אֶת־עַמּוֹ יִשְׂרָאֵל בַּשָּׁלוֹם: יִהְיוּ לְרָצוֹן אִמְ֒רֵי פִי וְהֶגְיוֹן לִבִּי לְפָנֶֽיךָ יְהֹוָה צוּרִי וְגוֹאֲלִי:",
    ),
  );
  parts.push(
    p(
      "<b>אֱלֹהַי</b> נְצוֹר לְשׁוֹנִי מֵרָע וּשְׂפָתַי מִדַּבֵּר מִרְמָה וְלִמְקַלְ֒לַי נַפְשִׁי תִדּוֹם וְנַפְשִׁי כֶּעָפָר לַכֹּל תִּהְיֶה פְּתַח לִבִּי בְּתוֹרָתֶֽךָ וְאַחֲרֵי מִצְוֹתֶֽיךָ תִּרְדּוֹף נַפְשִׁי וְכָל הַקָמִים וְהַחוֹשְׁ֒בִים עָלַי רָעָה מְהֵרָה הָפֵר עֲצָתָם וְקַלְקֵל מַחֲשַׁבְתָּם:",
    ),
  );
  parts.push(
    sup(
      "(יְהִי רָצוֹן מִלְפָנֶיךָ יְהֹוָה אֱלֹהַי וֵאלֹהֵי אֲבוֹתַי שֶׁלֹּא תַעֲלֶה קִנְאַת אָדָם עָלַי וְלֹא קִנְאָתִי עַל אֲחֵרִים וְשֶׁלֹּא אֶכְעֺס הַיּוֹם וְשֶׁלֹּא אַכְעִיסֶֽךָ וְתַצִּילֵנִי מִיֵּֽצֶר הָרָע וְתֵן בְּלִבִּי הַכְנָעָה וַעֲנָוָה. מַלְכֵּֽנוּ וֵאלֹהֵֽינוּ יַחֵד שִׁמְךָ בְּעוֹלָמֶֽךָ, בְּנֵה עִירְ֒ךָ, יַסֵּד בֵּיתֶֽךָ, וְשַׁכְלֵל הֵיכָלֶֽךָ. וְקַבֵּץ קִבּוּץ גָּלֻיּוֹת, וּפְדֵה צֺאנֶֽךָ וְשַׂמַּח עֲדָתֶֽךָ.)",
    ),
  );
  parts.push(
    p(
  ...
  if (isHallelDay) {
    ...
    hl.push(
      p(
        "<small>מברכים לפני הלל בכל יום. חצי הלל: מדלגים לֹא לָנוּ וְאָהַבְתִּי. הלל שלם: אומרים הכל.</small>",
      ),
    );
    hl.push(
      p(
        "<b>בָּרוּךְ</b> אַתָּה יְהֹוָה אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם אֲשֶׁר קִדְּשָׁנוּ בְּמִצְוֹתָיו וְצִוָּנוּ לִגְמוֹר אֶת הַהַלֵּל:",
      ),
    );
    hl.push(
      p(
    ...
    hl.push(
      p(
        "<small><b>יְהַלְלוּךָ</b> יְהֹוָה אֱלֹהֵינוּ כָּל מַעֲשֶׂיךָ וַחֲסִידֶיךָ וְצַדִּיקִים עוֹשֵׂי רְצוֹנֶךָ וְכָל עַמְּךָ בֵּית יִשְׂרָאֵל בְּרִנָּה יוֹדוּ וִיבָרְכוּ וִישַׁבְּחוּ וִיפָאֲרוּ וִירוֹמְמוּ וְיַעֲרִיצוּ וְיַקְדִּישׁוּ וְיַמְלִיכוּ אֶת שִׁמְךָ מַלְכֵּנוּ כִּי לְךָ טוֹב לְהוֹדוֹת וּלְשִׁמְךָ נָאֶה לְזַמֵּר כִּי מֵעוֹלָם וְעַד עוֹלָם אַתָּה אֵל: בָּרוּךְ אַתָּה יְהֹוָה מֶלֶךְ מְהֻלָּל בַּתִּשְׁבָּחוֹת:</small>",
      ),
    );
    hl.push(
      p(
        "<small><b>יִתְגַּדַּל</b> וְיִתְקַדַּשׁ שְׁמֵהּ רַבָּא בְּעָלְמָא דִּי בְרָא כִרְעוּתֵהּ וְיַמְלִיךְ מַלְכוּתֵהּ וְיַצְמַח פֻּרְקָנֵהּ בְּחַיֵּיכוֹן וּבְיוֹמֵיכוֹן וּבְחַיֵּי דְכָל בֵּית יִשְׂרָאֵל בַּעֲגָלָא וּבִזְמַן קָרִיב וְאִמְרוּ אָמֵן: יְהֵא שְׁמֵהּ רַבָּא מְבָרַךְ לְעָלַם וּלְעָלְמֵי עָלְמַיָּא: תִּתְקַבַּל צְלוֹתְהוֹן וּבָעוּתְהוֹן דְּכָל בֵּית יִשְׂרָאֵל קֳדָם אֲבוּהוֹן דִּי בִשְׁמַיָּא וְאִמְרוּ אָמֵן: עֹשֶׂה שָׁלוֹם בִּמְרוֹמָיו הוּא יַעֲשֶׂה שָׁלוֹם עָלֵינוּ וְעַל כָּל יִשְׂרָאֵל וְאִמְרוּ אָמֵן:</small>",
      ),
    );
    parts.push(sup(hl.join("")));
  }
  ...
  parts.push(
    p(
      "<b>רַבָּן</b> שִׁמְעוֹן בֶּן גַּמְלִיאֵל אוֹמֵר הַצֳּרִי אֵינוֹ אֶלָּא שְׂרָף הַנּוֹטֵף מֵעֲצֵי הַקְּ֒טָף. בֹּרִית כַּרְשִׁינָה לָמָּה הִיא בָאָה כְּדֵי לְיַפּוֹת בָּהּ אֶת הַצִּפֹּרֶן כְּדֵי שֶׁתְּ֒הֵא נָאָה. יֵין קַפְרִיסִין לָמָּה הוּא בָא כְּדֵי לִשְׁרוֹת בּוֹ אֶת הַצִּפֹּרֶן כְּדֵי שֶׁתְּ֒הֵא עַזָּה. וַהֲלֹא מֵי רַגְלַיִם יָפִין לָהּ אֶלָּא שֶׁאֵין מַכְנִיסִין מֵי רַגְלַיִם בַּמִּקְדָּשׁ מִפְּ֒נֵי הַכָּבוֹד:",
    ),
  );
  parts.push(
    p(
  ...
  parts.push(
    p(
      "<b>תַּנְיָא</b> בַּר קַפָּרָא אוֹמֵר אַחַת לְשִׁשִּׁים אוֹ לְשִׁבְעִים שָׁנָה הָיְ֒תָה בָאָה שֶׁל שִׁירַֽיִם לַחֲצָאִין: וְעוֹד תָּנֵי בַּר קַפָּרָא אִלּוּ הָיָה נוֹתֵן בָּהּ קוֹרְ֒טוֹב שֶׁל דְּבַשׁ אֵין אָדָם יָכוֹל לַעֲמֹד מִפְּ֒נֵי רֵיחָהּ וְלָֽמָּה אֵין מְעָרְ֒בִין בָּהּ דְּבַשׁ מִפְּ֒נֵי שֶׁהַתּוֹרָה אָמְ֒רָה כִּי כָל־שְׂאֹר וְכָל־דְּבַשׁ לֹא־תַקְטִֽירוּ מִמֶּֽנּוּ אִשֶּׁה לַיהוָֹה: <small>אומר ג' פעמים:</small> יְהֹוָה צְבָאוֹת עִמָּֽנוּ מִשְׂגַּב־לָֽנוּ אֱלֹהֵי יַעֲקֹב סֶֽלָה: יְהֹוָה צְבָאוֹת אַשְׁרֵי אָדָם בֹּטֵֽחַ בָּךְ: יְהֹוָה הוֹשִֽׁיעָה הַמֶּֽלֶךְ יַעֲנֵֽנוּ בְיוֹם־קָרְאֵֽנוּ: אַתָּה סֵֽתֶר לִי מִצַּר תִּצְּ֒רֵֽנִי רָנֵּי פַלֵּט תְּסוֹבְ֒בֵֽנִי סֶֽלָה: וְעָרְ֒בָה לַיהוָֹה מִנְחַת יְהוּדָה וִירוּשָׁלָֽםִ כִּימֵי עוֹלָם וּכְשָׁנִים קַדְמֹנִיּוֹת: תָּנָא דְבֵי אֵלִיָּֽהוּ כָּל הַשּׁוֹנֶה הֲלָכוֹת בְּכָל יוֹם מֻבְטָח לוֹ שֶׁהוּא בֶּן עוֹלָם הַבָּא שֶׁנֶּאֱמַר הֲלִיכוֹת עוֹלָם לוֹ אַל תִּקְרֵי הֲלִיכוֹת אֶלָּא הֲלָכוֹת:",
    ),
  );
  parts.push(
    p(
      "<b>אָמַר</b> רַבִּי אֶלְעָזָר אָמַר רַבִּי חֲנִינָא. תַּלְמִידֵי חֲכָמִים מַרְבִּים שָׁלוֹם בָּעוֹלָם. שֶׁנֶּאֱמַר וְכָל בָּנַֽיִךְ לִמּוּדֵי יְהֹוָה וְרַב שְׁלוֹם בָּנָֽיךְ: אַל תִּקְרֵי בָּנָֽיךְ אֶלָּא בּוֹנָֽיךְ: שָׁלוֹם רָב לְאֹהֲבֵי תוֹרָתֶֽךָ וְאֵין לָֽמוֹ מִכְשׁוֹל: יְהִי שָׁלוֹם בְּחֵילֵךְ שַׁלְוָה בְּאַרְמְּנוֹתָֽיךְ לְמַֽעַן אַחַי וְרֵעָי אֲדַבְּרָה נָא שָׁלוֹם בָּךְ: לְמַֽעַן בֵּית יְהֹוָה אֱלֹהֵֽינוּ אֲבַקְ֒שָׁה טוֹב לָךְ: <small>קדיש דרבנן:</small>",
    ),
  );
  parts.push(
    p(
  ...
  return {
    html: '<div class="prayer-richtext">' + parts.join("") + "</div>",
    sourceLabel: "מאגר פנימי – נוסח ספרד (תורת אמת)",
    sourceUrl: "https://www.toratemetfreeware.com/online/f_01774.html",
  };
}
...
async function getFullPrayerContent(key) {
  ...
  let sourcePriority = PRAYER_SOURCE_PRIORITY[key] || "wikisource-first";
  const attempts = [];
  if (key === "al-hamichya") {
  ...
  if (sourcePriority === "full-sefaria-compose") {
    ...
    sourcePriority !== "local-first" &&
    sourcePriority !== "local-only" &&
    key !== "al-hamichya"
  ) {
    pushWikisourceQueries();
    pushSefariaRefs();
  } else if (sourcePriority !== "local-only") {
    pushWikisourceQueries();
    pushSefariaRefs();
  }
