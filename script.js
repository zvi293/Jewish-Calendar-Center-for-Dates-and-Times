...
function buildShacharitAshkenazPayload(context) {
  ...
  parts.push(
    p(
      "<b>אַתָּה גִבּוֹר</b> לְעוֹלָם אֲדֹנָי מְחַיֵּה מֵתִים אַתָּה רַב לְהוֹשִֽׁיעַ:",
    ),
  );
  if (isSummer) {
    parts.push(p("מוֹרִיד הַטָּל"));
  ...
  if (isHallelDay) {
    ...
    parts.push(
      p(
        "<small>אומרים הלל שלם בחגים ובחנוכה, חצי הלל בראשי חודשים ובאחרון של פסח</small>",
      ),
    );
    parts.push(hr);
  }
